...
@symbol(name = "layout_forceatlas2", kind = ISymbolKind.SINGLE_STATEMENT, with_sequence = false, doc = @doc("Apply Force Atlas 2 Layout."))
...
@facets(value = {
	...
	@facet(name = GraphForceAtlasLayoutStatement.PREVENT_OVERLAP, type =  IType.BOOL, optional = true, doc = @doc("Should not be used with approximate_repulsion default: false")),
	@facet(name = GraphForceAtlasLayoutStatement.EDGE_WEIGHT_INFLUENCE, type =  IType.FLOAT, optional = true, doc = @doc("How much influence you give to the edges wight. 0 is no influence, 1 is normal. default: 1.0")),
	@facet(name = GraphForceAtlasLayoutStatement.SCALING, type =  IType.FLOAT, optional = true, doc = @doc("How much repulsion you want. More makes a more sparse graph. default: 2.0")),
	@facet(name = GraphForceAtlasLayoutStatement.STRONGER_GRAVITY, type =  IType.BOOL, optional = true, doc = @doc("A stronger gravity law default: false")),
	@facet(name = GraphForceAtlasLayoutStatement.GRAVITY, type =  IType.FLOAT, optional = true, doc = @doc("Attracts nodes to the center. Prevents islands from drifting away. default: 1.0")),
	@facet(name = GraphForceAtlasLayoutStatement.TOLERANCE, type =  IType.FLOAT, optional = true, doc = @doc("How much swinging you allow. Above 1 discouraged. Lower gives less speed and more precision. default: 0.1")),
	@facet(name = GraphForceAtlasLayoutStatement.APPROXIMATE_REPULSION, type =  IType.BOOL, optional = true, doc = @doc("Barnes Hut optimization: n2 complexity to n.ln(n); allows larger graphs. default: false")),
	@facet(name = GraphForceAtlasLayoutStatement.APPROXIMATION, type =  IType.FLOAT, optional = true, doc = @doc("Theta of the Barnes Hut optimization. default: 1.2")),
	@facet(name = GraphForceAtlasLayoutStatement.BOUNDED_P1, type =  IType.POINT, optional = true, doc = @doc("The new nodes positions are bounded within the two bound points if both are not null. default: null")),
	@facet(name = GraphForceAtlasLayoutStatement.BOUNDED_P2, type =  IType.POINT, optional = true, doc = @doc("The new nodes positions are bounded within the two bound points if both are not null. default: null"))},
    omissible = IKeyword.GRAPH)
public class GraphForceAtlasLayoutStatement extends AbstractGraphLayoutStatement {	

	public GraphForceAtlasLayoutStatement(IDescription desc) {
		super(desc);
...
//		IGraph data = createData(scope);
		...
		int thread_number=Cast.asInt(scope, getFacetValue(scope, GraphForceAtlasLayoutStatement.THREAD_NUMBER,THREAD_NUMBER_DEFAULT));
		boolean dissuade_hubs=Cast.asBool(scope, getFacetValue(scope, GraphForceAtlasLayoutStatement.DISSUADE_HUBS,DISSUADE_HUBS_DEFAULT));
		boolean linlog_mode=Cast.asBool(scope, getFacetValue(scope, GraphForceAtlasLayoutStatement.LINLOG_MODE,LINLOG_MODE_DEFAULT));
		boolean prevent_overlap=Cast.asBool(scope, getFacetValue(scope, GraphForceAtlasLayoutStatement.PREVENT_OVERLAP,PREVENT_OVERLAP_DEFAULT));
		double edge_weight_influence=Cast.asFloat(scope, getFacetValue(scope, GraphForceAtlasLayoutStatement.EDGE_WEIGHT_INFLUENCE,EDGE_WEIGHT_INFLUENCE_DEFAULT));
		double scaling=Cast.asFloat(scope, getFacetValue(scope, GraphForceAtlasLayoutStatement.SCALING,SCALING_DEFAULT));
		boolean stronger_gravity=Cast.asBool(scope, getFacetValue(scope, GraphForceAtlasLayoutStatement.STRONGER_GRAVITY,STRONGER_GRAVITY_DEFAULT));
		double gravity=Cast.asFloat(scope, getFacetValue(scope, GraphForceAtlasLayoutStatement.GRAVITY,GRAVITY_DEFAULT));
