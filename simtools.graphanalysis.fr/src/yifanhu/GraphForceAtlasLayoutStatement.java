...
@symbol(name = "layout_forceatlas2", kind = ISymbolKind.SINGLE_STATEMENT, concept = {
		IConcept.GRAPH }, with_sequence = false, doc = @doc("Apply Force Atlas 2 Layout."))
...
@facets(value = {
		...
		@facet(name = GraphForceAtlasLayoutStatement.BOUNDED_P2, type = IType.POINT, optional = true, doc = @doc("The new nodes positions are bounded within the two bound points if both are not null. default: null")) }, omissible = IKeyword.GRAPH)
public class GraphForceAtlasLayoutStatement extends AbstractGraphLayoutStatement {
	...
	protected Object privateExecuteIn(final IScope scope) throws GamaRuntimeException {
		...
		final double scaling = Cast.asFloat(scope,
				getFacetValue(scope, GraphForceAtlasLayoutStatement.SCALING, SCALING_DEFAULT));
		...
		final double gravity = Cast.asFloat(scope,
				getFacetValue(scope, GraphForceAtlasLayoutStatement.GRAVITY, GRAVITY_DEFAULT));
		final double tolerance = Cast.asFloat(scope,
		...
		final int nb_steps = Cast.asInt(scope,
				getFacetValue(scope, GraphForceAtlasLayoutStatement.NB_STEPS, NB_STEPS_DEFAULT));
