...
global {
	...
	float survivalProbability <- 1 / ( 70 * 365 ) parameter :
	'Survival Probability' ; // The survival probability
	...
	bool local_infection <- true parameter :
	'Is the infection is computed locally?' ;
	...
	reflex compute_distance {
		set distance <- distance + abs ( ( ( Host as list ) count ( each .
		is_susceptible ) / SimNumber ) - first ( my_SIR_maths ) . S ) ;
	}
}
environment width : 50 height : 50 {
	grid sir_grid width : 50 height : 50 {
		rgb color <- rgb ( 'black' ) ;
		list neighbours of : sir_grid <- ( self neighbours_at neighbours_size )
		of_species sir_grid ;
	}
}
entities {
	species Host skills : [ moving ] {
		...
		int sic_count <- 0 ;
		sir_grid myPlace ;
		int ngb_number function: { ( ( myPlace . neighbours + myPlace ) accumulate (
		each . agents ) ) of_species Host count ( each . simulationID = simulationID
		) - 1 // -1 is because the agent counts itself
		} ;
		init {
		...
		reflex become_infected when : is_susceptible {
			let hst_ngb <- ( ( ( myPlace . neighbours + myPlace ) accumulate ( each .
			agents ) ) of_species Host ) where ( each . simulationID = simulationID ) ;
...
experiment simulation type : gui {
	...
	//	    }
		display chart refresh_every : 1 {
			chart 'Susceptible' type : series background : rgb ( 'lightGray' ) style :
			exploded {
				data susceptible value : ( Host as list ) count ( each . is_susceptible )
				/ SimNumber color : rgb ( 'green' ) ;
				data infected value : ( Host as list ) count ( each . is_infected ) /
				SimNumber color : rgb ( 'red' ) ;
				data immune value : ( Host as list ) count ( each . is_immune ) /
				SimNumber color : rgb ( 'yellow' ) ;
...
experiment BatchExhaustive type : batch repeat : 1 keep_seed : true until :
time = 6 {
