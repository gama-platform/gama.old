...
entities {
	...
	species my_SIR_maths {
		...
		float Rm ;
		equation SIR {
			diff ( self . Sm , t ) = ( - _beta * Sm * Im / N ) ; diff ( self . Im , t )
			= ( _beta * Sm * Im / N ) - ( _delta * Im ) ; diff ( self . Rm , t ) = (
			_delta * Im ) ;
		}
...
//				data infected value : sum(I accumulate (each.beta * each. Isize)) color : rgb ( 'red' ) ;
		...
		display chart_1system_eq refresh_every : 1 {
			chart 'Susceptible' type : series background : rgb ( 'lightGray' ) {
				data susceptible_maths value : first ( my_SIR_maths ) . Sm color : rgb (
				'green' ) ;
				data infected_maths value : first ( my_SIR_maths ) . Im color : rgb (
				'red' ) ;
				data recovered_maths value : first ( my_SIR_maths ) . Rm color : rgb (
				'blue' ) ;
			}
		}
	}
