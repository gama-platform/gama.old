...
public class ClassicalSEIREquations {
	...
	public List<SingleEquationStatement> SEIR(IExpression with_vars, IExpression with_params) {
		...
		eq1.setFunction(GAML.getExpressionFactory()
				.createExpr(
						"diff(" + v[0].literalValue() + ","
								+ v[4].literalValue() + ")", getDescription()));
		...
		eq4.setFunction(GAML.getExpressionFactory().createExpr("diff(" + v[3].literalValue() + "," + v[4].literalValue() + ")", getDescription()));
		eq4.setExpression(GAML.getExpressionFactory().createExpr(
				"("   + p[2].literalValue() + " * " + v[2].literalValue() + ") + " +
