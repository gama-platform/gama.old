...
@facets({ @facet(name = ISymbol.NAME, type = IType.LABEL, optional = true),
	@facet(name = ISymbol.TYPE, type = IType.TYPE_ID, optional = true),
	@facet(name = ISymbol.INIT, type = IType.NONE_STR, optional = true),
	@facet(name = ISymbol.MIN, type = IType.NONE_STR, optional = true),
	@facet(name = ISymbol.MAX, type = IType.NONE_STR, optional = true),
	@facet(name = ISymbol.CATEGORY, type = IType.LABEL, optional = true),
	@facet(name = ISymbol.VAR, type = IType.ID, optional = false),
	@facet(name = ISymbol.UNIT, type = IType.LABEL, optional = true),
	@facet(name = ISymbol.STEP, type = IType.FLOAT_STR, optional = true),
	@facet(name = ISymbol.AMONG, type = IType.LIST_STR, optional = true) })
@symbol(name = { ISymbol.PARAMETER }, kind = ISymbolKind.VARIABLE)
...
public class ExperimentParameter extends Symbol implements IParameter.Batch {
	...
	public ExperimentParameter(final IDescription sd) throws GamlException, GamaRuntimeException {
		...
		setName(desc.getFacets().getString(ISymbol.VAR));
		type = desc.getType();
		title = getLiteral(ISymbol.NAME);
		unitLabel = getLiteral(ISymbol.UNIT);
		...
		setCategory(desc.getFacets().getString(ISymbol.CATEGORY));
		IExpression init = assertFacet(desc, ISymbol.INIT, type);
		IExpression min = assertFacet(desc, ISymbol.MIN, type);
		IExpression max = assertFacet(desc, ISymbol.MAX, type);
		IExpression step = assertFacet(desc, ISymbol.STEP, type);
		IExpression among = assertFacet(desc, ISymbol.AMONG, Types.get(IType.LIST));
	...
	private Number drawRandomValue() {
		...
		if ( type.id() == IType.INT ) {
			...
			final int val = (int) (RandomAgent.getDefault().between(0., max - min) + 0.5);
