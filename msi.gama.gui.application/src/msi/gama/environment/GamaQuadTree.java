...
public class GamaQuadTree implements ISpatialIndex {
	...
	protected Collection<IAgent> findIntersects(final IGeometry source, final Envelope r,
	...
	@Override
	public GamaList<IAgent> allAtDistance(final IGeometry source, final double dist,
		final IAgentFilter f) {

		// TODO filter result by topology's bounds

		double exp = dist * MathUtils.SQRT2;
	...
	@Override
	public IAgent firstAtDistance(final IGeometry source, final double dist, final IAgentFilter f) {
		double exp = dist * MathUtils.SQRT2;
		ENVELOPE.init(source.getEnvelope());
		ENVELOPE.expandBy(exp);
	...
	@Override
	public GamaList<IAgent> allInEnvelope(final IGeometry source, final Envelope envelope,
	...
	private class QuadNode implements Serializable {
		...
		public void split() {
			if ( isLeaf ) {
				...
				for ( int i = 0; i < tempNumber; i++ ) {
					...
					if ( !entry.dead() ) {
						GamaGeometry g = entry.getGeometry();

						if ( g.isPoint() ) {
							Coordinate p = g.getLocation();
							if ( add(p, entry) ) {
								addedNumber++;
							} else {
		...
		public void findIntersects(final IGeometry source, final Envelope r, final IAgentFilter f,
		...
		public void drawOn(final Graphics2D g2, final double xr, final double yr) {
			if ( isLeaf ) {
				...
				g2.drawRect(MathUtils.round(bounds.getMinX() * xr),
					MathUtils.round(bounds.getMinY() * yr),
					MathUtils.round(bounds.getWidth() * xr),
					MathUtils.round(bounds.getHeight() * yr));
				g2.setColor(colors[Math.min(31, (int) ((double) size / (double) maxCapacity * 32))]);
				g2.fillRect(MathUtils.round(bounds.getMinX() * xr) + 1,
					MathUtils.round(bounds.getMinY() * yr) + 1,
					MathUtils.round(bounds.getWidth() * xr) - 1,
					MathUtils.round(bounds.getHeight() * yr) - 1);
