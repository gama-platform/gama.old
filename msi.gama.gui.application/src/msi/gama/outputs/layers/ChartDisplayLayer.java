 ...
 * 
 */
@symbol(name = ISymbol.CHART, kind = ISymbolKind.LAYER)
...
@facets({
	...
	@facet(name = ISymbol.BACKGROUND, type = IType.COLOR_STR, optional = true),
	@facet(name = ISymbol.AXES, type = IType.COLOR_STR, optional = true),
	@facet(name = ISymbol.TYPE, type = IType.ID, values = { ISymbol.XY, ISymbol.HISTOGRAM,
		ISymbol.SERIES, ISymbol.PIE }, optional = true),
	@facet(name = ISymbol.STYLE, type = IType.ID, values = { ISymbol.EXPLODED, ISymbol.THREE_D,
		ISymbol.STACK, ISymbol.BAR }, optional = true),
...
public class ChartDisplayLayer extends AbstractDisplayLayer {
	...
	private String style = ISymbol.DEFAULT;
	...
	private void createSlices(final IScope scope) throws GamaRuntimeException {
		...
		for ( final Data e : datas ) {
			final String legend = (String) e.getFacet(ISymbol.NAME).value(scope);
	...
	private void createChart(final IScope scope) {
		switch (type) {
			...
			case HISTOGRAM_CHART: {
				...
				} else if ( style.equals(ISymbol.STACK) ) {
					chart =
	...
	public void prepare(final LayerDisplayOutput out, final IScope scope)
		...
		IExpression string1 = getFacet(ISymbol.TYPE);
		if ( string1 != null ) {
			String t = Cast.asString(string1.value(scope));
			type =
				ISymbol.SERIES.equals(t) ? SERIES_CHART : ISymbol.HISTOGRAM.equals(t)
					? HISTOGRAM_CHART : ISymbol.PIE.equals(t) ? PIE_CHART : XY_CHART;

		}
		IExpression color = getFacet(ISymbol.AXES);
		if ( color != null ) {
			axesColor = Cast.asColor(scope, color.value(scope));
		}
		IExpression color1 = getFacet(ISymbol.BACKGROUND);
		if ( color1 != null ) {
			backgroundColor = Cast.asColor(scope, color1.value(scope));
		}
		IExpression string = getFacet(ISymbol.STYLE);
		if ( string != null ) {
			style = Cast.asString(string.value(scope));
			// TODO Verifier style;
		}
		createChart(scope);
		createData(scope);
