...
public class GamaGraph<K, V> implements IGraph<K, V> {
	...
	public boolean addEdge(final Object v1, final Object v2, final Object e) {
		...
		try {
			...
			e1.addContext("Impossible to create edge from " + Cast.toGaml(e) + " in graph " + this);
			GAMA.reportError(e1);
			return false;
		}
	...
	public boolean addVertex(final Object v) {
		...
		try {
			...
			e.addContext("Impossible to create vertex from " + Cast.toGaml(v) + " in graph " + this);
			GAMA.reportError(e);
			return false;
		}
	...
	private IValue DShortestPath(final ITopology topology, final Object source, final Object target) {
		DijkstraShortestPath<GamaGeometry, GamaGeometry> p =
			new DijkstraShortestPath(getProxyGraph(), source, target);
		return pathFromEdges(topology, source, target, p.getPathEdgeList());
	}
	...
	private IValue BFShortestPath(final ITopology topology, final Object source, final Object target) {
		...
		return pathFromEdges(topology, source, target, p.getPathEdgeList(target));
	}
	...
	private IValue FWShortestPath(final ITopology topology, final Object source, final Object target) {
		...
		return pathFromEdges(topology, source, target, p.getEdgeList());
	}
	...
	public void add(final Object index, final Object value, final Object param)
		throws GamaRuntimeException {
		double weight = param == null ? DEFAULT_EDGE_WEIGHT : Cast.asFloat(param);
		if ( index == null ) {
	...
	public V max() throws GamaRuntimeException {
		return listValue(null).max();
		// Poids maximum ?
	}

	@Override
	public V min() throws GamaRuntimeException {
		return listValue(null).min();
		// Poids minimum ?
	}
	...
	public GamaPath getCircuit() {
		...
		List edges = new GamaList();
		for ( int i = 0; i < size - 1; i++ ) {
			edges.add(this.getEdge(vertices.get(i), vertices.get(i + 1)));
		}
		return (GamaPath) pathFromEdges(null, null, null, edges);
	}
