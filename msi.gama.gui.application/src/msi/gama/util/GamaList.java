...
public class GamaList<E> extends ArrayList<E> implements IGamaContainer<Integer, E> {
	...
	public Object sum() throws GamaRuntimeException {
		...
		for ( int i = 0, n = size(); i < n; i++ ) {
			...
			if ( o instanceof Integer ) {
				...
				allInt = false;
				allPoint = false;
			} else if ( o instanceof GamaPoint ) {
				allInt = false;
				allFloat = false;
			}
		}
		...
		if ( allPoint ) {
			GamaPoint sum = new GamaPoint(0, 0);
			for ( int i = 0, n = size(); i < n; i++ ) {
				GamaPoint o = Cast.asPoint(get(i));
				if ( o != null ) {
					sum.add(o.x, o.y);
				}
			}
	...
	public Object product() throws GamaRuntimeException {
		...
		if ( allPoint ) {
			...
			for ( int i = 0, n = size(); i < n; i++ ) {
				GamaPoint o = Cast.asPoint(get(i));
				if ( o != null ) {
					mul.setLocation(o.x * mul.x, o.y * mul.y);
				}
			}
			return mul;
		}
	...
	@Override
	public E max() throws GamaRuntimeException {
		boolean allInt = true;
		boolean allFloat = true;
		boolean allPoint = true;
		for ( int i = 0, n = size(); i < n; i++ ) {
		...
		if ( allPoint ) {
			GamaPoint max = new GamaPoint(Double.MIN_VALUE, Double.MIN_VALUE);
			for ( int i = 0, n = size(); i < n; i++ ) {
				GamaPoint o = Cast.asPoint(get(i));
		...
		Double max = Double.MIN_VALUE;
		for ( int i = 0, n = size(); i < n; i++ ) {
			Double o = Cast.asFloat(null, get(i));
			if ( o > max ) {
				max = o;
			}
		}
	...
	@Override
	public E min() throws GamaRuntimeException {
		boolean allInt = true;
		boolean allFloat = true;
		boolean allPoint = true;
		for ( int i = 0, n = size(); i < n; i++ ) {
		...
		if ( allPoint ) {
			GamaPoint min = new GamaPoint(Double.MIN_VALUE, Double.MIN_VALUE);
			for ( int i = 0, n = size(); i < n; i++ ) {
				GamaPoint o = Cast.asPoint(get(i));
