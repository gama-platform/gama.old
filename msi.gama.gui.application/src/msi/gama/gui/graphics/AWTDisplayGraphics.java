...
public class AWTDisplayGraphics implements IGraphics {
	 ...
	 * @param height int
	 */
	protected AWTDisplayGraphics(final int width, final int height) {
	...
	public Rectangle2D drawImage(final BufferedImage img, final Integer angle) {
		if ( angle != null ) {
			at.rotate(MathUtils.toRad * angle, curX + curWidth / 2, curY + curHeight / 2);
		}
	...
	public Rectangle2D drawString(final String string, final Color stringColor, final Integer angle) {
		...
		if ( angle != null ) {
			...
			g2.rotate(MathUtils.toRad * angle, curX + r.getWidth() / 2, curY + r.getHeight() / 2);
		}
	...
	public Rectangle2D drawShape(final Color c, final Shape s, final boolean fill,
		...
		AffineTransform saved = g2.getTransform();
		if ( angle != null ) {
			g2.rotate(MathUtils.toRad * angle, r.getX() + r.getWidth() / 2,
