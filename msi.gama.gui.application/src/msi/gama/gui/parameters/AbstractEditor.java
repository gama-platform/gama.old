...
public abstract class AbstractEditor implements SelectionListener, ModifyListener, EditorListener,
	MouseTrackListener, Comparable<AbstractEditor> {
	...
	public void createComposite(final Composite parent) {
		...
		titleLabel = GUI.createLeftLabel(parent, name);
		try {
		...
		paramControl.setBackground(GUI.normal_bg);
		...
		if ( hasUnit ) {
			...
			unitLabel.setFont(GUI.unitFont);
	...
	protected Control createLabelParameterControl(final Composite composite) {
		...
		fixedValue.setText(originalValue instanceof String ? (String) originalValue : Cast
	...
	protected Control createComboParameterControl(final Composite composite) {
		...
		for ( int i = 0; i < possibleValues.size(); i++ ) {
			if ( param.isLabel() ) {
				...
				valuesAsString[i] = Cast.toGaml(possibleValues.get(i));
			}
		}
	...
	public final void createToolTip(final Control control) {
		...
		if ( minValue != null && maxValue != null ) {
			originalTooltip +=
				"\nrange: [" + (minValue != null ? Cast.toGaml(minValue) : "?") + ".." +
					(maxValue != null ? Cast.toGaml(maxValue) : "?") + "]";
		}
		modifiedTooltip = originalTooltip + "\ninit: " + Cast.toGaml(originalValue);
		setTooltip(originalTooltip);
	}

	protected final void modifyValue(final Object val) {
		currentValue = val;
		titleLabel.setBackground(isValueModified() ? GUI.changed_bg : GUI.normal_bg);
		if ( tooltip != null ) {
	...
	protected final void modifyAndDisplayValue(final Object val) {
		modifyValue(val);
		if ( !isEditable ) {
			fixedValue.setText(val instanceof String ? (String) val : Cast.toGaml(val));
		} else if ( isCombo ) {
