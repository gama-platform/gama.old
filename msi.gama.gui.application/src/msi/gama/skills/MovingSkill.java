 ...
 */
@vars({
	@var(name = MovingSkill.SPEED, type = IType.FLOAT_STR, init = "1.0"),
	@var(name = MovingSkill.HEADING, type = IType.INT_STR, init = "rnd 359"),
	@var(name = MovingSkill.DESTINATION, type = IType.POINT_STR, depends_on = { MovingSkill.SPEED,
		MovingSkill.DESTINATION, IGamlAgent.LOCATION }) })
@skill({ MovingSkill.SKILL_NAME })
public class MovingSkill extends GeometricSkill {
	 ...
	 *             current heading
	 */
	@getter(var = "destination")
	public GamaPoint getDestination(final IAgent agent) throws GamaRuntimeException {
	...
	public double getSpeed(final IAgent agent) throws GamaRuntimeException {
		return (Double) agent.getDirectVarValue(SPEED);
	}

	public void setSpeed(final IScope scope, final IAgent agent, final double s)
		throws GamaRuntimeException {
		scope.setAgentVarValue(agent, SPEED, s);
	}

	public int getHeading(final IAgent agent) throws GamaRuntimeException {
		return (Integer) agent.getDirectVarValue(HEADING);
	}
	...
	private int computeHeadingFromAmplitude(final IScope scope, final IAgent agent) {
		int ampl = scope.hasArg("amplitude") ? Cast.asInt(scope.getArg("amplitude")) : 359;
		agent.setHeading(agent.getHeading() + GAMA.getRandom().between(-ampl / 2, ampl / 2));
		return agent.getHeading();
	}

	private int computeHeading(final IScope scope, final IAgent agent) {
		Integer heading = scope.hasArg(HEADING) ? Cast.asInt(scope.getArg(HEADING)) : null;
		if ( heading != null ) {
	...
	private double computeDistance(final IScope scope, final IAgent agent)
		...
		Double s = scope.hasArg(SPEED) ? Cast.asFloat(scope.getArg(SPEED)) : getSpeed(agent);
		return s * getTimeStep(scope);
	}

	private GamaPoint computeTarget(final IScope scope, final IAgent agent) {
	...
	private ITopology computeTopology(final IScope scope, final IAgent agent) {
		...
		if ( on != null ) {
			if ( on instanceof GamaGraph ) {
				...
				try {
					topo = Casting.asTopology(scope, on);
	...
	@action("wander")
	@args({ SPEED, "amplitude", "bounds" })
	public GamaPath primMoveRandomly(final IScope scope) throws GamaRuntimeException {
		IAgent agent = getCurrentAgent(scope);
		GamaPath pathFollowed;
		...
		GamaPoint loc = getTopology(agent).getDestination(location, heading, dist, true);
	...
	@action("move")
	@args({ SPEED, HEADING, "bounds" })
	public GamaPath primMoveForward(final IScope scope) throws GamaRuntimeException {
		IAgent agent = getCurrentAgent(scope);
		GamaPath pathFollowed;
		GamaPoint location = agent.getLocation();
		double dist = computeDistance(scope, agent);
	...
	@action("follow")
	@args({ SPEED, "path" })
	public GamaPath primFollow(final IScope scope) throws GamaRuntimeException {
		IAgent agent = getCurrentAgent(scope);
		GamaPath pathFollowed = null;
		GamaList<IGeometry> edges = new GamaList();
		GamaPoint location = agent.getLocation();
		double dist = computeDistance(scope, agent);

		GamaPath path = scope.hasArg("path") ? (GamaPath) scope.getArg("path") : null;
		...
		edges.add(new GamaGeometry(location));
	...
	public GamaPath primGoto(final IScope scope) throws GamaRuntimeException {
		final IAgent agent = getCurrentAgent(scope);
		GamaPoint source = agent.getLocation().copy();
		final double maxDist = computeDistance(scope, agent);
		final GamaPoint goal = computeTarget(scope, agent);
		if ( goal == null ) {
			scope.setStatus(ExecutionStatus.failure);
			return null;
		}
		...
		GamaPath path = (GamaPath) agent.getAttribute("current_path");
		...
		GamaPath pathFollowed = moveToNextLocAlongPath(agent, path, maxDist);
	 ...
	 */
	private GamaPath moveToNextLocAlongPath(final IAgent agent, final GamaPath path, double distance) {
		int index = 0;
		int indexSegment = 1;
		GamaPoint currentLocation = agent.getLocation();
		int nb = path.getEdgeList().size(); // instead of getGeometries() ?? Faster, and more
											// reliable. But is it the same ?
		GamaList<IGeometry> segments = new GamaList();
		...
		IGeometry lineEnd = path.getEdgeList().get(nb - 1);
		GamaPoint falseTarget = Points.opClosestPointTo(path.getEndVertex(), lineEnd);
		...
		for ( int i = index; i < nb; i++ ) {
			IGeometry line = path.getEdgeList().get(i);
	 ...
	 * scope.hasArg("square_size") ? Cast.asFloat(scope.getArg("square_size")) : null;
	 * 
	 * GamaPoint cs =
	...
	public GamaPoint gotoTargetInsideGeom(final IScope scope, final GamaPoint target,
		...
		if ( graph != null || matrix != null || geom.getInnerGeometry() instanceof Polygon ||
			...
			GamaPoint source = location;
			GamaPoint ptTarg = target;
			...
			if ( graph != null ) {
				...
				// Ces lignes sont commentées car je ne suis pas certain que ce soit nécessaire de
			...
			if ( geom.getInnerGeometry() instanceof Polygon ) {
				...
				for ( int i = 0; i < mp.getNumGeometries(); i++ ) {
					...
					if ( isGoodPolygon(scope, new GamaGeometry(p), source, ptTarg) ) {
						ps = p;
						break;
					}
				}
			...
			GamaIntMatrix tab =
				GeometricFunctions.discretisationGrid(ps, size, env.getWidth(), env.getHeight());
			tab.setCellSize(size);
			GamaPoint gp = findnextLocTowardPolygon(scope, tab, source, ptTarg, speed);
			if ( gp != null ) {
				agent.setLocation(gp);
				return gp;
			}
		...
		if ( geom.getInnerGeometry() instanceof MultiLineString ) {
			GamaPoint source = getCurrentAgent(scope).getLocation();
			GamaPoint ptTarg = target;
			GamaPath path = null;
			final GamaPath currentPath = (GamaPath) agent.getAttribute("currentPath");
			if ( currentPath != null ) {
	 ...
	 * @return the next location
	 */
	private GamaPoint findnextLocTowardPolygon(final IScope scope, final GamaIntMatrix matrix,
		final GamaPoint source, final GamaPoint target, final double distance) {
		double xs = source.x;
		double ys = source.y;
	 ...
	 * @return the next location
	 */
	private GamaPoint findnextLocTowardPolygonUsingTriangulation(final IScope scope, GamaPath path,
		final Polygon geom, final GamaPoint source, final GamaPoint target, final double distance) {
		if ( path == null ) {
			GamaList<Polygon> triangles = GeometricFunctions.triangulation(geom);
			GamaGraph graph = GeometricFunctions.buildPolygonGraph(scope, triangles);
	 ...
	 * @return the next location
	 */
	private GamaPoint findnextLocTowardNetwork(final IScope scope, final GamaPath path,
		final MultiLineString lines, final GamaPoint source, final GamaPoint target,
	 ...
	 * @return the next location
	 */
	private GamaPoint findnextLocTowardNetwork(final IScope scope, GamaPath path,
		final GamaList<LineString> lines, final GamaPoint source, final GamaPoint target,
		final double distance) throws GamaRuntimeException {
		if ( path == null ) {
			GamaSpatialGraph graph = GeometricFunctions.buildNetworkGraph(scope, lines, true);
			// graph.setPolygon(false);
			path = GeometricFunctions.pathBetweenPoints(scope, source, target, graph);
		}
		if ( path == null ) { return null; }
		GamaPoint gp = nextLocationLine(scope, path, source, target, distance);
		return gp;
	}
	 ...
	 */
	private GamaPoint findnextLocTowardNetwork(final IScope scope, GamaPath path,
		final GamaSpatialGraph graph, final GamaPoint source, final GamaPoint target,
		final double distance) throws GamaRuntimeException {
		if ( path == null ) {
			path = GeometricFunctions.pathBetweenPoints(scope, source, target, graph);
	 ...
	 * @return the next location
	 */
	public GamaPoint nextLocationLine(final IScope scope, final GamaPath path, GamaPoint source,
		final GamaPoint target, double distance) {
		...
		if ( path == null || path.getEdgeList().isEmpty() ) {
			...
			GamaPoint gp = new GamaPoint(x_s, y_s);
		...
		GamaPoint ns = path.getStartVertex();
	 ...
	 * @return the next location
	 */
	public GamaPoint nextLocationTriangle(final IScope scope, final GamaPath path,
		GamaPoint source, final GamaPoint target, final double distance) {
		ITopology m = scope.getAgentScope().getTopology();
		GamaPoint loc = null;
		Graph<IGeometry, IGeometry> g = path.getGraph();
		double distRemain = distance;
		List<IGeometry> edges = new GamaList(path.getEdgeList());
		int nb = edges.size();

		for ( int i = 0; i < nb; i++ ) {
			GamaPoint targ = null;
			IGeometry geom = edges.get(i);
			IGeometry ga = g.getEdgeSource(geom);
			IGeometry gb = g.getEdgeTarget(geom);
			/*
	...
	private GamaPoint findnextLocTowardLine(final IScope scope, final Coordinate[] coords,
		final GamaPoint source, final GamaPoint target, final Distance distance) {
		IAgent agent = getCurrentAgent(scope);
		ITopology m = scope.getAgentScope().getTopology();
		...
		for ( int i = 1; i < nbSp; i++ ) {
			...
			double dist =
				Math.sqrt(MathUtils.pow(x_s - x_t_it, 2) + MathUtils.pow(y_s - y_t_it, 2));
	...
	private GamaPoint computeLocationForward(final IScope scope, final double dist,
		...
		GamaPoint computedPt = Points.opClosestPointTo(new GamaGeometry(frontier), loc);
