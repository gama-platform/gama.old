...
import msi.gama.util.graph.GamaSpatialGraph.VertexRelationship;
...
public class Graphs {
	...
	private static class IntersectionRelation implements VertexRelationship {
		...
		@Override
		public boolean related(final IScope scope, final IGeometry p1, final IGeometry p2) {
		...
		@Override
		public boolean equivalent(final IGeometry p1, final IGeometry p2) {
	...
	private static class DistanceRelation implements VertexRelationship {
		 ...
		 */
		@Override
		public boolean related(final IScope scope, final IGeometry g1, final IGeometry g2) {
	...
	public static GamaPath toPath(final IScope scope, final Object object) {
		return GamaPathType.staticCast(scope, object, null);
	}

	@operator(value = "agent_from_geometry")
	public static IAgent getAgentFromGeom(final GamaPath path, final GamaGeometry geom) {
		if ( path == null ) { return null; }
		return path.getAgent(geom);
	}
	...
	public static Boolean containsEdge(final GamaGraph graph, final Object edge) {
		return graph.containsEdge(edge);
	}

	@operator(value = "contains_edge")
	public static Boolean containsEdge(final GamaGraph graph, final GamaPair edge) {
		return graph.containsEdge(edge.first(), edge.last());
	}

	@operator(value = "source_of", type = ITypeProvider.LEFT_CONTENT_TYPE)
	public static Object sourceOf(final GamaGraph graph, final Object edge) {
		if ( graph.containsEdge(edge) ) { return graph.getEdgeSource(edge); }
		return null;
	}

	@operator(value = "target_of", type = ITypeProvider.LEFT_CONTENT_TYPE)
	public static Object targetOf(final GamaGraph graph, final Object edge) {
		if ( graph.containsEdge(edge) ) { return graph.getEdgeTarget(edge); }
		return null;
	}

	@operator(value = "weight_of")
	public static Double weightOf(final GamaGraph graph, final Object edge) {
	...
	public static GamaList inEdgesOf(final GamaGraph graph, final Object vertex) {
		if ( graph.containsVertex(vertex) ) { return new GamaList(graph.incomingEdgesOf(vertex)); }
		return new GamaList();
	}

	@operator(value = "out_edges_of")
	public static GamaList outEdgesOf(final GamaGraph graph, final Object vertex) {
		if ( graph.containsVertex(vertex) ) { return new GamaList(graph.outgoingEdgesOf(vertex)); }
		return new GamaList();
	}

	@operator(value = "neighbours_of")
	public static GamaList neighboursOf(final GamaGraph graph, final Object vertex) {
	...
	public static GamaList predecessorsOf(final GamaGraph graph, final Object vertex) {
		if ( graph.containsVertex(vertex) ) { return new GamaList(
	...
	public static GamaList successorsOf(final GamaGraph graph, final Object vertex) {
		if ( graph.containsVertex(vertex) ) { return new GamaList(
	...
	public static IGraph spatialFromEdges(final IScope scope, final GamaList edges) {
	...
	@operator(value = "with_weights")
	public static IGraph withWeights(final IScope scope, final GamaGraph graph,
		final GamaList weights) {
	...
	// construit à partir des vertex (edges générés soit sous la forme d'une paire vertex::vertex,
	...
	// construit à partir des edges (vertex générés soit sous la forme d'une paire edge::edge, soit
