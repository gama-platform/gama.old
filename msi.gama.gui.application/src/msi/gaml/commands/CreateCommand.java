 ...
 * macro-agent.
 */
@symbol(name = ISymbol.CREATE, kind = ISymbolKind.SEQUENCE_COMMAND)
...
@facets({ @facet(name = ISymbol.SPECIES, type = IType.SPECIES_STR, optional = true),
	...
	@facet(name = ISymbol.FROM, type = IType.NONE_STR, optional = true),
	@facet(name = ISymbol.NUMBER, type = IType.INT_STR, optional = true),
...
public class CreateCommand extends AbstractCommandSequence implements ICommand.WithArgs {
	...
	public CreateCommand(final IDescription desc) {
		...
		from = getFacet(ISymbol.FROM);
		number = getFacet(ISymbol.NUMBER);
		speciesExpr = getFacet(ISymbol.SPECIES);
	...
	private String accumulateAvailableSpecs(final IAgent executor) {
		...
		while (!currentSpec.getName().equals(ISymbol.WORLD_SPECIES_NAME)) {
	...
	public Object privateExecuteIn(final IScope scope) throws GamaRuntimeException {
		...
		scope.addVarWithValue(ISymbol.MYSELF, scope.getAgentScope());

		if ( from != null ) {
			IExpression size = getFacet(ISymbol.SIZE);
			if ( size != null ) {
			...
			IExpression typeDisc = getFacet(ISymbol.TYPE);
			if ( typeDisc != null ) {
				typeDiscretisation = Cast.asString(typeDisc.value(scope));
			...
			if ( type.isSpeciesType() ) {
				...
				GamaList ags = Cast.asList(scope, from.value(scope));
				...
				if ( it3 != null ) {
					...
					while (it3.hasNext() && index <= max) {
						...
						GisUtil.setCurrentGisReader(fact);
		...
		String s = getLiteral(ISymbol.RETURNS);
		if ( s != null ) {
	...
	private void createAgentsFromAgent(final IScope scope, final IAgent agent,
		...
		if ( typeDiscretisation.equals("Squares") ) {
			geoms = GeometricFunctions.discretisation(geom.getInnerGeometry(), sizeSquare, false);
		} else {
			if ( sizeSquare > 0 ) {
				geoms = new GamaList<Geometry>();
				List<Geometry> parts =
					GeometricFunctions.discretisation(geom.getInnerGeometry(), sizeSquare, true);
				for ( Geometry gg : parts ) {
					geoms.addAll(GeometricFunctions.triangulation(gg));
				}
			} else {
				geoms =
					new GamaList<Geometry>(
						GeometricFunctions.triangulation(geom.getInnerGeometry()));
			}
		}
		...
		for ( int i = 0, n = agents.size(); i < n; i++ ) {
			agents.get(i).getGeometry().setInnerGeometry(finalGeoms.get(i));
		}
	}
		...
		try {
			shapeFile =
				scope.getSimulationScope().getModel()
					.getRelativeFilePath(Cast.asString(from.value(scope)), true);
