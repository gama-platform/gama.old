...
public class OsmSkill extends Skill {
	...
	@action(name = "load_osm", args = {
		...
		@arg(name = "split_lines", type = IType.BOOL, optional = true, doc = @doc("if false, the lines are not split at intersections")) }, doc = @doc(value = "load a OSM file and create the corresponding agents", returns = "list of created agents", examples = { "list<agent> <- load_osm(my_osm_file);" }))
	public List<IAgent> loadOSMFile(final IScope scope) throws GamaRuntimeException {
		...
		final GamaOsmFile file = (GamaOsmFile) scope.getArg("file", IType.NONE);
		if ( file == null ) { return createdAgents; }
		...
		try {
			OsmReader reader = new OsmReader(file.getGis(scope));
