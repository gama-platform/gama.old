...
global {
	...
	init{
		...
		create pedestrian number: 1{
			set shape <- polygon([{0, -0.5}, {0.3, 0.4}, {-0.3, 0.4}]);
			set location <- any_location_in(first(list(busstop)));
			set heading <- 270;
			set occupation_space <- 1;
			set target <- nil;
			set known_targets <- list(busstop);
    ...
    reflex paus_simulation when: (time mod 100 = 0){
    	do pause;
    }
}
			...
			switch behavior_id{
				...
				match 3{ // waiting for picking up people
					...
					if ((target != nil) and (target.location_type = 2) ){
						...
						else if (vehicle_availability = 0){
							set behavior_id <- 4;
						}
						else{
							let busstops_with_passengers type: list <- (list(busstop) where (each.waiting_passengers > 0));
...
experiment EastSideScenario3c type: gui {
	output{
