...
global {
	...
	list pedestrian_list of: pedestrian <- [];
	init{
		...
		set pedestrian_list <- list(pedestrian);
...
entities {
	...
	species wheelvehicle parent: occupationagent skills: [driving2d]{
		...
		reflex bus_behavior{
			...
			switch behavior_id{
				match 1{ // initially, select the busstop
					...
					if (length(pedestrian) > 0){
						set target <- ((pedestrian_list) with_min_of (self distance_to each));
						set behavior_id <- 2;
						remove target from: pedestrian_list;
					}
					else{
						set behavior_id <- 3;
    		...
    		add current_agent_infor to: live_buses;
    		do write message: "target: " + string(target);
		}
		aspect base{
...
experiment EastSideScenario3j type: gui {
	output{
		/* *
