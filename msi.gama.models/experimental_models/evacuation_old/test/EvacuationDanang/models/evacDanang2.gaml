...
global {
	...
	float moto_width <- 1;
	graph the_graph;
	int eastern_beach_population <- 500;
	int northern_beach_population <- 500;
...
entities {
	...
	species pedestrian skills: [driving]{
		...
		rgb color <- rgb('yellow');
		/* for driving skill */
		reflex move_with_pedestrianGoto when: (target != nil){
...
experiment evacDanang1 type: gui {
	output{
		display city_display refresh_every: 1{
			...
			species beach aspect: base transparency: 0.2;
