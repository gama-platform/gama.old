		...
		reflex bus_behavior{
			switch behavior_id{
				match 1{ // initially, select the busstop
					...
					do write message: "" + busstops_with_passengers; 
					if ((busstops_with_passengers != nil) and (length(busstops_with_passengers) > 0)){
						...
						do write message: "status 1: nobody's at the bus stop";
					}
				}
				...
				match 3{ // waiting for picking up people
					...
					if ((target != nil) and (target.location_type = 2) ){
						...
						do write message: "satus 3: stuck";
					}
				}
				...
				match 5{
					...
					if (target != nil){
						...
						do write message: "satus 5: stuck";
					}
				}
				match 6{
					do write message: "behavior id: " + behavior_id;
					if (target != nil){
						if (target.location_type = 1){ //building
							let building_target type: building <- building(target);
							if ((vehicle_availability > 0) and (building_target.availability > 0)){
								set vehicle_availability <- vehicle_availability - 1;
								ask building_target{
					...
					do write message: "at: " + last_arrival_time + ", " + number_of_survival + "th person arrive shelters by bus";
						...
						do write message: "satus 6: stuck";
					}
				}
			}
		}
...
experiment EastSideScenario3b type: gui {
	output{
