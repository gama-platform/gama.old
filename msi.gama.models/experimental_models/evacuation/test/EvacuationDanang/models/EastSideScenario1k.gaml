...
global {
    ...
    reflex save_dynamic_agents{
		set log_to_save <- string(int(time))+":"+length(pedestrian)+":";
    	//time: agentid, position, heading
    	loop item over: list(pedestrian){
    		let current_pedestrian type: pedestrian <- pedestrian(item);
    		set log_to_save <- log_to_save + (current_pedestrian.location.x with_precision 2) + "," + (current_pedestrian.location.y with_precision 2) + "," + current_pedestrian.heading + ";";
    	}
    }
}
...
experiment EastSideScenario1k type: gui {
	output{
		...
		file name: "replay" type: text data: log_to_save;
	}
}
