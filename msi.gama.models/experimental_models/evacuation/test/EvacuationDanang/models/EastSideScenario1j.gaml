...
global {
	...
	string output_folder <- "../ouputs/EastSideScenario1/";
    ...
    reflex save_dynamic_agents{
    	let filename type: string <- output_folder + "danangeastside_pedestrians_" + string(time) + ".shp";
...
experiment EastSideScenario1j type: gui {
	output{
		/* *
		...
		/* */
		display city_display_with_opengl refresh_every: 1 type: opengl autosave: true{
			species roadwidth aspect: base;
			species roadline aspect: base;
			species highground aspect: base;
			species busstation aspect: base;
