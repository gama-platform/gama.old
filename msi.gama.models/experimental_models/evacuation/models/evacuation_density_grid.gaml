model evacuation_density_grid

global {
	float simulated_population_rate <- 0.001 min: 0 max: 1  parameter: 'Population rate';
	...
	rgb WHITE <- rgb('white') const: true;
...
//	string simulation_output -> { self build_simulation_output [] } ;
	...
	reflex when: (time = 1) {
		if (is_hybrid_simulation) {
			write 'Hybrid experiment';
		} else {
			write 'ABM experiment';
		}
		
		write 'Creating pedestrians and initializing the density_grid';
		 ...
		 */
		let grid_dimension type: point value: {329, 430};
		let world_shape_sm type: matrix value: shape as_grid grid_dimension;
		...
		loop times: grid_dimension.x {
			...
			loop times: grid_dimension.y {
				create density_grid with: [ shape :: ( world_shape_sm at ({i, j}) ) ];
				set j value: j + 1;
			}
		...
		write 'Finished creating pedestrians and initializing the density_grid with length(list(density_grid)): ' + (string(length(list(density_grid)) ));
	}
}
...
//	species density_grid schedules: ( list(density_grid) where !( empty(each.roads) ) ) {
		list roads of: road;
...
//			remove from: speed_by_roads all: speed_by_roads;
			...
			if !(empty(pedestrians)) {
				loop r over: roads {
					let pedestrians_by_road type: int value: length(pedestrians where (each.current_road = r));

					let speed type: float value: pedestrian_max_speed;
					if (pedestrians_by_road > 0) {
						set speed value: pedestrian_max_speed * ( 1 - ( ( float( pedestrians_by_road ) / float( road_intersections_area at r) ) / max_density ) ) ;
					}
					
					add r :: speed to: speed_by_roads;
					
					if (speed < pedestrian_max_speed) {
						write name + ' with speed: ' + (speed) + '; pedestrians_by_roads: ' + (string(pedestrians_by_road));
					}
				}	
			}
		} 
		...
		aspect base {
			draw shape: geometry color: !(empty(roads)) ? BLUE : WHITE;
		}
	}
	...
	species pedestrian skills: moving {
		...
		bool reach_shelter <- false;
   		...
   		road current_road <- ( road closest_to self ) value: ( road closest_to self ) depends_on: location;
   		point previous_location;

		geometry window_viewer;
		float local_density_window_area;
		shelter goal <- (shelter closest_to self) depends_on: [ shape, location ];
...
experiment 'ABM (100% population)' type: gui {
...
//			species ward aspect: base transparency: 0.5;
		...
		monitor all_density_grids value: length(density_grid);
		monitor scheduled_density_grids value: length( list(density_grid) where !( empty(each.roads) ) );
		...
		display road_density_grid {
			species density_grid aspect: base transparency: 0.2;
...
experiment 'Hybrid (100% population)' type: gui {
