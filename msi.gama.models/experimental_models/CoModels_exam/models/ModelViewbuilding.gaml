...
import 'ReferencebuildingImport.gaml'
...
global {
	...
	list peopleOut;
	init {
		loop b over:buildingOut{
			create building from:b;
		}
		loop r over:roadOut{
			create road from:r;
		}
		loop p over:peopleOut{
			create people from:p;
		}
	}
	...
	//////////////////////////LAYOUT INIT//////////////////////////////////////////
	reflex InitAbstractBuildingLayout when: cycle = 1{
		int curIndus<-0;
		int curRes <-0;
...
entities {
	...
	species abstractBuilding mirrors: (building as list) {
	...
	species abstractRoad mirrors: (road as list) {	
	...
	species abstractPeople mirrors: (people as list) {
	  aspect abstract {
...
experiment road_trafficview type: gui {	
	output {
		display city_display {//} type:opengl focus:focusOnShape{
			species abstractBuilding aspect: abstract;
			species abstractRoad aspect: abstract ;
		    species abstractPeople aspect: abstract;		   
