...
global {
	...
	list buildingOut;// function:{building as list};
	
	list roadOut;// function:{road as list};
	
	list peopleOut;// function:{people as list};
...
//		write "init "+self;
		...
		create people number: nb_people {
			 ...
			 shape<-circle(10);
			 age <-int((rnd(100)/100*rnd(100)/100*rnd(100)/100)*100);
		} 
		ask 20 among people {
			is_infected<-true;
			color <- °red;
	...
	reflex takeout{
	 ...
	 buildingOut<-building as list;
	
	 roadOut<-road as list;
	
	 peopleOut <-people as list;
	}
...
experiment road_traffic type: gui {	
	output {
		display city_display refresh_every:1 {//type:opengl focus:focusOnShape{
