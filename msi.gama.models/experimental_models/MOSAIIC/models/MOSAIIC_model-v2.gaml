...
global skills: [osm]{
	...
	init {
		...
		ask signal {
			if (highway in signals_str) {
				...
				node nd <- (nodes at [location]);
				if (nd != nil) {nd.signal <- nil;}
				do die;
			}
		}
		do init_feux_rouges();
	...
	action init_feux_rouges {
		list<signal> feux <- signal where (each.highway = "traffic_signals");
		list<list<signal>> groupes <- feux simple_clustering_by_distance 50.0; 
		loop gp over: groupes {
			int cpt_init <- rnd(60);
			bool vert <- flip(0.5);
			loop si over: gp {
				(signal(si)).compteur <- cpt_init; 
				ask (signal(si)) {
...
species people skills: [osm_driving]{ 
	...
	bool engagement_prochaine_route(road route, int lane, bool vient_arriver) {
		...
		if (noeud = nil) {
			...
			signal sig <- signal(noeud.signal);
			if (sig = nil) {
				engagement <- pret_engagement(route, lane);
			} else {
				switch sig.highway {
					match ("stop") {
						engagement <- not (vient_arriver) and pret_engagement(route, lane);
					}
					match ("traffic_signals") {
						if (sig.est_croissement and current_road in sig.inverse) {
							engagement <- not(sig.est_vert) and pret_engagement(route, lane);
						} else {
							engagement <- sig.est_vert and pret_engagement(route, lane);
						}
