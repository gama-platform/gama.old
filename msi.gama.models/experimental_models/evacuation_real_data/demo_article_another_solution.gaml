model demo_article_another_solution
...
entities {
	...
	species pedestrian skills: moving {
		...
		bool towards_first_point <- false;
		bool towards_last_point <- false;
...
//			let obstacle_pedestrians type: list of: people value: (people overlapping window_viewer.shape);
			...
			let obstacle_pedestrians type: list of: pedestrian value: ( (list(pedestrian) where !((pedestrian(each)).reach_shelter) ) overlapping window_viewer);
...
// 			draw shape: geometry color: pedestrian_color;
	...
	species density_window_builder skills: [moving] {
		action build {
			...
			if (diameter_geom != nil) { // diameter_geom MAY BE NULL???
			...
			if (current_road != previous_road) {
				...
				if (current_road_part != nil) {
					if (local_window != nil) {
						...
						set local_window value: current_road_part;
					}
				}
			}
			set current_people.window_viewer value: local_window;
			
			// not correct when (current_road != previous_road)
			set current_people.local_density_window_area value: local_density_window_length * (previous_road.width);
		}
	}
}
