...
global {
	...
	var evaporation_rate type : float init : 0.10 min : 0.0 max : 1.0 parameter :
	'Rate of evaporation of the signal (%/cycle):' category : 'Signals';
	var diffusion_rate type : float init : 0.5 min : 0.0 max : 1.0 parameter :
	...
	var name: ants_number type : int init : 200 min : 1 parameter : 'Number of ants:'
	...
	const center type : point init : { round ( gridsize / 2 ) , round ( gridsize / 
	2 ) };
...
frequency : grid_frequency {
	...
	aspect default {
		draw geometry color : is_nest ? nest_color : ( ( food > 0 ) ? food_color : (
		( float ( road ) < 0.001 ) ? background : rgb ( #009900 ) + int ( road * 5 )
		) );
	}
}
entities {
	species ant skills : [ moving ] control : fsm {
		...
		aspect info {
			draw shape : circle at : location size : 1 rotate : my heading empty : !
			has_food;
			draw shape : line at : location to : destination + ( ( destination -
			location ) ) color : rgb ( 'white' );
			draw shape : circle at : location size : 4 empty : true color : rgb (
			'white' );
			draw text : string ( self as int ) color : rgb ( 'white' ) size : 1;
			draw text : state color : rgb ( 'white' ) size : 1 at : my location + { 1 ,
			1 };
		}
		aspect icon {
		...
		aspect default {
			//draw geometry : square ( 10 ) rotated_by heading color : rgb ( 'blue' ) 
			//empty : true;  
...
experiment Genetic type : batch repeat : 2 keep_seed : true until : (
