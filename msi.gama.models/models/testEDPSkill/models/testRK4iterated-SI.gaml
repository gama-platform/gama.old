...
entities {
	species node skills: [EDP] {
		float I <- float(iInit);
		float S <- N - I;
		map equations <- ["S"::"(- beta * S * I / N) + (gamma * I)", "I"::"(beta * S * I / N) - (gamma * I)"];
		map param <- ["beta"::beta, "gamma"::gamma, "N"::N];
		reflex go {
			let varValues type: list of: float <- [];
			add S to: varValues;
			add I to: varValues;
			let temp type: list of: float <- list(self RK4iterated [equations::equations, value:: varValues, param::param, h::hKR4, nbSteps::nbSteps]);
