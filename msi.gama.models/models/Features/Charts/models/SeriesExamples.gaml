	...
	output {
		display "data_cumulative_serie_spline_chart" type:java2D {
			chart "Nice cumulative series chart" type:series 
				background: #darkblue
			 	...
			 	y_label:'Nice Ylabel'
			 	{ 
				data "Spline" value:cos(100*cycle)*cycle*cycle
					color:#orange
					marker_shape:marker_empty
					style:spline;
				data "Step" value:cycle*cycle 
					style:step
					color:#lightgrey;
				
				data "Classic" value:[cycle+1,cycle]
		...
		display "style_cumulative_style_chart" type:java2D {
			chart "Style Cumulative chart" type:series 
			 	{ 
				data "Spline" value:cos(100*cycle)
					color:#orange
					style:spline;
				data "area" value:cos(100*cycle)*0.3
					color:#red
					style:"area";
				data "dot" value:cos(100*cycle+60)
					color:#green
					style:dot;
			}
		} 
		display "datalist_xy_chart" type:java2D {
			chart "datalist_xy_cumulative_chart" type:xy {
				datalist legend:["A","B","C"] 
					value:[[cycle*cos(cycle*100),cycle*sin(cycle*100),2],[cycle/2*sin(cycle*100),cycle*2*cos(cycle*100),1],[cycle+2,cycle-2,cos(cycle*100)]] 
					x_err_values:[3,2,10]
					y_err_values:[3,cos(cycle*100),2*sin(cycle*100)]
		...
		display "datalist_xy_line_chart" type:java2D {
			chart "datalist_xy_cumulative_chart" type:xy {
				datalist legend:["A","B"] 
					value:[[cycle*cos(cycle*100),cycle*sin(cycle*100),2],[cycle/2*sin(cycle*100),cycle*2*cos(cycle*100),1]] 
					marker_shape:marker_circle 					// same for all
					color:[#green,#blue]
					style:line;
			}
		...
		display "my_data_cumulative_xy" type:java2D {
			chart "my_data_cumulative_xy" type:xy {
				...
				data "ABC" value:[cycle+1,cycle*2,cos(cycle)] 
					marker_shape:marker_circle
					fill:false
					line_visible:false
					color:#black
					x_err_values:ln(cycle)
					y_err_values:cos(cycle*100)*3;
