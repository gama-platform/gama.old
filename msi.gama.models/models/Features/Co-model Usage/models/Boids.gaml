...
global torus: torus_environment{ 
	int number_of_agents <- 1000 min: 1 max: 1000000;
	...
	geometry envShape<-square(width_and_height_of_environment);
	geometry shape <- envShape;
	init { 
...
entities {
	...
	species boids skills: [moving] {
		...
		reflex separation when: apply_separation {
			point acc <- {0,0};
			list aa<-(boids overlapping (circle(minimal_distance)));
			if(length(aa)>1){					
				ask aa{
