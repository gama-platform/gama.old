...
global {
	...
	int ants_number <- 1 min: 1 parameter: 'Number of ants:' category: 'Environment and Population';
	...
	geometry envSh<-square(gridsize);
//	geometry shape <- circle(gridsize);
	init {
...
entities {
	grid ant_grid width: gridsize height: gridsize neighbours: 8 frequency: grid_frequency use_regular_agents: true use_individual_shapes: true{
	...
	species ant skills: [moving] control: fsm {
		float speed <- 1.0;
		bool has_food <- false;
		signal road type:float update: has_food ? 240 : 0 decay: evaporation_rate proportion: diffusion_rate environment: ant_grid;
