...
global { 
	...
	var shelter_shape type: geometry init: square (100.0);
	var number_of_shelter type: int init: 2 const: true;
	
	
	init {
		create species: prey number: number_of_prey;
		create species: predator number: number_of_predator; 
		
		create species: shelter number: number_of_shelter returns: shelters;
...
entities {  
	species prey skills: [situated, moving] control: fsm {
		...
		state flee_predator {
			...
			if condition: !(empty (nearby_predators)) {
				do action: move {
					arg name: heading value: (self) towards (nearest_shelter);
					arg name: speed value: prey_speed;
				}				
			}
	...
	species predator skills: [situated, moving] schedules:  shuffle (list (predator)) {
		...
		action choose_target_prey type: prey {
			if condition: ( (target_prey = nil) or (dead (target_prey) ) ) {
				return value: one_of ( (list (prey)) where (each.state = 'move_around') );
			}
			
			return value: target_prey;
		}
		
		reflex move_around when: (target_prey = nil) {
			do action: wander {
				arg name: speed value: predator_speed;
			}
		}
		
		reflex chase_prey when: (target_prey != nil) {
			do action: move {
				arg name: heading value: (self) towards (target_prey);
				arg name: speed value: predator_speed;
			}
		}
	...
	species shelter skills: [situated, moving] frequency: 2 {
		var shape type: geometry init: (square (100.0)) at_location {250, 250};
		var chased_preys type: list of: prey value: (list (prey)) where ( (each.shape intersects shape) and (each.state = 'flee_predator') );
		
		init {
			create species: shelter_info_drawer number: 1 with: [ target :: self ]; 
		}
		
		reflex move_around {
			do action: wander {
				arg name: speed value: shelter_speed;
			}
		}
		
		reflex capture_chased_preys when: !(empty (chased_preys)) {
			capture target: chased_preys as: prey_in_shelter {
		...
		reflex release_member_preys {
			...
			release target: to_be_released {
				set state value: 'invisible';
				set shape value:  (circle (prey_size)) at_location ((self).location);
			}
		}
	...
	species shelter_info_drawer skills: situated {
		var target type: shelter;
		
		aspect default {
			draw text: 'Members: ' + (string (length (list (target.members)))) color: rgb ('red') size: 15 at: {(target.location).x - 42, (target.location).y};
		}
	}
}
