...
entities {
	...
	species UnitDiskGraph skills: [situated, visible] {
		...
		action allocateNewNodeByStdDeviation
		{
			...
			loop from: 0 to: length(cell_list) - 1 var: cnt
			{
				let the_cell type: cellula_automata value: cell_list at cnt;
				...
				if (current_priority > maximum_priority)
				{
					...
					set location_found value: true; // Always TRUE :-)
				}
			}
			if(location_found)
			{
