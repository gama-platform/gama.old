 ...
 *  seirs
 *  Author: 
 *  Description: A compartmental SEIRS model based on Hethcote description
 */
...
global { 
		...
		float gamma <- 1/5 parameter: 'Gamma (R->S)'; // The parameter Gamma
...
environment width: 100 height: 100 {
        grid seirs_grid width: 100 height: 100 {
                var color type: rgb init: rgb('black');
                list neighbours of: seirs_grid <- self neighbours_at 4;
...
entities {
        species Host skills: [moving] {
                ...
                int sic_count <- 0;
                var myPlace type: seirs_grid function: {location as seirs_grid};
