...
entities {
	...
	species pedestrian skills: moving {
		...
		bool reach_shelter <- false;
   		...
   		road current_road value: ( (list(road)) closest_to self ) value: ( (list(road)) closest_to self ) depends_on: location;
		...
		action compute_speed type: float {
			...
			set window_viewer value: ( (circle(local_density_window_length)) at_location location ) intersection current_road;
			...
			set window_viewer value: window_viewer + 0.1; // buffer the windows wiewer
//			let obstacle_pedestrians type: list of: pedestrian value: (not_reach_schelters where (each.current_road = self.current_road)) overlapping window_viewer; // worst performance
//			let obstacle_pedestrians type: list of: pedestrian value: not_reach_schelters overlapping window_viewer; // better performance
			let obstacle_pedestrians type: list of: pedestrian value: (pedestrian overlapping window_viewer) where ( (each.current_road = current_road) and !(each.reach_shelter) ); // the best performance
