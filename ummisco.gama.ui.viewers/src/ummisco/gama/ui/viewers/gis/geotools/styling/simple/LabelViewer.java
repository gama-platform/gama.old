...
public class LabelViewer {
	...
	public TextSymbolizer get(final StyleBuilder build) {
		...
		final String fontName = this.font[0].getName();
		final boolean fontBold = this.font[0].getStyle() == SWT.BOLD;
		final boolean fontItalic = this.font[0].getStyle() == SWT.ITALIC;
		final double fontSize = this.font[0].getHeight();
		final Font gtFont = build.createFont(fontName, fontItalic, fontBold, fontSize);
		...
		if ( pointPlacement ) {
			...
			final double rotation = Double.parseDouble(this.place3.getText());

			placement = build.createPointPlacement(vert, horiz, rotation);
		} else {
			// LinePlacement
			final double offset = Double.parseDouble(this.place.getText());
	 ...
	 * @param mode
	 */
	public void set(final SimpleFeatureType schema, final TextSymbolizer sym, final Mode mode) {
		listen(false);
		try {
			this.schema = schema;
			...
			if ( pointPlacement ) {
				...
				if ( this.labelPlacement == null || !(this.labelPlacement instanceof PointPlacement) ) {
					...
					final AnchorPoint anchor = ((PointPlacement) labelPlacement).getAnchorPoint();
				...
				if ( this.labelPlacement == null || !(this.labelPlacement instanceof LinePlacement) ) {
					...
					final String offset = ((LinePlacement) labelPlacement).getPerpendicularOffset().toString();
	...
	private void initPlacementContentsLine() {
		...
		this.place.setToolTipText("Offset");
	...
	private void initPlacementContentsPoint() {
		final String[] itemsH = new String[] { "Left", "Center", "Right" };
		...
		this.place2.setToolTipText("Horizontal alignment");
		...
		this.place3.setToolTipText("Rotation");
