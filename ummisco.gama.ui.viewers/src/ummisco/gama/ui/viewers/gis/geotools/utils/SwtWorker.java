/*********************************************************************************************
 *
 * 'SwtWorker.java, in plugin ummisco.gama.ui.viewers, is part of the source code of the GAMA modeling and simulation
...
public abstract class SwtWorker<T, V> implements Future<T>, Runnable {
	 ...
	 */
	AccumulativeRunnable<V> doProcess;

	/**
	 * handler for progress property change notifications.
	 */
	AccumulativeRunnable<Integer> doNotifyProgressChange;

	static final AccumulativeRunnable<Runnable> doSubmit = new DoSubmitAccumulativeRunnable();
	 ...
	 * {@code doInBackground} method to deliver intermediate results for processing on the <i>Event Dispatch Thread</i>
	...
	protected final void setProgress(final int progress) {
		if (progress < 0 || progress > 100) { throw new IllegalArgumentException("the value should be from 0 to 100"); }
		if (this.progress == progress) { return; }
		final int oldProgress = this.progress;
		this.progress = progress;
		if (!getPropertyChangeSupport().hasListeners("progress")) { return; }
		synchronized (this) {
	 ...
	 * same listener object may be added more than once, and will be called as many times as it is added. If
	 ...
	 * the <i>Event Dispatch Thread</i> in the event that {@code firePropertyChange} or
	 * {@code fireIndexedPropertyChange} are called off the <i>Event Dispatch Thread</i>.
	 *
		...
		if (executorService == null) {
			...
			executorService = new ThreadPoolExecutor(0, MAX_WORKER_THREADS, 5L, TimeUnit.SECONDS,
				...
				public void execute(final Runnable command) {
					/*
					 * ThreadPoolExecutor first tries to run task in a corePool. If all threads are busy it tries to add
					 * task to the waiting queue. If it fails it run task in maximumPool.
					 *
					 * We want corePool to be 0 and maximumPool to be MAX_WORKER_THREADS We need to change the order of
					 * the execution. First try corePool then try maximumPool pool and only then store to the waiting
					 * queue. We can not do that because we would need access to the private methods.
					 *
					 * Instead we enlarge corePool to MAX_WORKER_THREADS before the execution and shrink it back to 0
					 * after. It does pretty much what we need.
					 *
					 * While we changing the corePoolSize we need to stop running worker threads from accepting new
					 * tasks.
					 */
	...
	static class DoSubmitAccumulativeRunnable extends AccumulativeRunnable<Runnable> implements ActionListener {
		...
		protected void run(final List<Runnable> args) {
			...
			try {
				...
				if (i < args.size()) {
					 ...
					 */
					final Runnable argsTail[] = new Runnable[args.size() - i];
	...
	private class SwingWorkerPropertyChangeSupport extends PropertyChangeSupport {
		...
		public void firePropertyChange(final PropertyChangeEvent evt) {
			...
			if (WorkbenchHelper.getDisplay() != null) {
				...
				doSubmit.add(() -> SwingWorkerPropertyChangeSupport.this.firePropertyChange(evt));
			}
		}
	}
}
