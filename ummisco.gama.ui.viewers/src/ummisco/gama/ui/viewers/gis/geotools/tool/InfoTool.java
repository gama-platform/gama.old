/*********************************************************************************************
 *
 * 'InfoTool.java, in plugin ummisco.gama.ui.viewers, is part of the source code of the
 ...
 * 
 * <p>
 * It works with {@code InfoToolHelper} objects which do
...
public class InfoTool extends CursorTool {
			...
			if ( layer.isSelected() ) {
				...
				if ( helper == null ) {
					if ( Utils.isGridLayer(layer) ) {
						try {
							final Class<?> clazz = Class.forName("ummisco.gama.ui.viewers.gis.geotools.tool.GridLayerHelper");
						...
						try {
							final Class<?> clazz = Class.forName("ummisco.gama.ui.viewers.gis.geotools.tool.VectorLayerHelper");
							final Constructor<?> ctor = clazz.getConstructor(MapContent.class, Layer.class);
							helper = (InfoToolHelper<?>) ctor.newInstance(context, layer);
							helperTable.put(layer, helper);
				...
				if ( helper instanceof VectorLayerHelper ) {
					...
					if ( info != null ) {
						...
						final List<Number> bandValues = (List<Number>) info;
	...
	private void createReporter() {
		if ( reporter == null || reporter.getShell() == null || reporter.getShell().isDisposed() ) {
			final Shell shell = new Shell(Display.getCurrent(), SWT.TITLE | SWT.CLOSE | SWT.BORDER | SWT.MODELESS);
