/*********************************************************************************************
 *
 * 'JFileDataStoreChooser.java, in plugin ummisco.gama.ui.viewers, is part of the source code of the GAMA modeling and
...
public class JFileDataStoreChooser {
	 ...
	 *
	 * @param extension
	 *            the file extension, with or without the leading '.'
	 */
	public JFileDataStoreChooser(final Shell parent, final int style, final String extension) {
		this(parent, style, new String[] { extension });
	}

	static Map<String, String> associations(final List<String> extensions) {
		final Map<String, String> fileAssociations = new TreeMap<>();
		...
		for (final String extension : extensions) {
			...
			final FileDataStoreFactorySpi factory = FileDataStoreFinder.getDataStoreFactory(ext);
			if (factory != null) {
	 ...
	 */
	public JFileDataStoreChooser(final Shell parent, final int style, final List<String> extensions) {
	 ...
	 *            the file extensions, with or without the leading '.'
	 */
	public JFileDataStoreChooser(final Shell parent, final int style, final String[] extensions) {
	 ...
	 *            a {@code Map} where keys are extensions (with or wirhout the leading dot) and values are descriptions.
	 */
	public JFileDataStoreChooser(final Shell parent, final int style, final Map<String, String> fileAssociations) {
	...
	private void init(final Map<String, String> fileAssociations) {
		final String[] exts = new String[fileAssociations.size()];
		final String[] descr = new String[fileAssociations.size()];
		int i = 0;
	 ...
	 *            data file format
	 */
	public JFileDataStoreChooser(final Shell parent, final int style, final FileDataStoreFactorySpi format) {
		fileDialog = new FileDialog(parent, style);
	 ...
	 * @param parent
	 *            parent GUI component (may be {@code null})
	 *
	 * @return the selected file or null if the user cancelled the selection
	 * @throws java.awt.HeadlessException
	 *             if run in an unsupported environment
	 */
	public static File showOpenFile(final String extension, final Shell parent) {
	 ...
	 *            file extension, with or without leading '.'
	 * @param initialDir
	 *            initial directory to display; if {@code null} the initial directory will be the user's default
	 *            directory
	 * @param parent
	 *            parent GUI component (may be {@code null})
	 *
	 ...
	 *             if run in an unsupported environment
	 */
	public static File showOpenFile(final String extension, final File initialDir, final Shell parent) {
		final JFileDataStoreChooser dialog = new JFileDataStoreChooser(parent, SWT.OPEN, extension);
		final FileDialog fileDialogInternal = dialog.getFileDialog();
		if (initialDir != null) {
		...
		final String filePath = fileDialogInternal.getFileName();
	 ...
	 *
	 * @param extensions
	 *            array of file extension, with or without leading '.'
	 * @param parent
	 ...
	 *             if run in an unsupported environment
	 */
	public static File showOpenFile(final String[] extensions, final Shell parent) {
	 ...
	 *             if run in an unsupported environment
	 */
	public static File showOpenFile(final String[] extensions, final File initialDir, final Shell parent) {

		final JFileDataStoreChooser dialog = new JFileDataStoreChooser(parent, SWT.OPEN, extensions);
		final FileDialog fileDialogInternal = dialog.getFileDialog();
		if (initialDir != null) {
			if (initialDir.isDirectory()) {
				fileDialogInternal.setFilterPath(initialDir.getAbsolutePath());
		...
		final String filePath = fileDialogInternal.open();
		if (filePath != null) {
			final File file = new File(filePath);
			return file;
		}
	 ...
	 *             if run in an unsupported environment
	 */
	public static File showOpenFile(final FileDataStoreFactorySpi format, final Shell parent) {
	 ...
	 *             if run in an unsupported environment
	 */
	public static File showOpenFile(final FileDataStoreFactorySpi format, final File initialDir, final Shell parent) {

		final JFileDataStoreChooser dialog = new JFileDataStoreChooser(parent, SWT.OPEN, format);
		final FileDialog fileDialogInternal = dialog.getFileDialog();
		if (initialDir != null) {
			if (initialDir.isDirectory()) {
				fileDialogInternal.setFilterPath(initialDir.getAbsolutePath());
	...
	public static void main(final String arg[]) {
		...
		final File file = JFileDataStoreChooser.showOpenFile("shp", shell);
		if (file != null) {
