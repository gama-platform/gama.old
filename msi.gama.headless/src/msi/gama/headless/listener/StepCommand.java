/*******************************************************************************************************
 *
 * StepCommand.java, in msi.gama.headless, is part of the source code of the GAMA modeling and simulation platform
 * (v.1.9.3).
 *
...
public class StepCommand implements ISocketCommand {
	...
	public CommandResponse execute(final WebSocket socket, final IMap<String, Object> map) {
		...
		if (exp_id == "") return new CommandResponse(GamaServerMessage.Type.MalformedRequest,
				"For 'step', mandatory parameter is: 'exp_id'", map, false);
