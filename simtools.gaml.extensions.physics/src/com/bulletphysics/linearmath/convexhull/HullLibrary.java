...
public class HullLibrary {
	...
	private boolean computeHull(final int vcount, final ObjectArrayList<Vector3f> vertices, final PHullResult result,
			final int vlimit) {
	...
	private Tri extrudable(final float epsilon) {
		...
		for (int i = 0; i < tris.size(); i++) {
			if (t == null || tris.getQuick(i) != null && t.rise < tris.getQuick(i).rise) { t = tris.getQuick(i); }
		}
		return t.rise > epsilon ? t : null;
	}

	private int calchull(final ObjectArrayList<Vector3f> verts, final int verts_count, final IntArrayList tris_out,
	...
	private int calchullgen(final ObjectArrayList<Vector3f> verts, final int verts_count, int vlimit) {
		...
		// int j;
		Vector3f bmin = VECTORS.get(verts.getQuick(0));
		Vector3f bmax = VECTORS.get(verts.getQuick(0));
		...
		try {
			...
			while (vlimit > 0 && (te = extrudable(epsilon)) != null) {
				...
				int j = tris.size();
				while (j-- != 0) {
					if (tris.getQuick(j) == null) { continue; }
					Int3 t = tris.getQuick(j);
					if (above(verts, t, verts.getQuick(v), 0.01f * epsilon)) { extrude(tris.getQuick(j), v); }
				}
				// now check for those degenerate cases where we have a flipped triangle or a really skinny triangle
				j = tris.size();
				while (j-- != 0) {
					if (tris.getQuick(j) == null) { continue; }
					if (!hasvert(tris.getQuick(j), v)) { break; }
				...
				while (j-- != 0) {
					...
					if (t == null) { continue; }
					if (t.vmax >= 0) { break; }
					triNormal(verts.getQuick(t.getCoord(0)), verts.getQuick(t.getCoord(1)),
							verts.getQuick(t.getCoord(2)), n);
					t.vmax = maxdirsterid(verts, verts_count, n, allow);
	...
	private Int4 findSimplex(final ObjectArrayList<Vector3f> verts, final int verts_count, final IntArrayList allow,
			final Int4 out) {
		Vector3f tmp = VECTORS.get();
		Vector3f tmp1 = VECTORS.get();
		Vector3f tmp2 = VECTORS.get();
	...
	private void extrude(final Tri t0, final int v) {
		...
		checkit(tb);
		checkit(tc);
		if (hasvert(tris.getQuick(ta.n.getCoord(0)), v)) { removeb2b(ta, tris.getQuick(ta.n.getCoord(0))); }
		if (hasvert(tris.getQuick(tb.n.getCoord(0)), v)) { removeb2b(tb, tris.getQuick(tb.n.getCoord(0))); }
		if (hasvert(tris.getQuick(tc.n.getCoord(0)), v)) { removeb2b(tc, tris.getQuick(tc.n.getCoord(0))); }
	...
	private void bringOutYourDead(final ObjectArrayList<Vector3f> verts, final int vcount,
			final ObjectArrayList<Vector3f> overts, final int[] ocount, final IntArrayList indices,
	...
	private boolean cleanupVertices(final int svcount, final ObjectArrayList<Vector3f> svertices, final int stride,
			final int[] vcount, // output number of vertices
			final ObjectArrayList<Vector3f> vertices, // location to store the results.
	...
	private static int maxdirfiltered(final ObjectArrayList<Vector3f> p, final int count, final Vector3f dir,
			final IntArrayList allow) {
		assert count != 0;
		int m = -1;
		for (int i = 0; i < count; i++) {
			if (allow.get(i) != 0) { if (m == -1 || p.getQuick(i).dot(dir) > p.getQuick(m).dot(dir)) { m = i; } }
		}
		assert m != -1;
		return m;
	}

	private static int maxdirsterid(final ObjectArrayList<Vector3f> p, final int count, final Vector3f dir,
			final IntArrayList allow) {
		Vector3f tmp = VECTORS.get();
		Vector3f tmp1 = VECTORS.get();
		Vector3f tmp2 = VECTORS.get();
	...
	private static boolean above(final ObjectArrayList<Vector3f> vertices, final Int3 t, final Vector3f p,
			final float epsilon) {
		Vector3f n = triNormal(vertices.getQuick(t.getCoord(0)), vertices.getQuick(t.getCoord(1)),
				vertices.getQuick(t.getCoord(2)), VECTORS.get());
		Vector3f tmp = VECTORS.get();
	...
	private static void addPoint(final int[] vcount, final ObjectArrayList<Vector3f> p, final float x, final float y,
			final float z) {
		// XXX, might be broken
