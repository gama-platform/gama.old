...
public class CollisionDispatcher extends Dispatcher {
	...
	public PersistentManifold getNewManifold(final Object b0, final Object b1) {
		 ...
		 * m_persistentManifoldPoolAllocator->allocate(sizeof(btPersistentManifold)); } else { mem =
		 * btAlignedAlloc(sizeof(btPersistentManifold),16);
		 *
		 * } btPersistentManifold* manifold = new(mem) btPersistentManifold (body0,body1,0); manifold->m_index1a =
		 * m_manifoldsPtr.size(); m_manifoldsPtr.push_back(manifold);
		 */
	...
	public void releaseManifold(final PersistentManifold manifold) {
		...
		/*
		 * manifold->~btPersistentManifold(); if (m_persistentManifoldPoolAllocator->validPtr(manifold)) {
		 * m_persistentManifoldPoolAllocator->freeMemory(manifold); } else { btAlignedFree(manifold); }
		 */
	}
