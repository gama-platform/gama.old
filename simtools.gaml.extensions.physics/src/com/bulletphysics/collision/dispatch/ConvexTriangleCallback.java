...
class ConvexTriangleCallback extends TriangleCallback {
	...
	public void setTimeStepAndCounters(final float collisionMarginTriangle, final DispatcherInfo dispatchInfo,
	...
	public void processTriangle(final Vector3f[] triangle, final int partId, final int triangleIndex) {
		...
		if (dispatchInfoPtr != null && dispatchInfoPtr.debugDraw != null
				&& dispatchInfoPtr.debugDraw.getDebugMode() > 0) {
			Vector3f color = VECTORS.get();
			color.set(255, 255, 0);
			Transform tr = ob.getWorldTransform(TRANSFORMS.get());
			...
			VECTORS.release(color, tmp1, tmp2);
