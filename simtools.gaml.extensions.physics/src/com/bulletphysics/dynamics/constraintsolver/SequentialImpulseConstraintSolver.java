...
public class SequentialImpulseConstraintSolver extends ConstraintSolver {
	...
	private void initSolverBody(final SolverBody solverBody, final CollisionObject collisionObject) {
		...
		if (rb != null) {
			...
			solverBody.centerOfMassPosition.set(collisionObject.getWorldTransform(tmp).origin);
	...
	protected void addFrictionConstraint(final Vector3f normalAxis, final int solverBodyIdA, final int solverBodyIdB,
		...
		VECTORS.release(vec, ftorqueAxis1);
		MATRICES.release(tmpMat);
	}

	public float solveGroupCacheFriendlySetup(final ObjectArrayList<CollisionObject> bodies, final int numBodies,
			final ObjectArrayList<PersistentManifold> manifoldPtr, final int manifold_offset, final int numManifolds,
			final ObjectArrayList<TypedConstraint> constraints, final int constraints_offset, final int numConstraints,
			final ContactSolverInfo infoGlobal, final IDebugDraw debugDrawer/* ,btStackAlloc* stackAlloc */) {
		...
		try {
			...
			// if (1)
			{
				...
				/*
				 * { for (int i=0;i<numBodies;i++) { btRigidBody* rb = btRigidBody::upcast(bodies[i]); if (rb &&
				 * (rb->getIslandTag() >= 0)) { btAssert(rb->getCompanionId() < 0); int solverBodyId =
				 * m_tmpSolverBodyPool.size(); btSolverBody& solverBody = m_tmpSolverBodyPool.expand();
				 * initSolverBody(&solverBody,rb); rb->setCompanionId(solverBodyId); } } }
				 */
	...
	public float solveGroupCacheFriendlyIterations(final ObjectArrayList<CollisionObject> bodies, final int numBodies,
			final ObjectArrayList<PersistentManifold> manifoldPtr, final int manifold_offset, final int numManifolds,
			final ObjectArrayList<TypedConstraint> constraints, final int constraints_offset, final int numConstraints,
			final ContactSolverInfo infoGlobal, final IDebugDraw debugDrawer/* ,btStackAlloc* stackAlloc */) {
				...
				for (iteration = 0; iteration < infoGlobal.numIterations; iteration++) {
...

					{
						...
						for (j = 0; j < numPoolConstraints; j++) {
							SolverConstraint solveManifold =
									tmpSolverConstraintPool.getQuick(orderTmpConstraintPool.get(j));
...

					{
						...
						for (j = 0; j < numFrictionPoolConstraints; j++) {
							SolverConstraint solveManifold =
									tmpSolverFrictionConstraintPool.getQuick(orderFrictionConstraintPool.get(j));
	...
	public float solveGroupCacheFriendly(final ObjectArrayList<CollisionObject> bodies, final int numBodies,
			final ObjectArrayList<PersistentManifold> manifoldPtr, final int manifold_offset, final int numManifolds,
			final ObjectArrayList<TypedConstraint> constraints, final int constraints_offset, final int numConstraints,
			final ContactSolverInfo infoGlobal, final IDebugDraw debugDrawer/* ,btStackAlloc* stackAlloc */) {
		...
		if (infoGlobal.splitImpulse) {
			for (int i = 0; i < tmpSolverBodyPool.size(); i++) {
				...
				BODIES.release(tmpSolverBodyPool.getQuick(i));
			}
		} else {
			for (int i = 0; i < tmpSolverBodyPool.size(); i++) {
		...
		for (int i = 0; i < tmpSolverConstraintPool.size(); i++) {
			CONSTRAINTS.release(tmpSolverConstraintPool.getQuick(i));
		}
		tmpSolverConstraintPool.clear();

		for (int i = 0; i < tmpSolverFrictionConstraintPool.size(); i++) {
			CONSTRAINTS.release(tmpSolverFrictionConstraintPool.getQuick(i));
		}
	 ...
	 */
	@Override
	public float solveGroup(final ObjectArrayList<CollisionObject> bodies, final int numBodies,
			final ObjectArrayList<PersistentManifold> manifoldPtr, final int manifold_offset, final int numManifolds,
			final ObjectArrayList<TypedConstraint> constraints, final int constraints_offset, final int numConstraints,
			final ContactSolverInfo infoGlobal, final IDebugDraw debugDrawer, final Dispatcher dispatcher) {
					...
					if ((infoGlobal.solverMode & SolverMode.SOLVER_RANDMIZE_ORDER) != 0) {
						if ((iteration & 7) == 0) {
							for (j = 0; j < totalPoints; ++j) {
								// JAVA NOTE: swaps references instead of copying values (but that's fine in this
								// context)
	...
	protected void prepareConstraints(final PersistentManifold manifoldPtr, final ContactSolverInfo info,
		...
		for (int i = 0; i < numpoints; i++) {
			...
			if (cp.getDistance() <= 0f) {
				...
				Matrix3f mat1 = body0.getCenterOfMassTransform(tmp).basis;
				mat1.transpose();

				Matrix3f mat2 = body1.getCenterOfMassTransform(tmp).basis;
				mat2.transpose();
		...
		VECTORS.release(tmpVec, pos1, pos2, rel_pos1, rel_pos2, vel1, vel2, vel, totalImpulse, torqueAxis0, torqueAxis1,
				ftorqueAxis0, ftorqueAxis1);
	...
	public float solveCombinedContactFriction(final RigidBody body0, final RigidBody body1, final ManifoldPoint cp,
			final ContactSolverInfo info, final int iter, final IDebugDraw debugDrawer) {
		float maxImpulse = 0f;
