...
public class PhysicalWorldAgent extends GamlAgent {
	...
	private final HashMap<IAgent, Body> registeredMap = new HashMap<IAgent, Body>();
	...
	private PolygonShape[] GamaPolyToPolyPhysic1(final IAgent geom) {
		...
		for ( IShape tr : triangles ) {
			...
			System.out.println("***********");
			trianglesShapes[i] =
				GamaTriangleToPolyPhysic(tr, (float) geom.getLocation().getX(), (float) geom
					.getLocation().getY());
			// if(i==0)
			{
	...
	public PolygonShape GamaTriangleToPolyPhysic(final IShape geom, final float locx,
		final float locy) {
	...
	public Object primComputeForces(final IScope scope) throws GamaRuntimeException {
		...
		Integer velocityIterations =
			scope.hasArg("velocityIterations") ? (Integer) scope.getArg("velocityIterations",
				IType.INT) : 1;
		Integer positionIterations =
			scope.hasArg("positionIterations") ? (Integer) scope.getArg("positionIterations",
				IType.INT) : 1;
		...
		for ( IAgent ia : registeredMap.keySet() ) {
			Body body = registeredMap.get(ia);
			GamaPoint position =
				new GamaPoint(new Double(body.getPosition().x), new Double(body.getPosition().y));
			System.out.println("Position : " + position.getX() + " : " + position.getY());
			ia.setLocation(position);

			GamaPoint velpoint =
				new GamaPoint(new Double(body.getLinearVelocity().x), new Double(
