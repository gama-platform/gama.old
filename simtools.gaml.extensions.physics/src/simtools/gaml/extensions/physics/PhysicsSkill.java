...
public class PhysicsSkill extends Skill {
	 ...
	 *         body.getPosition().y);
	 *         GamaPoint position = new GamaPoint(new Double(body.getPosition().x), new
	 *         Double(body.getPosition().y));
	 *         agent.setLocation(position);
	 *         System.out.println("Position : " + agent.getLocation().getX() + " : " +
	...
	/**
	 * @action("gotoTraffic")
	 * @args({ "target", IKeyword.SPEED, "on", "return_path", LIVING_SPACE, TOLERANCE,
	 *         LANES_ATTRIBUTE })
	 ...
	 *         IPath pathFollowed = moveToNextLocAlongPathTraffic(agent, path, maxDist,livingSpace,
