...
public class OptimizingSkill extends Skill{
            		...
            		for (int j = 0; j < N; j++){
            			if (C.get(scope, i,j) > 0){
                			...
                			rightHandExpression = rightHandExpression + 
                					P.get(scope, i,j)*C.get(scope, i,j);	
            			}
            		}
	...
	public Object primDoOptimizeDirection(final IScope scope) throws GamaRuntimeException {
		...
		if ((fixedPostions == null) || (fixedPostions.size() == 0)){
			return null;
		}
		...
		try{
            ...
            if (tempVariables.size() > 0){
    			...
    			cplex.addEq(cplex.scalProd(tempExpression, tempCoefficients), fixedPostions.length(scope));
    		}
	...
	public Object primDoComputeAverageEvacuationTime(final IScope scope) throws GamaRuntimeException {
		...
		try{
            ...
            if (tempVariables.size() > 0){
    			...
    			cplex.addEq(cplex.scalProd(tempExpression, tempCoefficients), signs_from.length(scope));
    		}
	...
	private boolean isExistSign(IScope scope, int ik, int jk, GamaList<Integer> signs_from, GamaList<Integer> signs_to){
