...
import msi.gama.gui.svn.SVNKitConnectorFactory.SVNKitConnector.ProgressMonitorWrapper;
...
public class SVNKitConnectorFactory implements ISVNConnectorFactory {
	...
	public static final class ConversionUtility {
		...
		public static org.tigris.subversion.javahl.ConflictResolverCallback convert(
			...
			return new org.tigris.subversion.javahl.ConflictResolverCallback() {
				...
				public org.tigris.subversion.javahl.ConflictResult resolve(
					final org.tigris.subversion.javahl.ConflictDescriptor descrip) throws RuntimeException {
					try {
		...
		public static SVNConflictDescriptor convert(final org.tigris.subversion.javahl.ConflictDescriptor descr) {
			return descr == null ? null : new SVNConflictDescriptor(descr.getPath(), descr.getKind(),
				descr.getNodeKind(), descr.getPropertyName(), descr.isBinary(), descr.getMIMEType(), descr.getAction(),
				descr.getReason(), descr.getOperation(), descr.getBasePath(), descr.getTheirPath(), descr.getMyPath(),
				descr.getMergedPath(), ConversionUtility.convert(descr.getSrcLeftVersion()),
		...
		public static org.tigris.subversion.javahl.ConflictResult convert(final SVNConflictResolution result) {
			return result == null ? null : new org.tigris.subversion.javahl.ConflictResult(result.choice,
				result.mergedPath);
		}
		...
		public static SVNEntryInfo convert(final org.tigris.subversion.javahl.Info2 info) {
			return info == null ? null : new SVNEntryInfo(info.getPath(), null, info.getUrl(), info.getRev(),
				...
				info.getLastChangedDate() == null ? 0 : info.getLastChangedDate().getTime(),
				info.getLastChangedAuthor(), ConversionUtility.convert(info.getLock()), info.isHasWcInfo(),
				info.getSchedule(), info.getCopyFromUrl(), info.getCopyFromRev(), info.getTextTime() == null ? 0 : info
					.getTextTime().getTime(), info.getPropTime() == null ? 0 : info.getPropTime().getTime(),
				info.getChecksum() == null ? null : new SVNChecksum(SVNChecksum.LEGACY, info.getChecksum().getBytes()),
				null, 0, 0, ISVNConnector.Depth.UNKNOWN, info.getConflictDescriptor() == null ? null
		...
		public static SVNChangeStatus convert(final org.tigris.subversion.javahl.Status st) {
			return st == null ? null : new SVNChangeStatus(st.getPath(), st.getUrl(), st.getNodeKind(),
				...
				st.getRepositoryTextStatus(), st.getRepositoryPropStatus(), st.isLocked(), st.isCopied(),
				st.isSwitched(), st.getLockToken() == null ? null : new SVNLock(st.getLockOwner(), st.getPath(),
					st.getLockToken(), st.getLockComment(), st.getLockCreationDate() == null ? 0 : st
						.getLockCreationDate().getTime(), 0), ConversionUtility.convert(st.getReposLock()),
				st.getReposLastCmtRevisionNumber(), st.getReposLastCmtDate() == null ? 0 : st.getReposLastCmtDate()
					.getTime(), st.getReposKind(), st.getReposLastCmtAuthor(), st.isFileExternal(),
				st.getConflictNew() != null, st.getConflictNew() == null ? null
					: new SVNConflictDescriptor[] { new SVNConflictDescriptor(st.getPath(),
		...
		public static SVNLock convert(final org.tigris.subversion.javahl.Lock lock) {
			return lock == null ? null : new SVNLock(lock.getOwner(), lock.getPath(), lock.getToken(),
				lock.getComment(), lock.getCreationDate() == null ? 0 : lock.getCreationDate().getTime(),
		...
		public static org.tigris.subversion.javahl.BlameCallback2 convert(final ISVNAnnotationCallback cb) {
			...
			return new org.tigris.subversion.javahl.BlameCallback2() {
				...
				@Override
				public void singleLine(final Date date, final long revision, final String author,
					final Date merged_date, final long merged_revision, final String merged_author,
					final String merged_path, final String line) {
					cb.annotate(line, new SVNAnnotationData(0, false, revision, date == null ? 0 : date.getTime(),
						author, merged_revision, merged_date == null ? 0 : merged_date.getTime(), merged_author,
						merged_path));
				}
			};
		}
		...
		public static org.tigris.subversion.javahl.Revision convert(final SVNRevision rev) {
			if ( rev != null ) {
				switch (rev.getKind()) {
					...
					case Kind.NUMBER:
						return org.tigris.subversion.javahl.Revision
							.getInstance(((SVNRevision.Number) rev).getNumber());
					case Kind.DATE:
					default:
		...
		public static SVNNotification convert(final org.tigris.subversion.javahl.NotifyInformation info) {
			return info == null ? null : new SVNNotification(info.getPath(), info.getAction(), info.getKind(),
				info.getMimeType(), ConversionUtility.convert(info.getLock()), info.getErrMsg(),
				info.getContentState(), info.getPropState(), info.getLockState(), info.getRevision());
		}
	...
	public static class SVNKitConnector implements ISVNConnector {
		...
		public SVNKitConnector() {
			...
			SVNSSHSession.setUsePersistentConnection(SVNTeamPlugin.instance().getOptionProvider()
				.isPersistentSSHEnabled());
		}
		...
		public long checkout(SVNEntryRevisionReference fromReference, String destPath, int depth, long options,
			...
			try {
				...
				wrapper.start();
				long retVal =
					this.client.checkout(fromReference.path, destPath,
						ConversionUtility.convert(fromReference.revision),
		...
		public long[] commit(String[] path, String message, String[] changelistNames, int depth, long options,
			...
			Map<String, Object> parameters = new HashMap<String, Object>();
			parameters.put("path", path);
			parameters.put("mainTaskName", message);
			...
			path = (String[]) parameters.get("path");
			message = (String) parameters.get("mainTaskName");
			...
			try {
				...
				long[] retVal =
					this.client.commit(path, message, depth, (options & Options.KEEP_LOCKS) != 0,
						(options & Options.KEEP_CHANGE_LIST) != 0, changelistNames,
						ConversionUtility.convert(revProps), true);
		...
		public void mergeReintegrate(SVNEntryReference reference, String localPath, long options,
			...
			try {
				this.composite.add(wrapper);
				wrapper.start();
				this.client.mergeReintegrate(reference.path, ConversionUtility.convert(reference.pegRevision),
					localPath, (options & Options.SIMULATE) != 0);
		...
		@Override
		public void removeFromChangeLists(String[] paths, int depth, String[] changelistNames,
			ISVNProgressMonitor monitor) throws SVNConnectorException {
			Map<String, Object> parameters = new HashMap<String, Object>();
			parameters.put("paths", paths);
			parameters.put("depth", Integer.valueOf(depth));
		...
		@Override
		public void merge(final SVNEntryReference reference, final SVNRevisionRange[] revisions,
		...
		@Override
		public void mergeStatus(final SVNEntryReference reference, final SVNRevisionRange[] revisions,
		...
		@Override
		public void merge(final SVNEntryReference reference, final String mergePath,
			...
			try {
				...
				for ( int i = 0; i < mergeStatus.length && !monitor.isActivityCancelled(); i++ ) {
					...
					if ( mergeStatus[i].textStatus == org.eclipse.team.svn.core.connector.SVNEntryStatus.Kind.DELETED ) {
						...
						for ( File parent = localFile.getParentFile(); !parent.exists() &&
						...
						this.client.merge(mergeStatus[i].startUrl,
							...
							(options & Options.FORCE) != 0, Depth.FILES, (options & Options.IGNORE_ANCESTRY) != 0,
							false, (options & Options.RECORD_ONLY) != 0);
		...
		protected void mergeStatus(final SVNEntryReference reference1, final SVNEntryRevisionReference reference2,
			...
			try {
				...
				if ( reference2 != null ) {
					...
					this.client.mergeReintegrate(reference1.path, ConversionUtility.convert(reference1.pegRevision),
						path, (options & Options.SIMULATE) != 0);
				}
				...
				SVNRevision to =
					reference2 == null ? revisions != null ? revisions[revisions.length - 1].to
						: reference1.pegRevision : reference2.revision;
				...
				if ( to.getKind() != SVNRevision.Kind.NUMBER ) {
					SVNLogEntry[] entries =
						SVNUtility.logEntries(this, reference2 == null ? reference1 : reference2, to,
							SVNRevision.fromNumber(1), ISVNConnector.Options.NONE, ISVNConnector.EMPTY_LOG_ENTRY_PROPS,
							1, monitor);
			...
			while (end > start) {
				...
				if ( !referenceToExisting.pegRevision.equals(lastRevision) ) {
					...
					SVNEntryReference tRef =
						new SVNEntryReference(referenceToExisting.path, SVNRevision.fromNumber(start));
					while (!this.exists(tRef, monitor)) {
						tRef =
							new SVNEntryReference(tRef.path.substring(0, tRef.path.lastIndexOf("/")), tRef.pegRevision);
					}
					SVNLogEntry[] log =
		...
		protected SVNEntryReference getLastValidReference(SVNEntryReference referenceToExisting,
			...
			do {
				...
				SVNEntryReference tRef =
					new SVNEntryReference(referenceToExisting.path, SVNRevision.fromNumber(middle));
		...
		public long doExport(SVNEntryRevisionReference fromReference, String destPath, String nativeEOL, int depth,
			...
			try {
				...
				wrapper.start();
				long retVal =
					this.client.doExport(fromReference.path, destPath,
						ConversionUtility.convert(fromReference.revision),
		...
		@Override
		public void diff(SVNEntryRevisionReference reference1, SVNEntryRevisionReference reference2,
			String relativeToDir, String outFileName, int depth, long options, String[] changelistNames,
			ISVNProgressMonitor monitor) throws SVNConnectorException {
			Map<String, Object> parameters = new HashMap<String, Object>();
			parameters.put("reference1", reference1);
			parameters.put("reference2", reference2);
		...
		@Override
		public void diff(SVNEntryReference reference, SVNRevision revision1, SVNRevision revision2,
			String relativeToDir, String outFileName, int depth, long options, String[] changelistNames,
			ISVNProgressMonitor monitor) throws SVNConnectorException {
			Map<String, Object> parameters = new HashMap<String, Object>();
			parameters.put("reference", reference);
			...
			try {
				...
				this.client.diff(reference.path, ConversionUtility.convert(reference.pegRevision),
					ConversionUtility.convert(revision1), ConversionUtility.convert(revision2), relativeToDir,
					outFileName, depth, changelistNames, (options & Options.IGNORE_ANCESTRY) != 0,
		...
		public void diffStatus(SVNEntryRevisionReference reference1, SVNEntryRevisionReference reference2, int depth,
			...
			try {
				...
				this.client.diffSummarize(reference1.path, ConversionUtility.convert(reference1.revision),
					reference2.path, ConversionUtility.convert(reference2.revision), depth, changelistNames,
		...
		public void info(SVNEntryRevisionReference reference, int depth, String[] changelistNames,
			...
			try {
				this.composite.add(wrapper);
				wrapper.start();
				this.client.info2(reference.path, ConversionUtility.convert(reference.revision),
		...
		@Override
		public void copy(SVNEntryRevisionReference[] srcPaths, String destPath, String message, long options,
			Map revProps, ISVNProgressMonitor monitor) throws SVNConnectorException {
			Map<String, Object> parameters = new HashMap<String, Object>();
			parameters.put("srcPaths", srcPaths);
			parameters.put("destPath", destPath);
		...
		public void logEntries(SVNEntryReference reference, SVNRevisionRange[] revisionRanges, String[] revProps,
			...
			try {
				...
				this.client.logMessages(reference.path, ConversionUtility.convert(reference.pegRevision),
					...
					(options & Options.DISCOVER_PATHS) != 0, (options & Options.INCLUDE_MERGED_REVISIONS) != 0,
					revProps, limit, ConversionUtility.convert(cb));
		...
		@Override
		public void annotate(SVNEntryReference reference, SVNRevision revisionStart, SVNRevision revisionEnd,
			long options, ISVNAnnotationCallback callback, ISVNProgressMonitor monitor) throws SVNConnectorException {
			Map<String, Object> parameters = new HashMap<String, Object>();
			parameters.put("reference", reference);
					...
					(options & Options.FETCH_LOCKS) != 0, new ListCallback() {
						...
						public void doEntry(final org.tigris.subversion.javahl.DirEntry dirent,
							...
							if ( path != null && path.length() != 0 ||
								...
								cb1.next(new SVNEntry(path, dirent.getLastChangedRevisionNumber(), date == null ? 0
									: date.getTime(), dirent.getLastAuthor(), dirent.getHasProps(), dirent
									.getNodeKind(), dirent.getSize(), ConversionUtility.convert(lock)));
							}
						}
					});
		...
		public SVNProperty getProperty(SVNEntryRevisionReference reference, String name, ISVNProgressMonitor monitor)
			...
			try {
				...
				SVNProperty retVal =
					ConversionUtility
						.convert(this.client.propertyGet(reference.path, name,
							ConversionUtility.convert(reference.revision),
							ConversionUtility.convert(reference.pegRevision)));
		...
		public void setRevisionProperty(SVNEntryReference reference, SVNProperty property, String originalValue,
			...
			try {
				this.composite.add(wrapper);
				wrapper.start();
				this.client.setRevProperty(reference.path, property.name,
					ConversionUtility.convert(reference.pegRevision), property.value, originalValue,
					(options & Options.FORCE) != 0);
		...
		@Override
		public void patch(String patchPath, String targetPath, int stripCount, long options,
			ISVNPatchCallback callback, ISVNProgressMonitor monitor) throws SVNConnectorException {
			Map<String, Object> parameters = new HashMap<String, Object>();
		...
		private class DiffCallback implements DiffSummaryReceiver {
			...
			public DiffCallback(final String prev, final String next, final boolean isFile,
				final ISVNDiffStatusCallback cb) {
		...
		protected class RepositoryInfoPrompt implements PromptUserPasswordSSL, PromptUserPasswordSSH, PromptUserPasswordProxy {
			...
			@Override
			public boolean
				promptSSH(final String realm, final String username, final int sshPort, final boolean maySave) {
		...
		public static ISVNProgressMonitor.ItemState makeItemState(final SVNNotification arg0) {
			return new ISVNProgressMonitor.ItemState(arg0.path, arg0.action, arg0.kind, arg0.mimeType,
				arg0.contentState, arg0.propState, arg0.revision, arg0.errMsg);
		}
		...
		protected void fireSucceeded(final String methodName, final Map<String, Object> parameters,
