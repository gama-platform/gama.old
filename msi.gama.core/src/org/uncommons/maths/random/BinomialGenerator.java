...
import org.uncommons.maths.binary.*;
...
public class BinomialGenerator implements NumberGenerator<Integer> {
	...
	/**
	 * <p>
	 * Creates a generator of binomially-distributed values. The number of trials ({@literal n}) and
	 * the probability of success in each trial ({@literal p}) are determined by the provided
	 * {@link NumberGenerator}s. This means that the statistical parameters of this generator may
	 * change over time. One example of where this is useful is if the {@literal n} and {@literal p}
	 * generators are attached to GUI controls that allow a user to tweak the parameters while a
	 * program is running.
	 * </p>
	 * <p>
	 * To create a Binomial generator with a constant {@literal n} and {@literal p}, use the
	 * {@link #BinomialGenerator(int, double, Random)} constructor instead.
	 * </p>
	 ...
	 */
	public BinomialGenerator(final NumberGenerator<Integer> n, final NumberGenerator<Double> p,
		final Random rng) {
		this.n = n;
		this.p = p;
		this.rng = rng;
