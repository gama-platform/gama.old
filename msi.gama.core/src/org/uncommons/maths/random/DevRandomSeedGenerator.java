 ...
 * 
 *
 * 'DevRandomSeedGenerator.java', in plugin 'msi.gama.core', is part of the source code of the 
	...
	public byte[] generateSeed(final int length) throws SeedException {
		...
		try {
			...
			while (count < length) {
				...
				if ( bytesRead == -1 ) { throw new SeedException(
					"EOF encountered reading random data."); }
				count += bytesRead;
			}
			return randomSeed;
			...
			// an applet sandbox).
			throw new SeedException("SecurityManager prevented access to " + DEV_RANDOM.getName(),
				ex);
		} finally {
			if ( file != null ) {
				try {
