...
@facets(value = {
	...
	@facet(name = STYLE,
		...
		optional = true,
		doc = @doc(deprecated = "Use the 'font' operator together with the #bold, #italic and #plain constant styles",
	...
	@facet(name = IKeyword.REFRESH,
		type = IType.BOOL,
		optional = true,
		doc = @doc("(openGL only) specify whether the display of the text is refreshed. (true by default, usefull in case of text that is not been modified over simulation)")) },
	omissible = IKeyword.NAME)
@doc(value = "`" + IKeyword.TEXT +
	...
	usages = {
		...
		@usage(value = "For instance, in a segregation model, `agents` will only display unhappy agents:",
			examples = {
				...
				@example(value = "   text 'Carrying ants : ' + (int(ant as list count(each.has_food)) + int(ant as list count(each.state = 'followingRoad'))) position: {0.5,0.03} color: rgb('black') size: {1,0.02};",
...
public class TextLayerStatement extends AbstractLayerStatement {
	...
	public TextLayerStatement(final IDescription desc) throws GamaRuntimeException {
		...
		if ( getFacet(IKeyword.SIZE) == null ) {
			...
			IExpression s =
				ef.createOperator("as", desc, desc.getUnderlyingElement(IKeyword.FONT), f,
	...
	public boolean _step(final IScope scope) {
		...
		if ( style != null ) {
			int currentStyle = DrawStatement.CONSTANTS.get(Cast.asString(scope, style.value(scope)));
