...
public class ChartJFreeChartOutputBoxAndWhiskerCategory extends ChartJFreeChartOutput {
	...
	/*
			if (flat) {
				BoxAndWhiskerRenderer.setDefaultBarPainter(new StandardBarPainter());
				BoxAndWhiskerRenderer.setDefaultShadowsVisible(false);
			} else {
		...
		protected AbstractRenderer createRenderer(final IScope scope, final String serieid) {
			...
			AbstractRenderer newr = new BoxAndWhiskerRenderer();
			return newr;
		}
				...
				for (int i = 0; i < CValues.size(); i++) {
					if (getY_LogScale(scope)) {
						...
						if (val <= 0) {
							...
							serie.add(new BoxAndWhiskerItem(YValues.get(i), SValues.get(i), dataserie.xerrvaluesmin.get(i),dataserie.xerrvaluesmax.get(i), dataserie.yerrvaluesmin.get(i),dataserie.yerrvaluesmax.get(i),null,null, null), serieid, CValues.get(i));
						}
						...
						serie.add(new BoxAndWhiskerItem(YValues.get(i), 
								...
								dataserie.yerrvaluesmax.size()>i?dataserie.yerrvaluesmax.get(i):YValues.get(i),
								null,null,null), serieid, CValues.get(i));
