...
@facets(value = {
		 ...
		 * true),
		 */
		@facet(name = ChartLayerStatement.XRANGE, type = { IType.FLOAT, IType.INT,
				IType.POINT }, optional = true, doc = @doc("range of the x-axis. Can be a number (which will set the axis total range) or a point (which will set the min and max of the axis).")),
		@facet(name = ChartLayerStatement.YRANGE, type = { IType.FLOAT, IType.INT,
				IType.POINT }, optional = true, doc = @doc("range of the y-axis. Can be a number (which will set the axis total range) or a point (which will set the min and max of the axis).")),
		...
		@facet(name = IKeyword.TRANSPARENCY, type = IType.FLOAT, optional = true, doc = @doc("the transparency of the chart")),
		...
		@facet(name = IKeyword.NAME, type = IType.LABEL, optional = true, doc = @doc("the human readable title of the chart layer")),
...
public class ChartLayerStatement extends AbstractLayerStatement {
		...
		for (int i = 0; i < datas.size(); i++) {
			...
			if (i != 0 | !isTimeSeries) { // the first data is the domain
				...
				if (type == SERIES_CHART || type == XY_CHART) {
					...
					// final XYSeries nserie = new XYSeries(serie.getKey(),
					// false, false);
				...
				// ((XYLineAndShapeRenderer)
				// plot.getRenderer(i)).setBaseShapesFilled(false);
				// TODO Control this with a facet
				// ((XYLineAndShapeRenderer)
				// plot.getRenderer(i)).setSeriesShapesVisible(0, false);
		...
		for (int i = 0; i < datas.size(); i++) {
			...
			for (int j = 0; j < categoryCount; j++) {
				...
				for (int k = 0; k < entityCount; k++) {
					...
					final double value2 = 11.25 + scope.getRandom().next(); // concentrate
																			// values
																			// in
																			// the
																			// middle
	...
	private void createBars(final IScope scope) {
		...
		for (final ChartData e : datas) {
			...
			// ((DefaultCategoryDataset) dataset).setValue(0d, new Integer(0),
			// legend/* , legend */);
		...
		yAxis.setTickLabelFont(getTickFont());
		yAxis.setLabelFont(getLabelFont());
		final IExpression expr = getFacet(YRANGE);
		final IExpression expr2 = getFacet(YTICKUNIT);
		if (expr != null) {
			final Object range = expr.value(scope);
			// Double range = Cast.asFloat(scope, expr.value(scope));
		...
		final CategoryAxis axis = plot.getDomainAxis();
		final Double gap = Cast.asFloat(scope, getFacetValue(scope, IKeyword.GAP, 0.01));
	...
	private void createChart(final IScope scope) {
		switch (type) {
		case SERIES_CHART: {
			chart = ChartFactory.createXYLineChart(getName(), "time", "", null, PlotOrientation.VERTICAL, true, false,
					false);
			break;
		}
		case PIE_CHART: {
		...
		case HISTOGRAM_CHART: {
			if (style.equals(IKeyword.THREE_D)) {
				chart = ChartFactory.createBarChart3D(getName(), null, null, null, PlotOrientation.VERTICAL, true, true,
						false);
			} else if (style.equals(IKeyword.STACK)) {
				chart = ChartFactory.createStackedBarChart(getName(), null, null, null, PlotOrientation.VERTICAL, true,
						true, false);
			} else {
				chart = ChartFactory.createBarChart(getName(), null, null, null, PlotOrientation.VERTICAL, true, true,
						false);
			}
			break;
		}
	...
	public boolean _init(final IScope scope) throws GamaRuntimeException {
		...
		final IExpression color = getFacet(IKeyword.AXES);
		if (color != null) {
			axesColor = Cast.asColor(scope, color.value(scope));
		}
		final IExpression color1 = getFacet(IKeyword.BACKGROUND);
		if (color1 != null) {
			backgroundColor = Cast.asColor(scope, color1.value(scope));
		}
		final IExpression string = getFacet(IKeyword.STYLE);
		if (string != null) {
	...
	public void updateseries(final IScope scope) throws GamaRuntimeException {
		...
		for (int dl = 0; dl < datalists.size(); dl++) {
			final ChartDataList datalist = datalists.get(dl);

			final Object val = datalist.valuelistexp.resolveAgainst(scope).value(scope);
			...
			if (datalist.doreverse) {
				final List tempvalues = Cast.asList(scope, val);
				values = new ArrayList<>();
				if (tempvalues.get(0) instanceof GamaList) {
					final IList nval = Cast.asList(scope, tempvalues.get(0));
					...
					// scope.getGui().debug("Reverse series but not list of
					// list..." + tempvalues);
					return;
				...
				if (tempvalues.size() > 1) {
					for (int i = 1; i < tempvalues.size(); i++) {
						if (tempvalues.get(i) instanceof GamaList) {
							final IList nval = Cast.asList(scope, tempvalues.get(i));
							for (int j = 0; j < nval.size(); j++) {
								// Cast.asList(scope,
							...
							// scope.getGui().debug("Reverse series but not list
							// of list..." + tempvalues);
							return;
			...
			final List defaultnames = new ArrayList<String>();
			final List defaultcolors = new ArrayList<GamaColor>();
			...
			if (datalist.colorlistexp != null) {
				final Object valcol = datalist.colorlistexp.resolveAgainst(scope).value(scope);
				if (valcol instanceof GamaList) {
					for (int c = 0; c < ((GamaList) valcol).size(); c++) {
						// if ( type == SERIES_CHART)
						// defaultcolors.set(c+1, Cast.asColor(scope,
						// ((GamaList)valcol).get(c)));
						// else
			...
			if (datalist.legendlistexp != null) {
				final Object valc = datalist.legendlistexp.resolveAgainst(scope).value(scope);
			...
			final int nbseries = values.size();
				...
				for (int i = datalist.previoussize; i < nbseries; i++) {
					...
					try {
						...
						newdata = ChartDataListStatement.newChartData(scope, r,
								Cast.asString(scope, defaultnames.get(i)), Cast.asColor(scope, defaultcolors.get(i)),
								values.get(i));
						...
						if (type == HISTOGRAM_CHART) {
							...
							for (final ChartData e : datas) {
								...
								// dataset).setValue(0d, new Integer(0),
								// legend/* , legend
								// */);
			...
			/*
			 * if (datalist.categlistexp!=null) { Object
			 * valc=datalist.categlistexp.resolveAgainst(scope).value(scope); if
			 * ((valc instanceof GamaList)) { dynamiccategorynames=true;
			 * categorynames=(GamaList)valc; }
			 *
			 * if (type==HISTOGRAM_CHART) { for ( int i=0; i<values.size(); i++
			 * ) { GamaList x = new GamaList(); Object obj = values.get(i); if (
			 * obj instanceof GamaList ) { x = (GamaList) obj; //
			 * clearvalues=true; if (dynamiccategorynames) { for (int
			 * j=0;j<x.length(scope);j++) if (j<categorynames.size()) {
			 * ((DefaultCategoryDataset) dataset).setValue(Cast.asFloat(scope,
			 * x.get(j)).doubleValue(),(String)defaultnames.get(i),categorynames
			 * .get(j).toString()+"("+j+")"); } else ((DefaultCategoryDataset)
			 * dataset).setValue(Cast.asFloat(scope,
			 * x.get(j)).doubleValue(),(String)defaultnames.get(i),"("+j+")"); }
			 * else { for (int j=0;j<x.length(scope);j++)
			 * ((DefaultCategoryDataset) dataset).setValue(Cast.asFloat(scope,
			 * x.get(j)).doubleValue(),(String)defaultnames.get(i), new
			 * Integer(j));
			 *
			 * } } else { ((DefaultCategoryDataset)
			 * dataset).setValue(Cast.asFloat(scope, obj).doubleValue(), new
		...
		if (type == PIE_CHART) {
			...
			for (int dl = 0; dl < datalists.size(); dl++) {
				...
				if (datalist.legendlistexp != null) {
					...
					if (valc instanceof GamaList) {
						// dynamicseriesnames = true;
						final GamaList seriesnames = (GamaList) valc;
				...
				if (datalist.categlistexp != null) {
					final Object valc = datalist.categlistexp.resolveAgainst(scope).value(scope);
					if (valc instanceof GamaList) {
				...
				if (dataset != null) {
					for (final ChartData d : datas) {
						...
						final Object obj = d.getValue(scope);
						...
						case HISTOGRAM_CHART: {
							// scope.getGui().debug("ChartLayerStatement._step
							// row " + ((DefaultCategoryDataset)
							// dataset).getRowCount() +
							// " col " + ((DefaultCategoryDataset)
							// dataset).getColumnCount());
							// ((DefaultCategoryDataset) dataset).setValue(n,
							// new Integer(0), s);
							break;
						}
						}
	...
	public boolean _step(final IScope scope) throws GamaRuntimeException {
		...
		/*
		 * switch (type) { case XY_CHART: computeSeries(scope,
		 * lastComputeCycle); case SERIES_CHART: computeSeries(scope,
		 * lastComputeCycle); return true; }
		 */
			...
			for (final Map.Entry<String, Double> d : lastValues.entrySet()) {
				...
				if (!(d instanceof GamaList)) {
					...
					case HISTOGRAM_CHART: {
						...
						// scope.getGui().debug("ChartLayerStatement._step row "
						// + ((DefaultCategoryDataset)
						// dataset).getRowCount() +
						// " col " + ((DefaultCategoryDataset)
						// dataset).getColumnCount());
		...
		if (!(type == SERIES_CHART & datas.size() < 2)) {
			for (int i = 0; i < datas.size(); i++) {
				if (!datasfromlists.contains(datas.get(i))) {
					...
					// DefaultTableXYDataset data = (DefaultTableXYDataset)
					// plot.getDataset(i);
					final XYDataset data = plot.getDataset(i);
					XYSeries serie = new XYSeries(0, false, false);
					if (type == SERIES_CHART || type == XY_CHART) {
					...
					final Object o = datas.get(i).getValue(scope);
					...
					for (int j = 0; j < n.size(); j++) {
						if (type == SERIES_CHART) {
							final double d2 = Cast.asFloat(scope, n.get(j));
							double d1;
							if (cumulative) {
							...
							try {
								final IList<Double> list = GamaListType.staticCast(scope, n.get(j), Types.FLOAT, false);
								final double d1 = list.get(0);
								final double d2 = list.get(1);
								if (cumulative) {
									serie.addOrUpdate(d1, d2);
								} else {
									serie.addOrUpdate(d1, d2);
								...
								// GamaRuntimeException g =
								// GamaRuntimeException.create(e,scope);
								// g.addContext("each point value should be a
								// gama-point or a 2-float list, value here:
								// "+(Cast.asList(scope, n.get(j))));
								final GamaRuntimeException g = GamaRuntimeException
	...
	public void saveHistory() {
		final IScope scope = output.getScope().copy("of History saving in ChartLayerStatement");
		if (scope == null) {
			return;
		}
		try {
