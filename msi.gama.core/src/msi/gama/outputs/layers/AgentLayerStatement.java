 ...
 *
 */
@symbol(name = IKeyword.AGENTS, kind = ISymbolKind.LAYER, with_sequence = true, concept = { IConcept.SPECIES,
		IConcept.DISPLAY })
@inside(symbols = IKeyword.DISPLAY)
@facets(value = {
		@facet(name = IKeyword.VALUE, type = IType.CONTAINER, of = IType.AGENT, optional = false, doc = @doc("the set of agents to display")),
		...
		@facet(name = IKeyword.NAME, type = IType.LABEL, optional = true, // Changed
																			// for
																			// issue
																			// #1300
				doc = @doc("Human readable title of the layer")),
		@facet(name = IKeyword.FOCUS, type = IType.AGENT, optional = true, doc = @doc("the agent on which the camera will be focused (it is dynamically computed)")),
		...
		@facet(name = IKeyword.REFRESH, type = IType.BOOL, optional = true, doc = @doc("(openGL only) specify whether the display of the species is refreshed. (true by default, useful in case of agents that do not move)")) }, omissible = IKeyword.NAME)
@validator(AgentLayerValidator.class)
@doc(value = "`" + IKeyword.AGENTS
		+ "` allows the modeler to display only the agents that fulfill a given condition.", usages = {
				...
				@usage(value = "For instance, in a segregation model, `agents` will only display unhappy agents:", examples = {
						...
						@example(value = "}", isExecutable = false) }) }, see = { IKeyword.DISPLAY, IKeyword.CHART,
...
public class AgentLayerStatement extends AbstractLayerStatement {
	...
	public static class AgentLayerValidator implements IDescriptionValidator<StatementDescription> {
		...
		public void validate(final StatementDescription description) {
			...
			if (ed != null) {
				target = description.computeSpecies();
				if (target == null) {
