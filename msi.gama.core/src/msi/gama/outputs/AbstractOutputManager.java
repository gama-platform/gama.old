 ...
 * 
 */
public abstract class AbstractOutputManager extends Symbol implements
		IOutputManager {
	...
	public void addOutput(final IOutput output) {
		if (output == null || outputs.containsValue(output)) {
			return;
		}
			...
			if (scope.init(output)) {
				...
				if (scope.step(output)) {
					try {
						...
						GuiUtils.debug("Error in AbstractOutputManager.step "
								+ e.getMessage());
						return false;
					}
				}
