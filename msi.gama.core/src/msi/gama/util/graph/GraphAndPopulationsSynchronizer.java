...
public class GraphAndPopulationsSynchronizer implements IPopulation.Listener, IGraphEventListener {
	...
	public void notifyAgentsAdded(final IPopulation pop, final Collection agents) {
		...
		/*
		 * if (pop == popEdges) { for (Object o : agents) { if
		 * (!graph.containsEdge(o)) { throw new GamaRuntimeException(
	...
	public void receiveEvent(final GraphEvent event) {
		...
		case EDGE_ADDED: {
			if (currentEventEdge != event.edge) {
				...
				final IList<? extends IAgent> createdAgents = popEdges.createAgents(event.scope, 1, initialValues,
						false, true);
				final IAgent createdAgent = createdAgents.get(0);

				// create the shape for this agent
				final GamaShape dl = GamaGeometryType.buildLink(event.scope, (IShape) graph.getEdgeSource(event.edge),
