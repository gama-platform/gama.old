...
public class GraphUtilsGephi {
	...
	private static void processGamaAttribute(final String gamaKey, final Object gamaValue, final Node node,
		final Edge edge) {
		...
		if ( gamaValue instanceof GamaColor ) {
			...
			if ( node != null ) {
				node.getNodeData().setColor((float) (gamaColor.getRed() / 255.0),
					(float) (gamaColor.getGreen() / 255.0), (float) (gamaColor.getBlue() / 255.0));
			}

			if ( edge != null ) {
				edge.getEdgeData().setColor((float) (gamaColor.getRed() / 255.0),
					(float) (gamaColor.getGreen() / 255.0), (float) (gamaColor.getBlue() / 255.0));
			}
		} else {
			if ( node != null ) {
	 ...
	 * @return
	 */
	public static Workspace loadIntoAGephiWorkspace(final IGraph gamaGraph) {
		...
		synchronized (gephiStaticLocker) {
			...
			if ( gephiCurrentProject == null ) { throw new RuntimeException(
				"unable to init a novel Gephi project, sorry."); }

			gephiCurrentWorkspace = pc.newWorkspace(gephiCurrentProject);

			if ( gephiCurrentWorkspace == null ) { throw new RuntimeException(
				"unable to init a novel Gephi workspace, sorry."); }
		...
		for ( Object gamaV : gamaGraph._internalVertexMap().keySet() ) {
			...
			if ( gamaV instanceof IAgent ) {
				...
				for ( Object gamaAttributeName : gamaAgent.getAttributes().keySet() ) {
					...
					if ( gephiColumn == null ) {
						gephiColumn =
							attributeModel.getNodeTable().addColumn(gamaAttributeName.toString(), AttributeType.STRING);
		...
		for ( Object edgeObj : gamaGraph._internalEdgeMap().keySet() ) {
			...
			if ( edgeObj instanceof IAgent ) {
				...
				for ( Object gamaAttributeName : gamaAgent.getAttributes().keySet() ) {
					...
					if ( gephiColumn == null ) {
						gephiColumn =
							attributeModel.getEdgeTable().addColumn(gamaAttributeName.toString(), AttributeType.STRING);
						// TODO everything is string ???
		...
		for ( Object v : gamaGraph._internalVertexMap().keySet() ) {
			...
			if ( v instanceof IAgent ) {
				...
				for ( Object key : a.getAttributes().keySet() ) {
					...
					processGamaAttribute(key.toString(), value, createdNode, null);
				}
			} else {
		...
		for ( Object edgeObj : gamaGraph._internalEdgeMap().keySet() ) {
			...
			Edge createdEdge =
				graphModel.factory().newEdge(gephiGraph.getNode(edge.getSource().toString()),
					gephiGraph.getNode(edge.getTarget().toString()));

			// always add an id for the edge (required for some formats)
			createdEdge.getAttributes().setValue(FIELD_EDGE_ID,
				edge.getSource().toString() + "_to_" + edge.getTarget().toString());
			...
			if ( edgeObj instanceof IAgent ) {
				...
				for ( Object key : a.getAttributes().keySet() ) {
					...
					processGamaAttribute(key.toString(), value, null, createdEdge);
				}
			} else {
				// store the object itself
			...
			if ( !gephiGraph.addEdge(createdEdge) ) {
				GAMA.reportError(
					GamaRuntimeException.error("an edge was ignored, probably because some edges are redondant"), true);
				continue;
			}
