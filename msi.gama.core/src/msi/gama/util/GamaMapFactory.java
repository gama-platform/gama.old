...
public class GamaMapFactory {
	...
	public static final GamaMap EMPTY_MAP = create(Types.NO_TYPE, Types.NO_TYPE, DEFAULT_SIZE);
	 ...
	 * @return
	 */
	public static <K, V> GamaMap<K, V> createWithoutCasting(final IType key, final IType contents, final Map<K, V> map) {
		GamaMap<K, V> result = create(key, contents, map.size());
		result.putAll(map);
		return result;
	}
