...
public class Use extends ShapeElement {
	 ...
	 * height = attrs.getValue("height"); String href = attrs.getValue("xlink:href");
	 *
	 * if (x != null) this.x = (float)XMLParseUtil.parseRatio(x); if (y != null) this.y =
	 * (float)XMLParseUtil.parseRatio(y); if (width != null) this.width = (float)XMLParseUtil.parseRatio(width); if
	 * (height != null) this.height = (float)XMLParseUtil.parseRatio(height);
	 *
	 *
	 * if (href != null) { try { URI src = getXMLBase().resolve(href); this.href = helper.universe.getElement(src); }
	 * catch (Exception e) { e.printStackTrace(); } }
	 *
	 * //Determine use offset/scale refXform = new AffineTransform(); refXform.translate(this.x, this.y);
	...
	public void render(final Graphics2D g) throws SVGException {
		...
		final RenderableElement rendEle = (RenderableElement) href;
	...
	public Rectangle2D getBoundingBox() throws SVGException {
		if (href instanceof ShapeElement) {
			final ShapeElement shapeEle = (ShapeElement) href;
	...
	// if (getPres(sty.setName("xlink:href"))) {
	// final URI src = sty.getURIValue(getXMLBase());
	// final SVGElement newVal = diagram.getUniverse().getElement(src);
	// if (newVal != href) {
	// href = newVal;
	// shapeChange = true;
	// }
	// }
	// /*
	// * if (getPres(sty.setName("xlink:href"))) { URI src = sty.getURIValue(getXMLBase()); href =
