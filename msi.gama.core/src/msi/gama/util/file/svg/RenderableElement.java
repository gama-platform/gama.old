...
abstract public class RenderableElement extends TransformableElement {
	...
	protected void beginLayer(final Graphics2D g) throws SVGException {
		...
		if (getStyle(styleAttrib.setName("clip-path"))) {
			...
			if (uri != null) {
				final ClipPath ele = (ClipPath) diagram.getUniverse().getElement(uri);
		...
		if (clipPath != null) {
			if (clipPathUnits == ClipPath.CP_OBJECT_BOUNDING_BOX && this instanceof ShapeElement) {
				final Rectangle2D rect = ((ShapeElement) this).getBoundingBox();
			...
			final Area newClip = new Area(cachedClip);
