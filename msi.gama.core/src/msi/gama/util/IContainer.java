...
public interface IContainer<KeyType, ValueType> extends IValue {
	...
	public static interface Addressable<KeyType, ValueType> {
			...
			usages = {
				@usage(value = "if it is a list or a matrix, at returns the element at the index specified by the right operand",
					examples = {
						...
						@example(value = "[{1,2}, {3,4}, {5,6}] at 0",
		...
		@operator(value = { "internal_at" },
			type = ITypeProvider.FIRST_CONTENT_TYPE,
			category = { IOperatorCategory.CONTAINER })
		@doc(value = "For internal use only. Corresponds to the implementation of the access to containers with [index]",
			see = { IKeyword.AT })
		public
	...
	@doc(value = "true, if the container contains the right operand, false otherwise",
		...
		usages = {
			@usage(value = "if it is a list or a matrix, contains returns true if the list or matrix contains the right operand",
				examples = { @example(value = "[1, 2, 3] contains 2", equals = "true"),
	...
	@doc(value = "the first value of the operand",
		...
		usages = {
			@usage(value = "if it is a list, first returns the first element of the list, or nil if the list is empty",
				examples = { @example(value = "first ([1, 2, 3])", returnType = IKeyword.INT, equals = "1") }),
			...
			@usage(value = "for a matrix of object or geometry, it will return nil if the matrix is empty") },
		see = { "last" })
	public
	...
	@doc(value = "the last element of the operand",
		...
		usages = {
			@usage(value = "if it is a list, last returns the last element of the list, or nil if the list is empty",
				examples = { @example(value = "last ([1, 2, 3])", returnType = IKeyword.INT, equals = "3") }),
	...
	@doc(value = "the number of elements contained in the operand",
		...
		usages = {
			@usage(value = "if it is a list or a map, length returns the number of elements in the list or map",
				examples = { @example(value = "length([12,13])", equals = "2"),
					@example(value = "length([])", equals = "0") }),
			...
			@usage(value = "if it is a matrix, length returns the number of cells",
				examples = { @example(value = "length(matrix([[\"c11\",\"c12\",\"c13\"],[\"c21\",\"c22\",\"c23\"]]))",
					equals = "6") }) })
	public
	...
	@doc(value = "true if the operand is empty, false otherwise.",
		...
		usages = {
			@usage(value = "if it is a list, empty returns true if there is no element in the list, and false otherwise",
				examples = { @example(value = "empty([])", equals = "true") }),
	...
	@doc(value = "the operand elements in the reversed order in a copy of the operand.",
		...
		usages = {
			@usage(value = "if it is a list, reverse returns a copy of the operand list with elements in the reversed order",
				examples = { @example(value = "reverse ([10,12,14])", equals = "[14, 12, 10]") }),
			@usage(value = "if it is a map, reverse returns a copy of the operand map with each pair in the reversed order (i.e. all keys become values and values become keys)",
				examples = { @example(value = "reverse (['k1'::44, 'k2'::32, 'k3'::12])",
			...
			@usage(value = "if it is a matrix, reverse returns a new matrix containing the transpose of the operand.",
				examples = { @example(value = "reverse(matrix([[\"c11\",\"c12\",\"c13\"],[\"c21\",\"c22\",\"c23\"]]))",
	...
	@doc(value = "one of the values stored in this container  at a random key",
		...
		usages = {
			@usage(value = "if the operand is empty, one_of returns nil", examples = { @example(value = "one_of([])",
				equals = "nil",
				returnType = IKeyword.UNKNOWN,
				isTestOnly = true) }),
			@usage(value = "if it is a list or a matrix, one_of returns one of the values of the list or of the matrix",
				examples = {
					@example(value = "any ([1,2,3])",
						var = "i",
						equals = "1, 2 or 3",
						returnType = IKeyword.INT,
						test = false),
					@example(value = "[1,2,3] contains i",
						returnType = IKeyword.BOOL,
						equals = "true",
						isTestOnly = true),
					@example("string sMat <- one_of(matrix([[\"c11\",\"c12\",\"c13\"],[\"c21\",\"c22\",\"c23\"]])); 	// sMat equals \"c11\",\"c12\",\"c13\", \"c21\",\"c22\" or \"c23\""),
					@example(value = "matrix([[\"c11\",\"c12\",\"c13\"],[\"c21\",\"c22\",\"c23\"]]) contains sMat",
						returnType = IKeyword.BOOL,
						equals = "true",
						isTestOnly = true) }),
			@usage(value = "if it is a map, one_of returns one the value of a random pair of the map",
				examples = {
					@example("int im <- one_of ([2::3, 4::5, 6::7]);	// im equals 3, 5 or 7"),
					@example(value = "[2::3, 4::5, 6::7].values contains im",
						returnType = IKeyword.BOOL,
						equals = "true") }),
			...
			@usage(value = "if it is a population, one_of returns one of the agents of the population",
				examples = {
					@example(value = "agent b <- one_of(agents);", isTestOnly = true),
					@example(value = "bug b <- one_of(bug);  	// Given a previously defined species bug, b is one of the created bugs, e.g. bug3",
						isExecutable = false) }) },
		see = { "contains" })
	public
