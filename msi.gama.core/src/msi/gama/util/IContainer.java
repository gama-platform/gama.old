...
public interface IContainer<KeyType, ValueType> extends IValue, Iterable<ValueType> {
	...
	@doc(value = "the element at the right operand index of the container", comment = "The first element of the container is located at the index 0. "
		...
		"if it is a graph, if the right operand is a node, at returns the in and out edges corresponding to that node; if it is an edge, at returns the pair node_out::node_in." }, examples = {
		"[1, 2, 3] at 2	--:    2 ", "[{1,2}, {3,4}, {5,6}] contains {3,4}     --:   true" }, see = { "contains_all, contains_any" })
	public ValueType get(KeyType index) throws GamaRuntimeException;

	@operator(value = "contains", can_be_const = true)
	@doc(value = "true, whether the container contains the right operand, false otherwise", comment = "the contains operator behavior depends on the nature of the operand", special_cases = {
	...
	@doc(value = "the maximum element found in the operand", comment = "the max operator behavior depends on the nature of the operand", special_cases = {
		...
		"if it is a matrix of geometry, max returns the maximum of the list of the geometries" }, see = { "min" }, examples = {
	...
	@doc(value = "the minimum element found in the operand.", comment = "the min operator behavior depends on the nature of the operand", special_cases = {
		...
		"if it is a matrix of geometry, min returns the minimum of the list of the geometries" }, see = { "max" }, examples = { "min ([100, 23.2, 34.5]) 	--: 	23.2" })
	...
	@doc(value = "the product of all the elements of the operand", comment = "the mul operator behavior depends on the nature of the operand", special_cases = {
		...
		"if it is a matrix of geometry, mul returns the product of the list of the geometries" }, see = { "sum" }, examples = { "mul ([100, 23.2, 34.5]) 	--:		80040.0" })
	...
	@doc(value = "the sum of all the elements of the operand", comment = "the sum operator behavior depends on the nature of the operand", special_cases = {
		...
		"if it is a matrix of geometry, sum returns the sum of the list of the geometries" }, see = { "mul" }, examples = {
		"sum ([12,10, 3]) 	--: 	25.0",
	...
	@doc(value = "the operand elements in the reversed order in a copy of the operand.", comment = "the reverse operator behavior depends on the nature of the operand", special_cases = {
		...
		"reverse ({10,13}) 					--: 	{13, 10}", "reverse ([10,12,14]) 				--: 	[14, 12, 10]",
	...
	@doc(value = "a random element from the container", comment = "the one_of operator behavior depends on the nature of the operand", special_cases = {
