...
public interface IContainer<KeyType, ValueType> extends IValue {
		...
		@doc(value = "the element at the right operand index of the container",
			...
			usages = {
				@usage(
					...
					examples = { @example(value = "[1, 2, 3] at 2", returnType = IKeyword.INT, equals = "3"),
	...
	@doc(value = "one of the values stored in this container  at a random key",
		...
		usages = {
			@usage(value = "if the operand is empty, one_of returns nil",
				examples = {
			...
			@usage(value = "if it is a population, one_of returns one of the agents of the population",
				examples = { @example(value = "agent b <- one_of(agents);", isTestOnly = true),
					@example(
