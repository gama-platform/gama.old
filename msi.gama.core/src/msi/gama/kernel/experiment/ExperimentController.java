...
public class ExperimentController implements Runnable, IExperimentController {
		...
		switch (command) {
			case IExperimentController._OPEN:
				...
				try {
					...
					// scope.getGui().debug("Error when opening the experiment: " + e.getMessage());
					closeExperiment(e);
				} finally {
			...
			case IExperimentController._RELOAD:
				...
				try {
					...
					if ( wasRunning ) {
						...
						GAMA.getGui().informStatus("Experiment reloaded");
					}
				} catch (final GamaRuntimeException e) {
					closeExperiment(e);
				} catch (final Exception e) {
	...
	// scope.getGui().neutralStatus("No simulation running");
