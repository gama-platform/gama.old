...
public abstract class AbstractFactory implements ISymbolFactory {
	...
	public void registerSymbol(final SymbolProto md, final List<String> names) {
		...
		if (handles(kind)) {
			...
			for (String s : names) {
				if (registeredSymbols.containsKey(s)) {
					return;
				}
	...
	@Override
	public void addNewKeywordToSymbolProto(final String s, int kind) {
		if (handles(kind)) {
			if (!ISymbolKind.Variable.KINDS.contains(kind)) {
				SymbolProto.nonVariableStatements.add(s);
			} // FIXME DONE SEVERAL TIMES. MAYBE UNNECESSARY
			if (registeredSymbols.containsKey(s)) {
				return;
			}
			SymbolProto p = findSymbolProtoOfKind(kind);
			if (p != null)
				registeredSymbols.put(s, p);
			// Exception si null ??
		} else {
			for (ISymbolFactory f : HIERARCHY.get(this)) {
				f.addNewKeywordToSymbolProto(s, kind);
			}
		}
	}

	private SymbolProto findSymbolProtoOfKind(int kind) {
		for (SymbolProto p : registeredSymbols.values())
			if (p.getKind() == kind)
				return p;
		return null;
	}
	...
	protected ISymbolFactory chooseFactoryFor(final String keyword,
			final String context) {
		ISymbolFactory contextFactory = context != null ? chooseFactoryFor(context)
				: this;
