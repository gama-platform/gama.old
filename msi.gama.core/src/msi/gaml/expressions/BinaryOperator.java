...
public class BinaryOperator extends AbstractNAryOperator {
	...
	public BinaryOperator(final IType ret, final IOpRun exec, final boolean canBeConst,
		final int tProv, final int ctProv, int iProv, final boolean lazy, int[] expectedContentType) {
		this.lazy = lazy;
	...
	private IType computeType(IDescription context, int t, IType def, int kind) {
		...
		return t == FIRST_TYPE ? left().getType() : t == SECOND_TYPE ? right().getType()
			: t == FIRST_CONTENT_TYPE ? left().getContentType() : t == FIRST_KEY_TYPE ? left()
				.getKeyType() : t == SECOND_CONTENT_TYPE ? right().getContentType()
				: t == SECOND_KEY_TYPE ? right().getKeyType() : t >= 0 ? Types.get(t) : def;
	...
	public void computeContentType(final IDescription context) {
		contentType =
			computeType(context, contentTypeProvider, type.defaultContentType(), _content);
	}
	...
	public BinaryOperator copy() {
		BinaryOperator copy =
			new BinaryOperator(type, helper, canBeConst, typeProvider, contentTypeProvider,
				keyTypeProvider, lazy, expectedContentType);
		copy.doc = doc;
		return copy;
	}
	...
	public static class BinaryVarOperator extends BinaryOperator implements IVarExpression {
		...
		public BinaryVarOperator(final IType ret, final IOpRun exec, final boolean canBeConst,
			final int type, final int contentType, final int keyType, final boolean lazy,
		...
		@Override
		public BinaryVarOperator init(final String operator, final IDescription context,
			final IExpression ... args) {
			if ( args.length < 2 ) {
				context.error("No attributes found after the '.'", IGamlIssue.UNKNOWN_VAR);
			} else if ( !(args[1] instanceof IVarExpression) ) {
				context.error("Attribute " + args[1].literalValue() + " unknown for " +
					args[0].getType() + " instances");
				return null;
			}
		...
		public BinaryVarOperator copy() {
			BinaryVarOperator copy =
				new BinaryVarOperator(type, helper, canBeConst, typeProvider, contentTypeProvider,
					keyTypeProvider, lazy, expectedContentType);
			copy.doc = doc;
			return copy;
		}
