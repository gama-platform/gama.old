...
@type(name = IKeyword.MATRIX, id = IType.MATRIX, wraps = { IMatrix.class, GamaIntMatrix.class, GamaFloatMatrix.class,
		GamaObjectMatrix.class }, kind = ISymbolKind.Variable.CONTAINER, concept = { IConcept.TYPE, IConcept.CONTAINER,
				IConcept.MATRIX })
...
public class GamaMatrixType extends GamaContainerType<IMatrix> {
	...
	public static IMatrix staticCast(final IScope scope, final Object obj, final Object param, final IType contentType,
		...
		if (size == null) {
			if (obj instanceof IContainer) {
				return ((IContainer) obj).matrixValue(scope, contentType, copy);
			}
		...
		if (obj instanceof IContainer) {
			return ((IContainer) obj).matrixValue(scope, contentType, size, copy);
		}
	...
	public static IMatrix from(final IScope scope, final IList list, final IType desiredType,
		...
		if (list == null || list.isEmpty()) {
			return new GamaObjectMatrix(0, 0, desiredType);
		}
	...
	public static IMatrix with(final IScope scope, final IExpression fillExpr, final int cols, final int rows) {
		IMatrix result;
		if (fillExpr == null) {
			return new GamaObjectMatrix(cols, rows, Types.NO_TYPE);
		}
	...
	public IType contentsTypeIfCasting(final IExpression exp) {
		...
		if (Types.CONTAINER.isAssignableFrom(itemType)) {
