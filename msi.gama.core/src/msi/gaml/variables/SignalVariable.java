...
@Deprecated
@facets(value = {
		@facet(name = IKeyword.NAME, type = IType.NEW_VAR_ID, optional = false, doc = @doc("Represents the name of the variable that will be introduced to represent this signal on the specified grid") ),
		// @facet(name = IKeyword.VAR, optional = true, type = IType.NEW_VAR_ID,
		// doc = @doc("The name of the variable that will be introduced to
		...
		@facet(name = IKeyword.UPDATE, type = IType.NONE, optional = true, doc = @doc("An expression that will be evaluated each cycle to update the value of the signal on each grid cell") ),
		@facet(name = IKeyword.ENVIRONMENT, type = IType.SPECIES, optional = true, doc = @doc("The name of the grid species on which this signal will be propagated") ),
		@facet(name = IKeyword.ON, type = { IType.SPECIES,
				IType.CONTAINER }, optional = true, doc = @doc("Either the name of the grid species on which this signal will be propagated (equivalent to 'environment:'), or an expression that returns a subset of the cells of this grid species") ),
		@facet(name = IKeyword.DECAY, type = IType.FLOAT, optional = false, doc = @doc("represents the amount to remove to the intensity of a signal, once dropped on a place, at each time step. It is a percentage between 0 and 1. If 'decay' is not defined, the signal will not be wiped from the places; otherwise, its intensity will be equal to (intensity `*` decay).") ),
...
public class SignalVariable extends NumberVariable {
	...
	public static class SignalValidator implements IDescriptionValidator {
		...
		public void validate(final IDescription d) {
			...
			final VariableDescription vd = (VariableDescription) DescriptionFactory.create(
					SyntacticFactory.create(IKeyword.FLOAT, new Facets(NAME, attributeName, MIN, "0.0"), false), s,
					null);
			s.addChild(vd);
			IExpressionFactory f = GAML.getExpressionFactory();
			IExpression v = s.getVarExpr(d.getName());
			IExpression value = f.createOperator("?", s, null,
	...
	public SignalVariable(final IDescription sd) throws GamaRuntimeException {
		...
		// variation = variationExpr == null ? typeExpr == null ? null : 0d :
		// null;
		// range = rangeExpr == null ? -1000.0 : rangeExpr.isConst() ?
		// Cast.as(rangeExpr, Double.class, false) : null;
	}
	...
	protected void _setVal(final IAgent agent, final IScope scope, final Object v) throws GamaRuntimeException {
		...
		if (result > 0.0 && !agent.dead()) {
			...
			// ? IKeyword.GRADIENT.equals(scope.evaluate(typeExpr, agent)) ?
			...
			final double variation = variationExpr == null ? 0d
					: Cast.asFloat(scope, scope.evaluate(variationExpr, agent));
			// this.variation == null ? Cast.asFloat(scope,
			// scope.evaluate(variationExpr, agent)) : this.variation;
			final double range = rangeExpr == null ? -1000.0
					: Math.max(0.0, Cast.asFloat(scope, scope.evaluate(rangeExpr, agent)));
