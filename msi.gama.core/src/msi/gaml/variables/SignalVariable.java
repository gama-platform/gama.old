...
@facets(value = { @facet(name = IKeyword.NAME,
	...
	@facet(name = IKeyword.PROPAGATION,
		...
		optional = true,
		doc = @doc("represents both the way the signal is propagated and the way to treat multiple propagation of the same signal occuring at once from different places. If propagation equals 'diffusion', the intensity of a signal is shared between its neighbors with respect to 'proportion', 'variation' and the number of neighbors of the environment places (4, 6 or 8). I.e., for a given signal S propagated from place P, the value transmitted to its N neighbors is : S' = (S / N / proportion) - variation. The intensity of S is then diminished by S `*` proportion on P. In a diffusion, the different signals of the same name see their intensities added to each other on each place. If propagation equals 'gradient', the original intensity is not modified, and each neighbors receives the intensity : S / proportion - variation. If multiple propagations occur at once, only the maximum intensity is kept on each place. If 'propagation' is not defined, it is assumed that it is equal to 'diffusion'.")),
	@facet(name = IKeyword.PROPORTION,
		type = IType.FLOAT,
		optional = true,
		doc = @doc("a value between 0 and 1 that represents the percentage of the intensity which will be shared between the neighbors in the diffusion. For instance, for an intensity of 80, and  a proportion of 0.5, in a 4-neighbors environment, each of the neighboring places will receive an intensity of (80 `*` 0.5) / 4 = 10. If no 'proportion' is defined, it is assumed that the propagation corresponds to a diffusion where 100% of the intensity is equally divided between the neighbors. I.e., for an intensity of 100, and 4 neighbors per place, each of them receives a signal with an intensity of 25.")),
...
public class SignalVariable extends NumberVariable {
	...
	public static class SignalValidator implements IDescriptionValidator {
		...
		public void validate(final IDescription d) {
			...
			IExpression v = s.getVarExpr(d.getName(), false);
			IExpression value =
