...
@facets(value = {
//	@facet(name = IKeyword.SOURCE, type = { IType.AGENT_STR, IType.CONTAINER_STR, IType.SPECIES_STR }, optional = false),
...
//	private IExpression source;
	...
	public MigrateCommand(IDescription desc) {
		super(desc);
		
		verifyFacetType(IKeyword.SOURCE);
//		source = getFacet(IKeyword.SOURCE);
	...
	private void verifyTarget() {
		SpeciesDescription species = (SpeciesDescription) this.getDescription().getSpeciesContext();
	...
	public Object privateExecuteIn(final IScope stack) throws GamaRuntimeException {
		...
		if (immigrantCandidates instanceof ISpecies) {
			immigrants.addAll(executor.migrateMicroAgent((ISpecies) immigrantCandidates, targetMicroSpecies));
		} else if (immigrantCandidates instanceof IList) {
			immigrants.addAll(executor.migrateMicroAgents((IList) immigrantCandidates, targetMicroSpecies));
