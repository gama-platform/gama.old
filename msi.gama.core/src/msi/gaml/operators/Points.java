...
public class Points {
	...
	// "special" operator introduced in the parser for the points
		public static
		ILocation toPoint(final Double a, final Double b) {
	...
	// "special" operator introduced in the parser for the points
		public static
		ILocation toPoint(final Integer a, final Double b) {
		return new GamaPoint(a, b);
	}
	...
	// "special" operator introduced in the parser for the points
		public static
		ILocation toPoint(final Double a, final Integer b) {
		return new GamaPoint(a, b);
	}
	...
	// "special" operator introduced in the parser for the points
		public static
		ILocation toPoint(final Integer a, final Integer b) {
		return new GamaPoint(a, b);
	}
	...
	public static
		ILocation divide(final GamaPoint p, final Double d) {
	...
	@operator(value = IKeyword.MULTIPLY, can_be_const = true, category = IOperatorCategory.POINT)
	@doc(value = "Returns a point with coordinates multiplied by a number.",
		usages = @usage(value = "if the left-hand operator is a point and the right-hand a number, returns a point with coordinates multiplied by the number",
			examples = { @example(value = "{2,5} * 4", equals = "{8.0, 20.0}"),
				@example(value = "{2, 4} * 2.5", equals = "{5.0, 10.0}") }))
	public static
		ILocation multiply(final GamaPoint p1, final Integer d) {
	...
	@operator(value = IKeyword.PLUS, can_be_const = true, category = IOperatorCategory.POINT)
	@doc(value = "Returns a point with coordinate summing of the two operands.",
		usages = @usage(value = "if the left-hand operand is a point and the right-hand a number, returns a new point with each coordinate as the sum of the operand coordinate with this number.",
			examples = { @example(value = "{1, 2} + 4", equals = "{5.0, 6.0,4.0}"),
				@example(value = "{1, 2} + 4.5", equals = "{5.5, 6.5,4.5}") }))
	public static
		ILocation add(final GamaPoint p1, final Double p) {
	...
	public static
		ILocation substract(final GamaPoint p1, final Double p) {
		return new GamaPoint(p1.x - p, p1.y - p, p1.z - p);
	}
	...
	@doc(value = "Rounds off the ordinates of the left-hand point to the precision given by the value of right-hand operand",
		examples = { @example(value = "{12345.78943, 12345.78943, 12345.78943} with_precision 2 ",
			equals = "{12345.79, 12345.79, 12345.79}") }, see = "round")
	public static
		ILocation round(final ILocation v, final Integer precision) {
