...
public class DeprecatedOperators {
	...
	@doc(value = "A link between the 2 elements of the pair.", deprecated = "Use link(g1, g2) instead", usages = {
			@usage("if the operand is nil, link returns a point {0,0}"),
			@usage("if one of the elements of the pair is a list of geometries or a species, link will consider the union of the geometries or of the geometry of each agent of the species") }, comment = "The geometry of the link is a line between the locations of the two elements of the pair, which is built and maintained dynamically ", examples = {
	...
	public static IShape link(final IScope scope, final GamaPair points) throws GamaRuntimeException {
		if (points == null || points.first() == null && points.last() == null) {
	...
	@doc(deprecated = "use neighbors_of instead", value = "a list, containing all the agents of the same species than the argument (if it is an agent) located at a distance inferior or equal to 1 to the right-hand operand agent considering the left-hand operand topology.", masterDoc = true, examples = {
			@example(value = "topology(self) neighbours_of self", equals = "returns all the agents located at a distance lower or equal to 1 to the agent applying the operator considering its topology.", test = false) }, see = {
					"neighbors_of" })
	...
	@doc(deprecated = "Use 'neighbors_of(topology, agent, distance)' instead", usages = @usage(value = "a list, containing all the agents of the same species than the key of the pair argument (if it is an agent) located at a distance inferior or equal to the right member (float) of the pair (right-hand operand) to the left member (agent, geometry or point) considering the left-hand operand topology.", examples = {
			@example(value = "topology(self) neighbours_of self::10", equals = "all the agents located at a distance lower or equal to 10 to the agent applying the operator considering its topology.", test = false) }))
	...
	@operator(value = "neighbours_of", content_type = IType.AGENT, category = { IOperatorCategory.SPATIAL,
			IOperatorCategory.SP_QUERIES })
	/* TODO, expected_content_type = { IType.FLOAT, IType.INT } */
	@doc(deprecated = "use neighbors_of instead", usages = @usage(value = "a list, containing all the agents of the same species than the left argument (if it is an agent) located at a distance inferior or equal to the third argument to the second argument (agent, geometry or point) considering the first operand topology.", examples = {
			@example(value = "neighbours_of (topology(self), self,10)", equals = "all the agents located at a distance lower or equal to 10 to the agent applying the operator considering its topology.", test = false) }))
	...
	@operator(value = "neighbours_at", content_type = ITypeProvider.FIRST_TYPE, category = { IOperatorCategory.SPATIAL,
			IOperatorCategory.SP_QUERIES })
	@doc(deprecated = "use neighbors_at instead", value = "a list, containing all the agents of the same species than the left argument (if it is an agent) located at a distance inferior or equal to the right-hand operand to the left-hand operand (geometry, agent, point).", comment = "The topology used to compute the neighborhood  is the one of the left-operand if this one is an agent; otherwise the one of the agent applying the operator.", examples = {
			@example(value = "(self neighbours_at (10))", equals = "all the agents located at a distance lower or equal to 10 to the agent applying the operator.", test = false) }, see = {
					"neighbors_at" })
	...
	@doc(deprecated = "Use 'char' instead", special_cases = { "convert ACSII integer value to character" }, examples = {
			@example(value = "toChar (34)", equals = "'\"'") })
	...
	@doc(deprecated = "use image_file instead", value = "opens a file that is a kind of image.", comment = "The file should have an image extension, cf. file type definition for supported file extensions.", special_cases = "If the specified string does not refer to an existing image file, an exception is risen.", examples = {
			@example("let fileT type: file value: image(\"../includes/testImage.png\");  // fileT represents the file \"../includes/testShape.png\"") }, see = {
	...
	@doc(deprecated = "use the operator \"writable\" instead", value = "marks the file so that only read operations are allowed.", comment = "A file is created by default in read-only mode. The operator write can change the mode.", examples = {
			@example("read(shapefile(\"../images/point_eau.shp\"))  --:  returns a file in read-only mode representing \"../images/point_eau.shp\"") }, see = {
					"file", "writable" })
	@Deprecated
	...
	@doc(deprecated = "use the operator \"writable\" instead", value = "marks the file so that read and write operations are allowed.", comment = "A file is created by default in read-only mode.", examples = {
			@example("write(shapefile(\"../images/point_eau.shp\"))   --: returns a file in read-write mode representing \"../images/point_eau.shp\"") }, see = {
					"file", "writable" })
	@Deprecated
	...
	@doc(deprecated = "use text_file instead", value = "opens a file that a is a kind of text.", comment = "The file should have a text extension, cf. file type definition for supported file extensions.", special_cases = "If the specified string does not refer to an existing text file, an exception is risen.", examples = {
			...
			@example("				// fileT represents the text file \"../includes/Stupid_Cell.Data\"") }, see = {
					"file", "properties", "image", "shapefile" })
	@Deprecated
	...
	@doc(deprecated = "use property_file instead", value = "opens a file that is a kind of properties.", comment = "The file should have a properties extension, cf. type file definition for supported file extensions.", special_cases = "If the specified string does not refer to an existing propserites file, an exception is risen.", examples = {
			@example("let fileT type: file value: properties(\"../includes/testProperties.properties\");  // fileT represents the properties file \"../includes/testProperties.properties\"") }, see = {
	...
	@doc(deprecated = "use set location instead", value = "add_z", comment = "Return a geometry with a z value"
			...
			+ "For each point of the cell the same z value is set.", examples = {
					@example("set shape <- shape add_z rnd(100);") }, see = { "add_z_pt" })
	@Deprecated
	...
	@doc(deprecated = "Please use 'interleave' instead.", value = "a new list containing the interleaved elements of the containers contained in the operand", comment = "the operand should be a list of lists of elements. The result is a list of elements. ", examples = {
	...
	@doc(deprecated = "Use 'g2 covers g1' instead of 'g1 covered_by g2'", value = "A boolean, equal to true if the left-geometry (or agent/point) is covered by the right-geometry (or agent/point).", usages = {
			@usage("if one of the operand is null, returns false.") }, examples = {
					@example(value = "square(5) covered_by square(2)", equals = "false") }, see = { "disjoint_from",
	...
	@doc(deprecated = "Use the standard construction {x,y,z} instead. ")
	...
	@doc(deprecated = " Use binomial(int, float) instead", value = "A value from a random variable following a binomial distribution. The operand {n,p} represents the number of experiments n and the success probability p.", comment = "The binomial distribution is the discrete probability distribution of the number of successes in a sequence of n independent yes/no experiments, each of which yields success with probability p, cf. Binomial distribution on Wikipedia.", examples = {
	...
	@doc(deprecated = "Use rnd instead with a float argument", examples = {
			@example(value = "rnd_float(3)", equals = "a random float between 0.0 and 3.0", test = false) }, see = {
					"rnd" })
	...
	@doc(deprecated = "Use 'set_z' instead", value = "add_z_pt", comment = "Return a geometry with a z value", examples = {
			@example("loop i from: 0 to: length(shape.points) - 1{" + "shape <- shape add_z_pt {i,valZ};"
					+ "}") }, see = { "add_z" })
	...
	@operator(value = { "neighbours_of" }, type = IType.LIST, content_type = ITypeProvider.FIRST_KEY_TYPE, category = {
			IOperatorCategory.GRAPH })
	@doc(deprecated = "use neighbors_of instead", value = "returns the list of neighbours of the given vertex (right-hand operand) in the given graph (left-hand operand)", examples = {
	...
	@doc(value = "The shortest path between a list of two objects in a graph", examples = {
			@example(value = "my_graph path_between (ag1:: ag2)", equals = "A path between ag1 and ag2", isExecutable = false) }, deprecated = "use 'path_between(graph, geometry, geometry)' instead")
