...
public class DeprecatedOperators {
	...
	usages = @usage(
		value = "a list, containing all the agents of the same species than the key of the pair argument (if it is an agent) located at a distance inferior or equal to the right member (float) of the pair (right-hand operand) to the left member (agent, geometry or point) considering the left-hand operand topology.",
	...
	// @operator(value = "gridfile", can_be_const = true, index_type = IType.INT)
	// @doc(deprecated = "use grid_file instead",
	// value = "opens a file that a is a kind of gridfile. The integer parameter allows to specify a coordinate reference system (CRS). If equal to zero, it forces reading the data as alreay
	// projected",
	// comment = "The file should have a gridfile extension, cf. file type definition for supported file extensions.",
	// special_cases = "If the specified string does not refer to an existing gridfile file, an exception is risen.",
	// examples = { @example("file fileT <- gridfile(\"../includes/testProperties.asc\");"),
	...
	examples = {
		@example("read(shapefile(\"../images/point_eau.shp\"))  --:  returns a file in read-only mode representing \"../images/point_eau.shp\"") },
	see = { "file", "writable" })
	@Deprecated
	...
	examples = {
		@example("write(shapefile(\"../images/point_eau.shp\"))   --: returns a file in read-write mode representing \"../images/point_eau.shp\"") },
	see = { "file", "writable" })
	@Deprecated
	...
	examples = {
		@example("let fileT type: file value: properties(\"../includes/testProperties.properties\");  // fileT represents the properties file \"../includes/testProperties.properties\"") },
	...
	@doc(deprecated = "Please use 'interleave' instead.",
	...
	@doc(deprecated = "Use rnd instead with a float argument",
	examples = { @example(value = "rnd_float(3)", equals = "a random float between 0.0 and 3.0", test = false) },
	see = { "rnd" })
	...
	examples = { @example(value = "file f <- file('Correlation.r');", isTestOnly = true),
		...
		@example(value = "R_compute('Correlation.R')",
		var = "result",
