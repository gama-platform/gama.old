...
public class Logic {
	...
	@doc(value = "opposite boolean value.", special_cases = { "if the parameter is not boolean, it is casted to a boolean value." }, examples = { @example(value="! (true)", equals="false") }, see = "bool")
	...
	@doc(value = "if the left-hand operand evaluates to true, returns the value of the left-hand operand of the :, otherwise that of the right-hand operand of the :", comment = "These functional tests can be combined together.", examples = {
		@example(value="[10, 19, 43, 12, 7, 22] collect ((each > 20) ? 'above' : 'below')", equals="['below', 'below', 'above', 'below', 'below', 'above']"),
		@example("rgb color value:(food > 5) ? 'red' : ((food >= 2)? 'blue' : 'green');") }, see = ":")
