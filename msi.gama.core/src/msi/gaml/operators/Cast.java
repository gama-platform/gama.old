...
public class Cast {
	...
	// @operator(value = { "containing" }, type = ITypeProvider.FIRST_TYPE,
	// content_type = ITypeProvider.SECOND_TYPE)
	// @doc(value = "Internal use only. Allows to cast the contents type of
	// containers")
	// public static Object containing(final IScope scope, final Object a, final
	// IExpression b) {
	// return a;
	// }

	@operator(value = { IKeyword.IS }, category = { IOperatorCategory.CASTING }, concept = { IConcept.CAST,
			IConcept.TYPE })
	...
	@operator(value = IKeyword.IS_SKILL, category = { IOperatorCategory.CASTING }, concept = { IConcept.CAST,
			IConcept.SKILL })
	@doc(value = "returns true if the left operand is an agent whose species implements the right-hand skill name", examples = {
			@example(value = "agentA is_skill 'moving'", equals = "true", isExecutable = false) })
	...
	@operator(value = IKeyword.TOPOLOGY, content_type = IType.GEOMETRY, category = {
			IOperatorCategory.CASTING }, concept = { IConcept.CAST, IConcept.TOPOLOGY })
	@doc(value = "casting of the operand to a topology.", usages = {
			...
			@usage("otherwise, casts the operand to a geometry and build a topology from it.") }, examples = {
					...
					@example(value = "topology(a_graph)	--: Multiple topology in POLYGON ((24.712119771887785 7.867357373616512, 24.712119771887785 61.283226839310565, 82.4013676510046  7.867357373616512)) "
							+ "at location[53.556743711446195;34.57529210646354]", isExecutable = false) }, see = {
									"geometry" })
	...
	@operator(value = IKeyword.AS, type = ITypeProvider.SECOND_TYPE, content_type = ITypeProvider.SECOND_CONTENT_TYPE, index_type = ITypeProvider.SECOND_KEY_TYPE, can_be_const = true, category = {
			IOperatorCategory.CASTING }, concept = { IConcept.CAST })
	@doc(value = "casting of the first argument into a given type", comment = "It is equivalent to the application of the type operator on the left operand.", examples = @example(value = "3.5 as int", returnType = "int", equals = "int(3.5)"))
	...
	// defined in the java.awt.Color class, "
	...
	@operator(value = "as_int", can_be_const = true, category = { IOperatorCategory.CASTING }, concept = {
			IConcept.CAST })
	@doc(value = "parses the string argument as a signed integer in the radix specified by the second argument.", usages = {
			...
			@usage("if the left operand does not represent an integer in the specified radix, as_int throws an exception ") }, examples = {
					@example(value = "'20' as_int 10", equals = "20"), @example(value = "'20' as_int 8", equals = "16"),
					@example(value = "'20' as_int 16", equals = "32"),
					@example(value = "'1F' as_int 16", equals = "31"),
					@example(value = "'hello' as_int 32", equals = "18306744") }, see = { "int" })
	...
	@operator(value = "list_with", content_type = ITypeProvider.SECOND_TYPE, can_be_const = false, concept = {
			IConcept.CAST, IConcept.CONTAINER })
	@doc(value = "creates a list with a size provided by the first operand, and filled with the second operand", comment = "Note that the right operand  should be positive, and that the second one is evaluated for each position  in the list.", see = {
			"list" })
	...
	@operator(value = "matrix_with", content_type = ITypeProvider.SECOND_TYPE, can_be_const = true, category = {
			IOperatorCategory.CASTING }, concept = { IConcept.CAST, IConcept.CONTAINER })
	@doc(value = "creates a matrix with a size provided by the first operand, and filled with the second operand", comment = "Note that both components of the right operand point should be positive, otherwise an exception is raised.", see = {
			IKeyword.MATRIX, "as_matrix" })
	public static IMatrix matrix_with(final IScope scope, final ILocation size, final IExpression init) {
		if (size == null) {
			throw GamaRuntimeException.error("A nil size is not allowed for matrices", scope);
		}
	...
	@operator(value = "as_matrix", content_type = ITypeProvider.FIRST_CONTENT_TYPE_OR_TYPE, can_be_const = true, // AD:
																													// was
																													// previously
																													// true
																													// --
																													...
																													// Issue
																													// 1127
			category = { IOperatorCategory.CASTING }, concept = { IConcept.CAST, IConcept.CONTAINER })
	@doc(value = "casts the left operand into a matrix with right operand as preferred size", comment = "This operator is very useful to cast a file containing raster data into a matrix."
			...
			+ "If the size is to short, some elements will be omitted. Matrix remaining elements will be filled in by nil.", usages = {
					@usage("if the right operand is nil, as_matrix is equivalent to the matrix operator") }, see = {
							IKeyword.MATRIX })
	...
	@operator(value = { IKeyword.SPECIES, "species_of" }, content_type = ITypeProvider.FIRST_TYPE, category = {
			IOperatorCategory.CASTING }, concept = { IConcept.CAST, IConcept.SPECIES })
	...
	@doc(value = "returns the literal description of an expression or description -- action, behavior, species, aspect, even model -- in gaml", examples = {
			@example(value = "to_gaml(0)", equals = "'0'"), @example(value = "to_gaml(3.78)", equals = "'3.78'"),
			...
			@example(value = "to_gaml(node1)", equals = " 1 as node", isExecutable = false) }, see = {})
