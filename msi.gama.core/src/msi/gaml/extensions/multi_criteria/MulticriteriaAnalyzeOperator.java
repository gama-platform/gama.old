/*********************************************************************************************
 *
 * 'MulticriteriaAnalyzeOperator.java, in plugin msi.gama.core, is part of the source code of the
...
public class MulticriteriaAnalyzeOperator {
	...
	@operator(value = "weighted_means_DM", category = { MULTICRITERIA }, concept = { IConcept.MULTI_CRITERIA })
	@doc(value = "The index of the candidate that maximizes the weighted mean of its criterion values. The first operand is the list of candidates (a candidate is a list of criterion values); the second operand the list of criterion (list of map)", special_cases = {
			"returns -1 is the list of candidates is nil or empty" }, examples = {
					@example(value = "weighted_means_DM([[1.0, 7.0],[4.0,2.0],[3.0, 3.0]], [[\"name\"::\"utility\", \"weight\" :: 2.0],[\"name\"::\"price\", \"weight\" :: 1.0]])", equals = "1") }, see = {
							"promethee_DM", "electre_DM", "evidence_theory_DM" })
	public static Integer WeightedMeansDecisionMaking(final IScope scope, final List<List> cands,
			final List<Map<String, Object>> criteriaMap) throws GamaRuntimeException {
	...
	@operator(value = "evidence_theory_DM", category = { MULTICRITERIA }, concept = { IConcept.MULTI_CRITERIA })
	@doc(usages = {
	...
	public static Integer evidenceTheoryDecisionMaking(final IScope scope, final List<List> cands,
		...
		for (final Map<String, Object> critMap : criteriaMap) {
			...
			final CritereFonctionsCroyances cfc = new CritereFctCroyancesBasique(name, s1, v2Pour, v1Pour, v1Contre,
					v2Contre, s2);
			criteresFC.add(cfc);
		}
