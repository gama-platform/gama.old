...
@facets(value = {
	...
	@facet(name = IKeyword.EDGE,
		type = IType.NONE,
		optional = true,
		doc = @doc("Indicates that the item to remove should be considered as an edge of the receiving graph. Soon to be deprecated, use 'remove edge(item)...' instead")),
	...
	@facet(name = IKeyword.NODE,
		type = IType.NONE,
		optional = true,
		doc = @doc("Indicates that the item to remove should be considered as a node of the receiving graph. Soon to be deprecated, use 'remove node(item)...' instead")),
	@facet(name = IKeyword.KEY,
		type = IType.NONE,
		optional = true,
		doc = @doc("any expression, the key at which to remove the element from the container ")),
...
@doc(value = "Allows the agent to remove an element from a container (a list, matrix, map...).",
	usages = {
		@usage(value = "This statement should be used in the following ways, depending on the kind of container used and the expected action on it:",
			examples = { @example(value = "remove expr from: expr_container;", isExecutable = false),
		...
		@usage(value = "In the case of list, the facet `item:` is used to remove the first occurence of a given expression, whereas `all` is used to remove all the occurences of the given expression.",
			examples = {
				...
				@example(value = "remove index: 1 from: removeList;",
					...
					equals = "[1]",
					returnType = "null") }),
		@usage(value = "In the case of map, the facet `key:` is used to remove the pair identified by the given key.",
			examples = {
				@example("map<string,int> removeMap <- [\"x\"::5, \"y\"::7, \"z\"::7];"),
				@example(value = "remove key: \"x\" from: removeMap;",
					var = "removeMap",
					equals = "[\"y\"::7, \"z\"::7]",
					returnType = "null"),
				@example(value = "remove 7 all: true from: removeMap;",
					var = "removeMap",
					equals = "map([])",
					returnType = "null") }),
		@usage(value = "In addition, a map a be managed as a list with pair key as index. Given that, facets item:, all: and index: can be used in the same way:",
			examples = {
				@example("map<string,int> removeMapList <- [\"x\"::5, \"y\"::7, \"z\"::7, \"t\"::5];"),
				@example(value = "remove 7 from: removeMapList;",
					var = "removeMapList",
					equals = "[\"x\"::5, \"z\"::7, \"t\"::5]",
					returnType = "null"),
				@example(value = "remove [5,7] all: true from: removeMapList;",
					var = "removeMapList",
					equals = "[\"t\"::5]",
					returnType = "null"),
				@example(value = "remove index: \"t\" from: removeMapList;",
					var = "removeMapList",
					equals = "map([])",
					returnType = "null") }),
		@usage(value = "In the case of a graph, both edges and nodes can be removes using node: and edge facets. If a node is removed, all edges to and from this node are also removed.",
