...
public class FacetsArray {
	...
	public FacetsArray(final String... strings) {
		...
		for (; index < strings.length; index += 2) {
			final Facet f = new Facet(strings[index], StringBasedExpressionDescription.create(strings[index + 1]));
			facets[i++] = f;
		}
	}
	...
	public void putAll(final FacetsArray newFacets) {
		if (facets.length == 0) {
			...
			for (final Facet f : newFacets.entrySet()) {
				if (f != null) {
					put(f.key, f.value);
				}
			}
	...
	public IExpressionDescription remove(final String key) {
		...
		if (i == -1) { return null; }
		final IExpressionDescription expr = facets[i].value;
		facets[i] = null;
		return expr;
	}
	...
	private IExpressionDescription add(final String key, final IExpressionDescription expr) {
		...
		final Facet[] ff = new Facet[facets.length + 1];
	...
	public IExpressionDescription put(final String key, final IExpressionDescription expr) {
		final int i = indexOf(key);
		if (i > -1) {
			final IExpressionDescription previous = facets[i].value;
			facets[i].value = expr;
	...
	public FacetsArray cleanCopy() {
		final FacetsArray result = new FacetsArray();
