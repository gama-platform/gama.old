...
public class CameraArcBall implements MouseListener, MouseMotionListener, MouseWheelListener {
    ...
    private Point mousePosition;        //keep your cunrent mouse position
    private Vector cameraPosition;      //determine where camare should be located
    private double mouseSensitivity = 1;//I think that this is clear :)
...
///////////////////////////////////////////////////////////////////////////////////////////
    public CameraArcBall(GLCanvas canvas) {//we will send to constructor our canvas reference
...
//auxiliary method
    private int e(int row, int col) {
        return row * 4 + col;
    }

    private float[] mutliplyMatrixAByMatrixB(float ma[], float mb[]) {
        float m[] = new float[16];
     ...
     */
    private Vector updateRadius(int px, int py, GL gl) {
...
//mp[0],mp[1] contains x,y components , mp[2]-contains value from 0-1 and mean what is the
        ...
        glu.gluUnProject(px, viewport[3] - py, mp[2],
                modelViewM, 0, projectionM, 0, viewport, 0,
                coordinates, 0);
