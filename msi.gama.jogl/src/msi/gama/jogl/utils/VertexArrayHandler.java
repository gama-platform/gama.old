...
public class VertexArrayHandler {
	...
	private ArrayList<Vertex[]> vertexArrayContours = new  ArrayList<Vertex[]>();
	private ArrayList<Color> colorBorder = new ArrayList<Color>();
		...
		while (it.hasNext()) {
			...
			for ( int i = 0; i < curGeometry.geometry.getNumGeometries(); i++ ) {
				...
				if ( curGeometry.geometry.getGeometryType() == "MultiPolygon" ) {
					...
					if ( curGeometry.type.equals("sphere") ) {
						if(!createIcosphere)
						{	
							makeIcosphere(2);
	 ...
	 * 
	 * */
	public void buildVertexArrayContours(double z_layer)
	{
...
//			// Build the 4 vertices of the face.
	 ...
	 */
	public void buildMultiPolygonVertexArray(final MultiPolygon polygons, final double z_layer, final Color c, final double alpha,
	...
	private void fillPlanBuffers()
	{
		...
		for(int i = 0; i<planContours.size();i++)
		{
			...
			planBorderColorBuffer.put((float) planBorderColor.getRed());
			planBorderColorBuffer.put((float) planBorderColor.getGreen());
			planBorderColorBuffer.put((float) planBorderColor.getBlue());
			...
			final double alpha) 
	{
		...
		for(int i =0; i<icosphereVertexBuffer.capacity();i=i+3)
		{
	            ...
	            this.sphereNormals.add((float) (icosphereVertexBuffer.get(i)));
	            this.sphereNormals.add((float) (icosphereVertexBuffer.get(i+1)));
		...
		{
	            this.sphereIndices.add((int) (icosphereIndexBuffer.get(i)+this.size));
		}
	...
	{	
		...
		{
			spheresVertexBuffer.put((float) sphereTriangles.get(i));
			spheresNormalBuffer.put((float) sphereNormals.get(i));
		}
   ...
   private void makeIcosphere(int level) {
      ...
      for (int i : faceIndx)  // Copy the face data into the face index buffer.
