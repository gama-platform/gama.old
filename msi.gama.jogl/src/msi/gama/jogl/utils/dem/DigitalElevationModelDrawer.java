...
public class DigitalElevationModelDrawer {
	...
	static int terrain;
	public DigitalElevationModelDrawer(){
	...
	public static void InitDEM(GL gl,final String TEXTURE_FILEPATH, final String DEM_FILEPATH){
        ...
        DigitalElevationModelDrawer.loadTexture(TEXTURE_FILEPATH);
	}
	
	public static void DisplayDEM(GL gl){
		gl.glCallList(terrain);
	}
	private static void CreateDEM(GL gl, String demFileName, float w, float h){
        ...
        for(y = 0; y < rows; y++)
        {
            ...
            for(x = 0; x <= cols; x++)
            {
                ...
                float alt1 = (float)(dem.getRGB(cols-x, y) & 255) * 0.025f;
                float alt2 = (float)(dem.getRGB(cols-x, y+1) & 255) * 0.025f;
     ...
     */
    private static BufferedImage readPNGImage(String resourceName) throws RuntimeException
    {
        try
        {
