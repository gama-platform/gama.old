...
public class EDPSkill extends Skill {
	...
	public GamaList<Double> kr4SI(final IScope scope) throws GamaRuntimeException {
		...
		return rungeKuta4(scope, SIinit, new SI(beta, nu, N), h);
	}
	...
	public GamaList<Double> kr4SIR(final IScope scope) throws GamaRuntimeException {
		...
		return rungeKuta4(scope, SIRinit, new SIRS(alpha, beta, 0.0, 0.0, 0.0, 0.0, N), h);
	}
	...
	public GamaList<Double> kr4SIRS(final IScope scope) throws GamaRuntimeException {
		...
		return rungeKuta4(scope, SIRSinit, new SIRS(alpha, beta, gamma, b, d1, d2, N), h);
	}
	...
	public GamaList<Double> kr4SEIR(final IScope scope) throws GamaRuntimeException {
		...
		return rungeKuta4(scope, SEIRinit, new SEIR(alpha, beta, a, b, d, N), h);
	}
	...
	public GamaList<Double> kr4User(final IScope scope) throws GamaRuntimeException {
		...
		HashMap<String, String> equationsVar =
			(HashMap<String, String>) scope.getArg("equations", IType.MAP);
		initVal = (GamaList<Double>) scope.getArg("value", IType.LIST);
		param = (GamaMap) scope.getArg("param", IType.MAP);
		h = (Double) scope.getArg("h", IType.FLOAT);
		...
		UserEDPSystem systEDP = new UserEDPSystem(scope, equations, varName, param);

		return rungeKuta4(scope, initVal, systEDP, h);
	}
	...
	public GamaList<Double> kr4UserIterated(final IScope scope) throws GamaRuntimeException {
		...
		for ( int i = 0; i < nbSteps; i++ ) {
			intermVal = rungeKuta4(scope, intermVal, systEDP, h);
		}

		return intermVal;
	}

	GamaList<Double> rungeKuta4(final IScope scope, final GamaList<Double> init,
		final SystemEDP edp, final double h) {
		...
		for ( int i = 0; i <= numEq - 1; i++ ) {
			edp_temp.set(i, init.get(i) + h / 6.0 *
				(Q1.get(i) + 2.0 * Q2.get(i) + 2.0 * Q3.get(i) + Q4.get(i)));
		}
