...
@vars({ 
	@var(name = "considering_range", type = IType.INT_STR, init = "30"),
	@var(name = "isCalculatedPerimeter", type = IType.BOOL_STR, init = "false"),
	@var(name = "isPassedFalseTarget", type = IType.BOOL_STR, init = "false"),
	@var(name = "currentPerimeter", type = IType.FLOAT_STR, init = "0.0"),
	@var(name = "currentDistance", type = IType.FLOAT_STR, init = "0.0"),
	@var(name = "obstacle_species", type = IType.LIST_STR, init = "[]"),
	@var(name = "background_species", type = IType.LIST_STR, init = "[]"),
...
public class DrivingSkill2d extends MovingSkill{
	...
	protected boolean computeIsCalculatedPerimeter(final IScope scope, final IAgent agent) 
		throws GamaRuntimeException {
		return scope.hasArg("isCalculatedPerimeter") ? scope.getBoolArg("isCalculatedPerimeter") : getIsCalculatedPerimeter(agent);
	}
	...
	@action(name = "vehicle_goto", args = {
			...
			@arg(name = "background", type = { IType.LIST_STR, IType.AGENT_STR, IType.GRAPH_STR, IType.GEOM_STR }, optional = false, doc = @doc("list, agent, graph, geometry on which the agent moves (the agent moves inside this geometry)")),
	...
	public Integer primVehicleGoto(final IScope scope) throws GamaRuntimeException{
		...
		if ( path == null ) {
			scope.setStatus(ExecutionStatus.failure);
			System.out.println("M: DrivingSKill - can not get the path");
			return -1;
		}
		...
		if (hasBackground){
			for (ISpecies currentSpecy:backgroundSpecies){
				IList<IAgent> overlappingBackgroundWithCurrentPosition = msi.gaml.operators.Spatial.Queries.overlapping(scope, currentSpecy, agentShape);
				for (IAgent ia:overlappingBackgroundWithCurrentPosition){
					if (consideringBackgroundAgentForCurrentPosition == null)
						...
						consideringBackgroundAgentForCurrentPosition = msi.gaml.operators.Spatial.Operators.union(consideringBackgroundAgentForCurrentPosition, ia.getGeometry());
				}
			}
			maxDist = distanceToNearestInFront(scope, agent, obstacleAgents, 45, consideringBackgroundAgentForCurrentPosition, consideringRange, maxDist);
		}
		else{
		...
		for (int i=-4; i<=4; i++){
			...
			pointToAdd = new GamaPoint(currentLocation.x + maxDist*Maths.cos(candidateHeading), currentLocation.y + maxDist*Maths.sin(candidateHeading));
			//System.out.println("heading: " + currentHeadingAngle + " | " + " candidate angle: " + candidateAngle);
			//System.out.println("current location: " + currentLocation.toString() + " | " + " candidate location: " + pointToAdd.toString());
			GamaShape candidateShape = (GamaShape) msi.gaml.operators.Spatial.Transformations.rotated_by(scope, agentShape, dAngle);
			...
			if (isNonOverlapping(scope, candidateShape, obstacleAgents)){
				if (hasBackground){
					...
					for (ISpecies currentSpecy:backgroundSpecies){
						IList<IAgent> overlappingBackgroundWithCandidatePosition = msi.gaml.operators.Spatial.Queries.overlapping(scope, currentSpecy, candidateShape);
						for (IAgent ia:overlappingBackgroundWithCandidatePosition){
							if (consideringBackgroundAgent == null)
								...
								consideringBackgroundAgent = msi.gaml.operators.Spatial.Operators.union(consideringBackgroundAgent, ia.getGeometry());
						}
					}
					if (consideringBackgroundAgent != null){
						//System.out.println(((GamaShape)consideringBackgroundAgent).getPoints().toString());
						if (msi.gaml.operators.Spatial.Properties.covered_by(candidateShape, consideringBackgroundAgent)){
							CandidateEntry candidateEntry = new CandidateEntry(pointToAdd, candidateHeading, dAngle);
		...
		if (candidateEntries.size() > 0){
			...
			for (CandidateEntry currenEntry:candidateEntries){
				...
				double candidateDistance = topo.distanceBetween(scope, currentPoint.getLocation(), falseTarget);
			...
			//System.out.println("distance: "+ minDistance + ", location: " + chosenCandidate.toString());
			...
			candidateHeading = Maths.checkHeading(candidateHeading);
			GamaPoint chosenCandidate = new GamaPoint(currentLocation.x + maxDist*Maths.cos(candidateHeading), currentLocation.y + maxDist*Maths.sin(candidateHeading));
			GamaShape candidateShape = (GamaShape) msi.gaml.operators.Spatial.Transformations.at_location(scope, agentShape, chosenCandidate);
			if (isNonOverlapping(scope, candidateShape, obstacleAgents)){
	...
	private boolean isNonOverlapping(final IScope scope, GamaShape candidateShape, GamaList<IAgent> obstacleAgents){
		for (IAgent ia: obstacleAgents){
			if (msi.gaml.operators.Spatial.Properties.overlaps(scope, candidateShape, ia.getGeometry()))
				return false;
		}
		return true;
	}
	...
	private boolean isReachedTarget(final IScope scope, final IAgent agent, final ILocation goal){
		...
		double maxDist = computeDistance(scope, agent);
		if (currentPerimeter < maxDist) currentPerimeter = maxDist;
		double distanceToTarget = agent.euclidianDistanceTo(goal);
		return (distanceToTarget <= (currentPerimeter));
	}
	
	private double distanceToNearestInFront(IScope scope, final IAgent agent, GamaList<IAgent> obstacleAgents, int dAngle, IShape consideringBackgroundAgentForCurrentPosition, int consideringRange, double minDistance){
		...
		GamaPoint headPoint = new GamaPoint(currentPointX + agentRadius*Maths.cos(currentHeading), currentPointY + agentRadius*Maths.sin(currentHeading));		
		...
		try{
			...
			System.out.println("obstacle " + e.getStackTrace());
		}
		...
		try{
			if (!isFoundObstacleInFront){
				if (consideringBackgroundAgentForCurrentPosition != null){
					...
					if (gl != null){
						IShape interShape = msi.gaml.operators.Spatial.Operators.inter(gl, consideringBackgroundAgentForCurrentPosition);
						if (interShape != null){
							...
							for (int i=0; i<coords.length; i++){
								...
								if ((borderDistance>0.1) && (borderDistance < minDistance)) minDistance = borderDistance;
							}
						}
					}
				}
			...
			System.out.println(agent.getName() + " background ");
		}
		if (minDistance < 0.5) minDistance = 0.5;
		return minDistance;
	}
	@action(name = "read_replay_file", args = {
			@arg(name = "file_name", type = IType.STRING_STR, optional = false, doc = @doc("File name."))})
	...
	@action(name = "save_replay", args = {
			@arg(name = "file_name", type = IType.STRING_STR, optional = false, doc = @doc("File name.")),
			@arg(name = "agent_information", type = { IType.LIST_STR }, optional = false, doc = @doc("list of position"))
			})
	...
	public Object primSaveReplayStep(final IScope scope) throws GamaRuntimeException{
		...
		System.out.println("2: " + agent_information.size());
		String currentFileName = scope.getSimulationScope().getModel().getRelativeFilePath(Cast.asString(scope, "."), true) + "/log/" + fileName;
		System.out.println("3: " + currentFileName);
		try{
			//File file = new File(fileName);
			//if (!file.exists() || !file.isFile()) return null;
