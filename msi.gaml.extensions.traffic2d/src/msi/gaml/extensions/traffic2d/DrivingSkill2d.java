...
public class DrivingSkill2d extends MovingSkill{
	...
	public Integer primVehicleGoto(final IScope scope) throws GamaRuntimeException{
		...
		final boolean hasBackground =  ((backgroundSpecies != null) && (backgroundSpecies.length() > 0));
		...
		if (hasBackground){
			for (ISpecies currentSpecy:backgroundSpecies){
				IList<IAgent> overlappingBackgroundWithCurrentPosition = msi.gaml.operators.Spatial.Queries.opOverlapping(scope, currentSpecy, agentShape);
				for (IAgent ia:overlappingBackgroundWithCurrentPosition){
					if (consideringBackgroundAgentForCurrentPosition == null)
						...
						consideringBackgroundAgentForCurrentPosition = msi.gaml.operators.Spatial.Operators.opUnion(consideringBackgroundAgentForCurrentPosition, ia.getGeometry());
				}
			}
			maxDist = distanceToNearestInFront(agent, obstacleAgents, 45, consideringBackgroundAgentForCurrentPosition, consideringRange, maxDist);
		}
		else{
			maxDist = distanceToNearestInFront(agent, obstacleAgents, 45, null, consideringRange, maxDist);
		}
		...
		for (int i=-4; i<=4; i++){
			...
			GamaShape candidateShape = (GamaShape) msi.gaml.operators.Spatial.Transformations.primRotation(agentShape, dAngle);
			...
			if (isNonOverlapping(scope, candidateShape, obstacleAgents)){
				if (hasBackground){
					...
					for (ISpecies currentSpecy:backgroundSpecies){
						IList<IAgent> overlappingBackgroundWithCandidatePosition = msi.gaml.operators.Spatial.Queries.opOverlapping(scope, currentSpecy, candidateShape);
						for (IAgent ia:overlappingBackgroundWithCandidatePosition){
							if (consideringBackgroundAgent == null)
								...
								consideringBackgroundAgent = msi.gaml.operators.Spatial.Operators.opUnion(consideringBackgroundAgent, ia.getGeometry());
						}
					}
					if (consideringBackgroundAgent != null){
						//System.out.println(((GamaShape)consideringBackgroundAgent).getPoints().toString());
						if (msi.gaml.operators.Spatial.Properties.opCoveredBy(candidateShape, consideringBackgroundAgent)){
		...
		if (candidateEntries.size() > 0){
			...
			for (CandidateEntry currenEntry:candidateEntries){
				...
				double candidateDistance = topo.distanceBetween(currentPoint.getLocation(), falseTarget);
			...
			if (chosenCandidate == null){
				chosenCandidate = candidateEntries.any();
			}
			...
			agent.setGeometry(msi.gaml.operators.Spatial.Transformations.primRotation(agent, newRotateAngle));
			...
			GamaShape candidateShape = (GamaShape) msi.gaml.operators.Spatial.Transformations.primTranslationTo(agentShape, chosenCandidate);
			if (isNonOverlapping(scope, candidateShape, obstacleAgents)){
	...
	private boolean isNonOverlapping(final IScope scope, GamaShape candidateShape, GamaList<IAgent> obstacleAgents){
		for (IAgent ia: obstacleAgents){
			if (msi.gaml.operators.Spatial.Properties.opOverlaps(scope, candidateShape, ia.getGeometry()))
				return false;
		}
		return true;
	}
	...
	private double distanceToNearestInFront(final IAgent agent, GamaList<IAgent> obstacleAgents, int dAngle, IShape consideringBackgroundAgentForCurrentPosition, int consideringRange, double minDistance){
		...
		try{
			if ((obstacleAgents != null) && (obstacleAgents.length() > 0)){
				for ( IAgent ia : obstacleAgents ) {
		...
		try{
			if (!isFoundObstacleInFront){
				if (consideringBackgroundAgentForCurrentPosition != null){
					...
					if (gl != null){
						IShape interShape = msi.gaml.operators.Spatial.Operators.opInter(gl, consideringBackgroundAgentForCurrentPosition);
						if (interShape != null){
