...
@vars({ @var(name = "considering_range", type = IType.INT_STR, init = "30"),
...
public class DrivingSkill2d extends MovingSkill {
	...
	@action(name = "vehicle_goto", args = {
		...
		@arg(name = "background", type = { IType.LIST_STR, IType.AGENT_STR, IType.GRAPH_STR,
			IType.GEOM_STR }, optional = false, doc = @doc("list, agent, graph, geometry on which the agent moves (the agent moves inside this geometry)")),
	...
	private double distanceToNearestInFront(final IScope scope, final IAgent agent,
		...
		final IShape consideringBackgroundAgentForCurrentPosition, final int consideringRange,
		double minDistance) {
