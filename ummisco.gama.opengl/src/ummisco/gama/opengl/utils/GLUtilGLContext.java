...
public class GLUtilGLContext {
	...
	public static void TranslateContext(GL2 gl,double[] loc, LayeredDisplayData data) {
		// loc in opengl coordinates (y already multiplied by Y_FLAG)
		gl.glTranslated(loc[0], loc[1], loc[2]);
		//float[] locArr = new float[] {(float) loc.x, (float) (JOGLRenderer.Y_FLAG * loc.y), (float) loc.z};
		// FIXME (julien 18/05/16) : does not work for draw shape with facet rotate. Decomment the following line should work, but it does not...
		//GLUtilLight.NotifyOpenGLTranslation(gl, new double[] {loc[0], JOGLRenderer.Y_FLAG * loc[1], loc[2]}, data.getDiffuseLights(), data);
	}
	
	public static void RotateContext(GL2 gl,double[] axis, double angle, LayeredDisplayData data) {
		// loc in opengl coordinates (y already multiplied by Y_FLAG)
