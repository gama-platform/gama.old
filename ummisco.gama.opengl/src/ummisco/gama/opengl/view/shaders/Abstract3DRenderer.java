...
import ummisco.gama.opengl.SWTOpenGLDisplaySurface;
import ummisco.gama.opengl.camera.CameraArcBall;
import ummisco.gama.opengl.camera.FreeFlyCamera;
import ummisco.gama.opengl.camera.ICamera;
import ummisco.gama.opengl.renderer.IKeystoneState;
import ummisco.gama.opengl.renderer.PickingHelper;
...
public abstract class Abstract3DRenderer extends AbstractDisplayGraphics implements GLEventListener {
	...
	protected final PickingHelper pickingState = new PickingHelper(this);
	...
	public PickingHelper getPickingState() {
		return pickingState;
	}
	...
	public void drawScene() {
		...
		if (pickingState.isBeginningPicking()) {
			pickingState.beginPicking(gl, openGL, camera);
			currentScene.draw(openGL);
			pickingState.endPicking(gl, openGL);
		}
	...
	public void beginDrawingLayer(final ILayer layer) {
		...
		if (!layer.isOverlay()) {
			...
			currentScale = new GamaPoint(getLayerWidth() / getWidth(), getLayerHeight() / getHeight(), zScale);
		} else {
