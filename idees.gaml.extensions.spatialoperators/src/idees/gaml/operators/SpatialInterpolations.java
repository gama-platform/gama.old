...
public class SpatialInterpolations {
	...
	@doc(value = "Inverse Distance Weighting (IDW) is a type of deterministic method for multivariate " +
		...
		examples = { @example(value = "IDW([ag1, ag2, ag3, ag4, ag5],[{10,10}::25.0, {10,80}::10.0, {100,10}::15.0], 2)",
			...
			isExecutable = false) })
	public static
		GamaMap<IShape, Double> primIDW(final IScope scope,
				final IContainer<?, ? extends IShape> geometries, final GamaMap<GamaPoint, Double> points, final int power) {
		GamaMap<IShape, Double> results = new GamaMap<IShape, Double>();
