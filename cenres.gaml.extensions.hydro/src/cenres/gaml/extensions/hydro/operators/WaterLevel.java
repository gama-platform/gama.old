...
public class WaterLevel {
	...
	@doc(special_cases = { "if the left operand is a polyline and the right operand a float for the area, returrns the y coordinate of the water (water level)" },
		examples = { @example(value = "waterlevel <- my_river_polyline water_level_for my_area_value",
			isExecutable = false) })
	public static
		Double opWaterLevel(final IScope scope, final IShape shape, final Double val) {
		if ( shape == null || val == null ) { return null; }
		return WaterLevelUtils.heigth(
	...
	@doc(special_cases = { "if the left operand is a polyline and the right operand a float for the water y coordinate, returrns the area of the water (water flow area)" },
		examples = { @example(value = "waterarea <- my_river_polyline water_area_for my_height_value",
			isExecutable = false) })
	public static
		Double opWaterArea(final IScope scope, final IShape shape, final Double val) {
		if ( shape == null || val == null ) { return null; }
		return WaterLevelUtils.area(
			GamaListFactory.create(scope, Types.POINT, shape.getInnerGeometry().getCoordinates()), val);
	}
	...
	@doc(special_cases = { "if the left operand is a polyline and the right operand a float for the water y coordinate, returrns the shapes of the river sections (list of list of points)" },
		examples = { @example(value = "waterarea <- my_river_polyline water_area_for my_height_value",
			isExecutable = false) })
	public static
		IList<IList<GamaPoint>> opWaterPolylines(final IScope scope, final IShape shape, final Double val) {
		if ( shape == null || val == null ) { return null; }
		return WaterLevelUtils.areaPolylines(
			GamaListFactory.create(scope, Types.POINT, shape.getInnerGeometry().getCoordinates()), val);
	}
}
