...
public class GPUGraphOperators {
	...
	@operator(value = "GPU_path_between", content_type = ITypeProvider.FIRST_CONTENT_TYPE, category={IOperatorCategory.GRAPH, IOperatorCategory.PATH})
	@doc(value = "The shortest path between a list of two objects in a graph computed with GPU", examples = { @example(value="my_graph GPU_path_between (ag1:: ag2)", equals="A path between ag1 and ag2", isExecutable=false) })
	public static IPath GPUpath_between(final IScope scope, final GamaGraph graph, final IShape source, final IShape target)
		throws GamaRuntimeException {
		return path_between_GPU(scope, graph, source, target);
	}
	
	@operator(value = "CPU_path_between", content_type = ITypeProvider.FIRST_CONTENT_TYPE, category={IOperatorCategory.GRAPH, IOperatorCategory.PATH})
	@doc(value = "The shortest path between a list of two objects in a graph computed with CPU", examples = { @example(value="my_graph CPU_path_between (ag1:: ag2)", equals="A path between ag1 and ag2", isExecutable=false) })
	public static IPath CPUpath_between(final IScope scope, final GamaGraph graph, final IShape source, final IShape target)
		throws GamaRuntimeException {
		return path_between_CPU(scope, graph, source, target);
	}
	
	static Graph toGPUGraph(GamaGraph graph) {
		...
		int i = 0;
		int k = 0;
		GamaMap<Object, Integer> vertexId = new GamaMap<Object, Integer>();
		int id = 0;
		...
		for (Object v : graph.getVertices()) {
			...
			for (Object e: edgs) {
				if (graph.isDirected()) {
					...
					if (tt == v) tt = graph.getEdgeSource(e);
	...
	static IPath path_between_GPU(final IScope scope, final GamaGraph graph, final IShape source,  final IShape target) {
		...
		if (edges == null) {
			...
			edges = graph.isSaveComputedShortestPaths() ?
					graph.savePaths( M,(GamaList) graph.getVertices(), graph.getVertices().size(), source, idS, idT) :
					graph.getPath( M,(GamaList) graph.getVertices(), graph.getVertices().size(), source,target, idS, vertexId.get(target))	; 
