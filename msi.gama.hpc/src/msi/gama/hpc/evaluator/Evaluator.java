 ...
 * 
 *
 * 'Evaluator.java', in plugin 'msi.gama.hpc', is part of the source code of the 
...
public class Evaluator {
	...
	public Evaluator(final String function, final ArrayList<String> variables, final String file,
		final short fitnessCombination) {
		super();
	...
	public double evaluate() {
		...
		try {
			...
			for ( int i = 0; i < listChar.size(); i++ ) {
				...
				Double value =
					fitnessCombination == IExploration.C_MAX ? Collections.max(getValues(variables.get(i)))
	...
	public ArrayList<String> getCharacterVariableFromString(final String strFunc) {
	...
	public ArrayList<Double> getValues(final String variable) {
	...
	public ArrayList<String> getOutput(final String outputFile, final String variable) {

		String strDocInput = "doc('" + outputFile + "')";

		String strQuery =
			"for $x in" + strDocInput + "//Simulation/Step/Variable" + " where $x/@name='" + variable +
	...
	public static double calculateMean(final Collection values, final boolean includeNullAndNaN) {
		...
		if ( values == null ) { throw new IllegalArgumentException("Null 'values' argument."); }
		int count = 0;
		double total = 0.0;
		...
		while (iterator.hasNext()) {
			...
			if ( object == null ) {
				if ( includeNullAndNaN ) { return Double.NaN; }
			} else {
