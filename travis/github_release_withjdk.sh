...
for s in ${suff[@]}; 
do
    RELEASEFILES[$n]=$GITHUB_WORKSPACE"/gama-platform_1.9.1-1_amd64."$(echo $s | rev | cut -d "." -f 1 | rev)
		NEWFILES[$n]='GAMA_1.9.1_Linux'$s 
		n=$n+1
		RELEASEFILES[$n]=$GITHUB_WORKSPACE"/gama-platform-jdk_1.9.1-1_amd64."$(echo $s | rev | cut -d "." -f 1 | rev)
		NEWFILES[$n]='GAMA_1.9.1_Linux_with_JDK'$s
		n=$n+1
done
...
LK="https://api.github.com/repos/gama-platform/gama/releases"
  ...
  -d '{"tag_name": "'$RELEASE'", "name":"Alpha Version 1.9.1 ('$COMMIT')","body":"Alpha release for GAMA 1.9.1, which adds compatibility with JDK 17 . Please test and report issues","draft": false,"prerelease": true}' \
    "$LK"`
echo $RESULT	
