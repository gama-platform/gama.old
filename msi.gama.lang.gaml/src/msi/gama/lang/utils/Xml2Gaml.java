...
public class Xml2Gaml {
	...
	public String doConv(final Document doc) {
		...
		gaml.append("model " + model.getAttributeValue("name") + "\n" + "// gen by Xml2Gaml\n" +
			"import \"platform:/plugin/msi.gama.core/generated/std.gaml\"\n\n");
	...
	private void appendStatment(final Element statment) {
		...
		if ( markupName.equals("include") ) {
			gaml.append("import \"" + statment.getAttributeValue("file") + ".gaml\"\n");
			return;
		}
		...
		for ( final Object ofacet : statment.getAttributes() ) {
			...
			if ( !facet.getName().equals("name") && !facet.getName().equals("const") ) {
				appendFacet(facet);
			}
		}
		@SuppressWarnings("rawtypes")
	...
	private void appendFacet(final Attribute facet) {
		...
		if ( (isString && (name.equals("value") || name.equals("init")) || name.equals("parameter") || name
			.equals("category")) && value.indexOf('\'') < 0 ) {
	...
	private Document getDocumentJDOM(final String path2xml) throws JDOMException, IOException {
		final SAXBuilder sax = new SAXBuilder();
		final Document doc = sax.build(new File(path2xml));
		return doc;
	}

	private Document getDocumentJDOM(final InputStream in) throws JDOMException, IOException {
		final SAXBuilder sax = new SAXBuilder();
