/*******************************************************************************************************
 *
 * BinaryQueue.java, in ummisco.gama.serialize, is part of the source code of the GAMA modeling and simulation platform
 * (v.1.9.3).
 *
 ...
 *
 * poll* methods don't throw excpetions but return incomplete or no result instead if the queue's content is not
 * sufficient. read* methods throw excpetions in case the q contains not enough bytes (need to use available() to ensure
 * data is present).
 *
...
public class BinaryQueue {
	...
	/**
	 * Instantiates a new binary queue.
	 *
	 ...
	 * @date 29 sept. 2023
	 */
	public BinaryQueue(final int qsize) {
		storage = new HeapBytez(qsize);
	}

	/**
	 * add bytes to the queue. Again by using (reusable) Wrapper classes any kind of memory (offheap, byte arrays, nio
	 * bytebuffer, memory mapped) can be added.
	 *
	 * @param source
	 */
	public void add(final ByteSource source) {
	 ...
	 * @param sourcelen
	 */
	public void add(final ByteSource source, final long sourceoff, final long sourcelen) {
		...
		for (int i = 0; i < sourcelen; i++) {
			...
			if (addIndex >= storage.length()) { addIndex -= storage.length(); }
		}
	}
	 ...
	 * @date 29 sept. 2023
	 */
	public void addInt(final int written) {
	 ...
	 * @date 29 sept. 2023
	 */
	protected void grow(final long sourcelen) {
	...
	/**
	 * read up to destlen bytes (if available). Note you can use HeapBytez (implements ByteSink) in order to read to a
	 * regular byte array. HeapBytez wrapper can be reused to avoid unnecessary allocation. Also possible is
	 * ByteBufferBasicBytes to read into a ByteBuffer and MallocBytes or MMFBytes to read into Unsafe alloc'ed off heap
	 * memory or persistent mem mapped memory regions.
	 *
	 * @param destination
	 * @param destoff
	 * @param destlen
	 * @return
	 */
	public long poll(final ByteSink destination, final long destoff, final long destlen) {
		long count = 0;
		try {
	 ...
	 * @param len
	 */
	public void back(final int len) {
		if (pollIndex >= len) {
			pollIndex -= len;
		} else {
	...
	public String toString() {
		return "BinaryQueue{" + "storage=" + storage + ", addIndex=" + addIndex + ", pollIndex=" + pollIndex + '}';
	}
}
