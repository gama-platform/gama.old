...
public class JOGLAWTGLRenderer implements GLEventListener {
	...
	public void init(final GLAutoDrawable drawable) {
		GuiUtils.debug("JOGLAWTGLRenderer.init");
		startTime = System.currentTimeMillis();
		width = drawable.getWidth();
		height = drawable.getHeight();
		gl = drawable.getGL().getGL2();
		GuiUtils.debug("GL_VENDOR: " + gl.glGetString(GL.GL_VENDOR));
		GuiUtils.debug("GL_RENDERER: " + gl.glGetString(GL.GL_RENDERER));
		GuiUtils.debug("GL_VERSION: " + gl.glGetString(GL.GL_VERSION));
		glu = new GLU();
		glut = new GLUT();
	...
	public void display(final GLAutoDrawable drawable) {
		...
		if ( !displaySurface.isPaused() && GAMA.getSimulation() != null ) {
			GuiUtils.debug("JOGLAWTGLRenderer.display");
			gl = drawable.getGL().getGL2();
			setContext(drawable.getContext());
	...
	public void reshape(final GLAutoDrawable drawable, final int x, final int y, final int width, final int height) {
		GuiUtils.debug("JOGLAWTGLRenderer.reshape x=" + x + " y=" + y + " width=" + width + " height=" + height);
	...
	public MyTexture createTexture(final BufferedImage image, final boolean isDynamic, final int index) {
		...
		Texture texture;
		try {
			texture = AWTTextureIO.newTexture(profile, image, false /* true for mipmapping */);
		} catch (final GLException e) {
			return null;
		}
		// FIXME:need to see the effect of this bending
		gl.glBindTexture(GL.GL_TEXTURE_2D, index);
		texture.setTexParameteri(gl, GL_TEXTURE_MIN_FILTER, minAntiAliasing);
		texture.setTexParameteri(gl, GL_TEXTURE_MAG_FILTER, magAntiAliasing);
	...
	public Point2D.Double getWindowPointPointFromRealWorld(final Point realWorldPoint) {
		...
		glu.gluProject(realWorldPoint.x, realWorldPoint.y, 0d, model.array(), model.arrayOffset(), proj.array(),
	...
	private void drawROI(final GL2 gl, final double x1, final double y1, final double x2, final double y2,
