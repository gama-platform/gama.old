...
public abstract class AbstractCamera implements ICamera {
	...
	public final void updateCamera(final GL gl, final GLU glu, final int width, final int height) {
		...
		if(!this.getRenderer().getOrtho()){
			...
			if (aspect >= 1.0){
				((GL2ES1) gl).glOrtho(-getMaxDim()*aspect, getMaxDim()*aspect, -getMaxDim(), getMaxDim(), getMaxDim(), -getMaxDim());	
			}			
			else{
				((GL2ES1) gl).glOrtho(-getMaxDim(), getMaxDim(), -getMaxDim()/aspect, getMaxDim()/aspect, getMaxDim(), -getMaxDim());	
			}
			((GL2) gl).glTranslated(0.0, 0.0, getMaxDim()*1.5);
		}
		makeGluLookAt(glu);
		animate();
	}
