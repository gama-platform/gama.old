...
public class GLUtil {
	...
	public static void DrawColorTriangle(final GL2 gl, final float x, final float y, final float z, final float alpha,
		final float size) {
		// ----- Render a triangle -----
	...
	public static void DrawSphere(final GL2 gl, final GLU glu, final double radius) {
	...
	public static void InitializeLighting(final GL2 gl, final GLU glu, final float widthEnv, final float heightEnv,
		...
		gl.glColorMaterial(GL_FRONT, GLLightingFunc.GL_AMBIENT_AND_DIFFUSE);
	...
	public static void SetAmbiantLightFromValue(final GL2 gl, final GLU glu, final Color c) {
		float[] lightAmbientValue =
			{ (float) c.getRed() / 255, (float) c.getGreen() / 255, (float) c.getBlue() / 255, 1.0f };
		gl.glLightfv(GLLightingFunc.GL_LIGHT1, GLLightingFunc.GL_AMBIENT, lightAmbientValue, 0);
	}

	public static void UpdateAmbiantLight(final GL2 gl, final GLU glu, final Color ambiantLightValue) {
	...
	public static void UpdateDiffuseLight(final GL2 gl, final GLU glu, final Color diffuseLightValue) {
	...
	public static void InitializeLighting2(final GL2 gl) {
	...
	public static void setLineWidth(final GL2 gl, final float size, final boolean smooth) {
		gl.glLineWidth(size);
		if ( smooth ) {
	 ...
	 */
	public static void TcreateTexture2Dmipmap(final GL2 gl, final int texIDs[], final int texture[][], final int texW,
		...
		for ( int i = 0; i < texIDs.length; i++ ) {
			...
			gl.glTexEnvf(GL2ES1.GL_TEXTURE_ENV, GL.GL_TEXTURE, GL2ES1.GL_MODULATE);
	 ...
	 * @return - texture ID
	 */
	public static int TcreatTexture2DFromImage(final GL2 gl, final URL url, final boolean gluMipMaps) {
	...
	public static void HandleNormal(final Vertex[] vertices, final Color c, final double alpha, final int norm_dir,
		...
		if ( renderer.getDrawNorm() ) {
			...
			if ( c != null ) {
				renderer.gl.glColor4ub((byte) c.getRed(), (byte) c.getGreen(), (byte) c.getBlue(),
					(byte) (alpha * c.getAlpha()));
			}
